...
export const SalesReturnList = () => {
    ...
    const filteredReturns = returns.filter(ret =>
        (ret.return_no && ret.return_no.toLowerCase().includes(searchTerm.toLowerCase())) ||
    ...
    return (
        <div className="p-6 space-y-6 bg-slate-50 min-h-screen font-sans" dir="rtl">
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">مرتجعات المبيعات</h1>
                    <p className="text-slate-500 text-sm mt-1">إدارة مرتجعات المبيعات (إشعارات دائنة)</p>
                </div>
                <Button onClick={() => navigate('/trade/sales/return/new')} className="bg-blue-600 hover:bg-blue-700 gap-2">
                        ...
                        <table className="w-full text-right">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filteredReturns.map((ret) => (
                                    <tr key={ret.id} className="hover:bg-blue-50/50 transition-colors group cursor-pointer" onClick={() => navigate(`/trade/sales/return/${ret.id}`)}>
                                        <td className="px-4 py-3 font-mono font-bold text-blue-600 flex items-center gap-2">
                                            <FileText className="w-4 h-4" />
                                            {ret.return_no}
                                        </td>
                                        <td className="px-4 py-3 text-slate-600">{new Date(ret.date).toLocaleDateString('en-GB')}</td>
                                        <td className="px-4 py-3 font-medium">{ret.customer_name || '-'}</td>
                                        <td className="px-4 py-3 text-center font-bold font-mono">
                                            {Number(ret.grand_total || 0).toLocaleString()}
                                        </td>
                                        <td className="px-4 py-3 text-center">
                                            <Badge className={ret.status === 'POSTED' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-700'}>
