...
const ImportInvoiceList = () => {
    ...
    const loadData = async () => {
        try {
            ...
            if (api && api.getAllCommercialInvoices) {
                const data = await api.getAllCommercialInvoices();
                setInvoices(data || []);
            } else {
                // Fallback or dev mode
                try {
                    // Try to fetch manually if new API not exposed yet (should be after restart)
                    // But for now just handle array
                    setInvoices([]);
                } catch (e) { console.error(e) }
            }
        } catch (error) {
            console.error("Failed to load import invoices", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="p-6 space-y-6 bg-slate-50 min-h-screen font-sans" dir="rtl">
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">فواتير الاستيراد (Commercial Invoices)</h1>
                    <p className="text-slate-500 text-sm mt-1">إدارة الفواتير التجارية للاستيراد</p>
                </div>
                <Button onClick={() => navigate('/import/invoice')} className="bg-blue-600 hover:bg-blue-700 gap-2">
                        ...
                        <table className="w-full text-right">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filteredInvoices.map((inv) => (
                                    <tr key={inv.id} className="hover:bg-blue-50/50 transition-colors group cursor-pointer" onClick={() => navigate(`/import/invoice/${inv.id}`)}>
                                        ...
                                        <td className="px-4 py-3 text-slate-500 text-sm">
                                            {inv.shipment_no ? (
                                                <span className="bg-slate-100 px-2 py-1 rounded text-xs">{inv.shipment_no}</span>
                                            ) : '-'}
                                        </td>
                                        <td className="px-4 py-3 text-center font-bold font-mono">
                                            {Number(inv.total_amount).toLocaleString()} {inv.currency_id}
                                        </td>
