...
import { Plus, Banknote, FileText, Search, Filter, Calendar } from 'lucide-react';
...
export const ReceiptVoucherList = () => {
    ...
    const filteredVouchers = vouchers.filter(v =>
        ...
        (v.payer_name && v.payer_name.toLowerCase().includes(searchTerm.toLowerCase())) ||
        ...
        <div className="p-6 bg-[#f8fafc] h-full flex flex-col gap-6" dir="rtl">
            ...
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-slate-800">سندات القبض</h1>
                        <p className="text-xs text-slate-500">إدارة المقبوضات النقدية والشيكات</p>
                    </div>
                </div>
                <button
                    ...
                    <table className="w-full text-right">
                        <thead className="bg-slate-50 text-slate-600 font-bold text-xs sticky top-0 z-10">
                            <tr>
                                ...
                                <th className="p-3 border-b border-slate-100">المستلم منه</th>
                            ...
                            {loading ? (
                                ...
                                filteredVouchers.map(v => (
                                    <tr
                                        ...
                                        onClick={() => navigateInTab(`/treasury/receipt/${v.id}`, `سند قبض ${v.voucher_no}`)}
                                    >
                                        <td className="p-3 font-mono text-emerald-600 font-bold group-hover:text-emerald-700">
                                            {v.voucher_no}
                                        </td>
                                        <td className="p-3">
                                            <span className={`px-2 py-1 rounded text-[10px] font-bold 
                                                ${v.status === 'POSTED' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}
                                            `}>
                                                {v.status === 'POSTED' ? 'مرحل' : v.status}
                                            </span>
                                        </td>
                                        <td className="p-3 font-medium text-slate-700">{v.payer_name || '-'}</td>
