...
export const SectorsPage: React.FC = () => {
    ...
    const loadRows = async () => {
        try {
            setLoading(true);
            const data = await window.electronAPI.partner.getSectors();
            setRows(data || []);
        } catch (err) {
            console.error(err);
            setError('تعذر تحميل القطاعات');
        } finally {
            setLoading(false);
        }
    };
    ...
    const onSave = async (e: React.FormEvent) => {
        ...
        if (!form.name_ar.trim()) {
            setError('اسم القطاع بالعربية مطلوب');
            return;
        }
        ...
        try {
            ...
            if (editingId) {
                await window.electronAPI.partner.saveSector({ id: editingId, ...payload });
            } else {
                await window.electronAPI.partner.saveSector(payload);
            }
            setIsOpen(false);
            resetForm();
            await loadRows();
            ...
            console.error(err);
            setError(err?.message || 'فشل حفظ القطاع');
        } finally {
            setSaving(false);
        }
    };
    ...
    const onDelete = async (id: string) => {
        ...
        try {
            await window.electronAPI.partner.deleteSector(id);
            await loadRows();
        } catch (err: any) {
            alert(err?.message || 'تعذر حذف القطاع');
        }
    };
    ...
    return (
        <div className="app-page" dir="rtl">
                ...
                <div>
                    ...
                    <p className="text-gray-500 mt-1 mr-12">إدارة قطاعات العملاء</p>
                </div>
            ...
            <DefinitionMasterList
                ...
                headerTitle="تعريف القطاعات"
                headerSubtitle="إدارة قطاعات العملاء"
                headerBadges={[
                ...
                <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-xl overflow-hidden" dir="rtl">
                        <div className="p-4 border-b flex items-center justify-between">
                            <h2 className="font-bold text-lg text-gray-800">{editingId ? 'تعديل قطاع' : 'إضافة قطاع'}</h2>
