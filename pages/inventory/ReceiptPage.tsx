...
import { Save, Truck, Search, FileText, Calendar, MapPin, User, Package, Trash2, Download } from 'lucide-react';
...
export const ReceiptPage = () => {
    ...
    const [header, setHeader] = useState({
        ...
        source: '',      // Supplier or Source Name
        receivedBy: '',
        notes: '',
        ref_no: 'RCP-NEW'
    });
        ...
        try {
            ...
            if (window.electronAPI && window.electronAPI.inventory.saveTransaction) {
                ...
                await window.electronAPI.inventory.saveTransaction({
                    ...
                    date: header.date,
                    notes: `Receipt from: ${header.source} - ${header.notes}`,
                    items: lines.map(l => ({ itemId: l.itemId, quantity: l.quantity })) // Positive for add
                });
                alert("تم حفظ سند الاستلام بنجاح");
                setLines([]);
                setHeader(h => ({ ...h, source: '', notes: '', receivedBy: '' }));
            } else {
        ...
        <div className="flex flex-col h-full bg-slate-50 relative animate-in fade-in duration-300">
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-lg font-bold text-slate-800">سند استلام</h1>
                        <p className="text-xs text-slate-500">Receipt Note (Stock In)</p>
                    </div>
                </div>
                <div className="flex gap-2">
                    <button
                        onClick={handleSave}
                        disabled={loading}
                        className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg font-medium transition-colors shadow-sm disabled:opacity-50"
                    >
                        <Save size={18} />
                        <span>حفظ السند</span>
                    </button>
                ...
                <div className="max-w-6xl mx-auto space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div>
                                <label className="block text-sm font-medium text-slate-600 mb-1.5">المستودع (المستلم)</label>
                                <select
                                    value={header.warehouseId}
                                    onChange={e => setHeader({ ...header, warehouseId: e.target.value })}
                                    className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none"
                                >
                                    {warehouses.map(w => <option key={w.id} value={w.id}>{w.name || w.name_ar}</option>)}
                                </select>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-600 mb-1.5">تاريخ الاستلام</label>
                                <div className="relative">
                                    <input
                                        type="date"
                                        value={header.date}
                                        onChange={e => setHeader({ ...header, date: e.target.value })}
                                        className="w-full pl-3 pr-10 py-2.5 border border-slate-200 rounded-lg focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none"
                                    />
                                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                                </div>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-600 mb-1.5">المصدر / المورد</label>
                                <div className="relative">
                                    <input
                                        type="text"
                                        value={header.source}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-600 mb-1.5">تم الاستلام بواسطة</label>
                                <div className="relative">
                                    <input
                                        type="text"
                                        value={header.receivedBy}
                                        onChange={e => setHeader({ ...header, receivedBy: e.target.value })}
                            ...
                            <div className="lg:col-span-2">
                                ...
                                <input
                                    ...
                                    className="w-full p-2.5 border border-slate-200 rounded-lg focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none"
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col min-h-[400px]">
                        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-t-xl">
                            ...
                            <div className="relative">
                                {/* Search Input */}
                                <div className="flex items-center bg-white border border-slate-200 rounded-lg px-3 py-1.5 focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-100 w-64 shadow-sm transition-all">
                                ...
                                {showItemPicker && (
                                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-xl z-50 max-h-60 overflow-y-auto animate-in fade-in zoom-in-95 duration-100">
                                        {filteredItems.map(item => (
                                            ...
                                            >
                                                <span className="font-medium text-slate-700 group-hover:text-emerald-700">{item.name_ar}</span>
                                        ...
                                        <tr key={line.id} className="group hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-3">
                                                <input
                                                    ...
                                                    className="w-full p-1.5 border border-slate-200 rounded text-center focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none font-bold text-slate-800"
                                                />
                                            </td>
                                            <td className="px-6 py-3">
                                                <input
                                                    ...
                                                    placeholder="ملاحظة..."
                                                    className="w-full p-1.5 border border-slate-200 rounded text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none"
                                                />
                                            </td>
