...
import { Save, Plus, Trash2, Search, FileText, Calendar, MapPin, Truck } from 'lucide-react';
import { v4 as uuidv4 } from 'uuid';
import { Item, Warehouse } from '../../types';

// Local helper for line items if needed, or reuse Item
interface OrderLine {
    id: string;
    itemId: string;
    name: string;
...
export const InternalOrderPage = () => {
    ...
    const [header, setHeader] = useState({
        ...
        requiredDate: '',
        fromWarehouseId: '', // Requesting Warehouse (User's location)
        toWarehouseId: '',   // Supplying Warehouse (Main Store)
        notes: '',
        ref_no: 'ORD-NEW'
    });
        ...
        try {
            ...
            if (window.electronAPI) {
                ...
                if (whs && whs.length > 0) {
                    setHeader(h => ({
                        ...h,
                        fromWarehouseId: whs[0].id, // Default to first
                        toWarehouseId: whs.length > 1 ? whs[1].id : whs[0].id // Default to second if exists
                    }));
                }
            }
        } catch (err) {
    ...
    const handleSave = async () => {
        ...
        if (header.fromWarehouseId === header.toWarehouseId) {
            alert("لا يمكن أن يكون المستودع الطالب والمزود هو نفسه");
            // return; // Warning only?
        }
        if (lines.length === 0) {
        ...
        try {
            ...
            // Note: Using transfer request handler effectively for internal orders
            // @ts-ignore
            if (window.electronAPI && window.electronAPI.inventory.transferRequest) {
                // @ts-ignore
                const res = await window.electronAPI.inventory.transferRequest({
                    type: 'INTERNAL_ORDER',
                    from_warehouse_id: header.toWarehouseId, // From Source
                    to_warehouse_id: header.fromWarehouseId, // To Requester
                    date: header.date,
                    notes: header.notes,
                    items: lines.map(l => ({ item_id: l.itemId, quantity: l.quantity }))
                });
                alert("تم حفظ الطلبية بنجاح: " + (res.code || ''));
                setLines([]);
            } else {
                // Fallback simulation
                console.log("Saving Internal Order:", payload);
                alert("تم الحفظ (محاكاة) - لم يتم العثور على Handler");
            }
        } catch (err: any) {
        ...
        <div className="flex flex-col h-full bg-slate-50 relative animate-in fade-in duration-300">
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-lg font-bold text-slate-800">طلبية مستودع داخلية</h1>
                        <p className="text-xs text-slate-500">Internal Warehouse Order</p>
                    </div>
                </div>
                <div className="flex gap-2">
                    <button
                        onClick={handleSave}
                        disabled={loading}
                        className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg font-medium transition-colors shadow-sm disabled:opacity-50"
                    >
                        <Save size={18} />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            <div>
                                ...
                                <select
                                    ...
                                    className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-100 outline-none"
                                >
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    className="w-full p-2.5 bg-white border border-slate-200 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-100 outline-none"
                                >
                                    {warehouses.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
                                </select>
                            </div>
                            <div>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full pl-3 pr-10 py-2.5 border border-slate-200 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-100 outline-none"
                                    />
                            ...
                            <div className="lg:col-span-4">
                                ...
                                <input
                                    ...
                                    className="w-full p-2.5 border border-slate-200 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-100 outline-none"
                                />
                            </div>
                        </div>
                    </div>
                                ...
                                {showItemPicker && (
                                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-xl z-50 max-h-60 overflow-y-auto animate-in fade-in zoom-in-95 duration-100">
                                        {filteredItems.map(item => (
                                            <button
                                                key={item.id}
                                                className="w-full text-right px-4 py-2 hover:bg-purple-50 text-sm border-b border-slate-50 last:border-0 flex justify-between group"
                                                onClick={() => handleItemSelect(item)}
                                            >
                                                <span className="font-medium text-slate-700 group-hover:text-purple-700">{item.name_ar}</span>
                                        ...
                                        <tr key={line.id} className="group hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-3">
                                                <input
                                                    ...
                                                    className="w-full p-1.5 border border-slate-200 rounded text-center focus:border-purple-500 focus:ring-2 focus:ring-purple-100 outline-none font-bold text-slate-800"
                                                />
                                            </td>
                                            <td className="px-6 py-3">
                                                <input
                                                    ...
                                                    placeholder="ملاحظة..."
                                                    className="w-full p-1.5 border border-slate-200 rounded text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-100 outline-none"
                                                />
                                            </td>
