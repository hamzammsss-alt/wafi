...
exports.JournalPostingValidationService = void 0;
const Money_1 = require("../../valueObjects/Money");
class JournalPostingValidationService {
    constructor(accountsRepo, fiscalPeriodRepo, postingRegistryRepo) {
        this.accountsRepo = accountsRepo;
        this.fiscalPeriodRepo = fiscalPeriodRepo;
        this.postingRegistryRepo = postingRegistryRepo;
    }
    validate(command) {
        const issues = [];
        ...
        for (let index = 0; index < lines.length; index += 1) {
            ...
            const debit = Money_1.Money.round(Number(line.baseDebit ?? line.debit ?? 0));
            const credit = Money_1.Money.round(Number(line.baseCredit ?? line.credit ?? 0));
            totalDebit = Money_1.Money.round(totalDebit + debit);
            totalCredit = Money_1.Money.round(totalCredit + credit);
            if (debit > 0 && credit > 0) {
                issues.push({
        ...
        const duplicate = this.postingRegistryRepo.findBySource(command.companyId, command.sourceType, command.sourceId, sourceVersion);
        if (duplicate) {
            issues.push({
        ...
        if (Money_1.Money.round(totalDebit - totalCredit) !== 0) {
            issues.push({
        ...
        if (command.totalDebit != null && Money_1.Money.round(Number(command.totalDebit) - totalDebit) !== 0) {
            issues.push({
                code: 'ERR_HEADER_TOTAL_DEBIT_MISMATCH',
                message: 'Header total debit does not match line totals',
                details: {
                    headerTotalDebit: Money_1.Money.round(Number(command.totalDebit)),
                    lineTotalDebit: totalDebit,
                },
            });
        }
        if (command.totalCredit != null && Money_1.Money.round(Number(command.totalCredit) - totalCredit) !== 0) {
            issues.push({
                code: 'ERR_HEADER_TOTAL_CREDIT_MISMATCH',
                message: 'Header total credit does not match line totals',
                details: {
                    headerTotalCredit: Money_1.Money.round(Number(command.totalCredit)),
...
exports.JournalPostingValidationService = JournalPostingValidationService;
