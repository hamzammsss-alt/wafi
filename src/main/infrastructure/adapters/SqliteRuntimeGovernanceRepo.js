...
exports.SqliteRuntimeGovernanceRepo = void 0;
const GB = 1024 * 1024 * 1024;
class SqliteRuntimeGovernanceRepo {
    ...
    getConcurrencyLicense(companyId) {
        this.db.prepare(`
            INSERT OR IGNORE INTO sys_concurrency_license(company_id, base_seats, extra_seats)
            VALUES (?, 1, 0)
        `).run(companyId);
    ...
    updateExtraSeats(companyId, extraSeats) {
        this.db.prepare(`
    ...
    releaseSessionByWebContents(webContentsId) {
        this.db.prepare(`
            UPDATE sys_active_sessions
            SET is_active = 0,
                ended_at = CURRENT_TIMESTAMP
    ...
    updateAttachmentTier(companyId, tier) {
    ...
    addAttachmentAddon(companyId, addonGb) {
        this.db.prepare(`
            INSERT OR IGNORE INTO sys_attachment_quota(company_id, quota_tier, base_quota_bytes, addon_quota_bytes, used_quota_bytes)
            VALUES (?, 'BASE_5GB', ?, 0, 0)
        `).run(companyId, 5 * GB);
    ...
    increaseUsedQuota(companyId, sizeBytes) {
        this.db.prepare(`
            UPDATE sys_attachment_quota
    ...
    decreaseUsedQuota(companyId, sizeBytes) {
        this.db.prepare(`
            UPDATE sys_attachment_quota
    ...
    listAttachmentFiles(companyId, entityName, entityId) {
        return this.db.prepare(`
            SELECT *
            FROM sys_attachment_files
            WHERE company_id = ?
        ...
        `).all(companyId, entityName, entityId);
    }
    markAttachmentDeleted(fileId) {
        this.db.prepare(`
...
exports.SqliteRuntimeGovernanceRepo = SqliteRuntimeGovernanceRepo;
