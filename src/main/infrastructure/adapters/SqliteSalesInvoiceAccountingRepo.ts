...
import {
    ...
    SalesInvoiceCustomerRecord,
    SalesInvoiceHeaderRecord,
    SalesInvoiceItemMeta,
    SalesInvoiceLineRecord,
    SalesInvoicePostingStateRecord,
} from '../../application/ports/SalesInvoiceAccountingPorts';
...
export class SqliteSalesInvoiceAccountingRepo implements SalesInvoiceAccountingRepositoryPort {
    ...
    getInvoiceHeaderById(companyId: string, branchId: string, invoiceId: string): SalesInvoiceHeaderRecord | null {
        const row = this.db
            ...
            .get(invoiceId, companyId, branchId) as SalesInvoiceHeaderRow | undefined;
    ...
    getInvoiceLinesByInvoiceId(invoiceId: string): SalesInvoiceLineRecord[] {
        const rows = this.db
            ...
            .all(invoiceId) as SalesInvoiceLineRow[];
    ...
    getItemMeta(itemId: string, warehouseId: string | null): SalesInvoiceItemMeta {
        let itemRow:
            | { item_group_id?: string | null; item_type?: string | null; cost_price?: number | null }
            | undefined;
        try {
            itemRow = this.db
                .prepare(
                ...
                .get(itemId) as { item_group_id?: string | null; item_type?: string | null; cost_price?: number | null } | undefined;
        } catch {
            itemRow = undefined;
        }
    ...
    getPostingState(companyId: string, branchId: string, invoiceId: string): SalesInvoicePostingStateRecord | null {
        const row = this.db
            ...
            .get(invoiceId, companyId, branchId) as SalesInvoiceStateRow | undefined;
    ...
    private getWarehouseItemCost(itemId: string, warehouseId: string | null): number {
        ...
        try {
            const row = this.db
                ...
                .get(itemId, warehouseId) as { avg_cost?: number | null } | undefined;
    ...
    private ensureSalesInvoiceColumns(): void {
        const columns = this.db.prepare('PRAGMA table_info(sales_invoices)').all() as Array<{ name: string }>;
    ...
    private ensureSalesInvoiceLineColumns(): void {
        const columns = this.db.prepare('PRAGMA table_info(sales_invoice_lines)').all() as Array<{ name: string }>;
        const has = (name: string) => columns.some((column) => column.name === name);
