...
import {
    ...
    SaveTreasuryPostingStateInput,
    SaveTreasuryReversalStateInput,
    TreasuryAccountPostingState,
    TreasuryChequeEventRecord,
    TreasuryDocumentRepositoryPort,
    TreasuryPartnerRecord,
    TreasuryPostingStateRecord,
    UpdateChequeStateInput,
    UpdateTreasuryDocumentDbInput,
    UpsertDocumentChequeInput,
} from '../../application/ports/TreasuryPorts';

type TreasuryDocumentHeaderRow = {
    id: string;
...
type ChequeRow = {
    ...
    treasury_document_id: string | null;
    deposited_bank_account_id: string | null;
...
export class SqliteTreasuryRepo implements TreasuryDocumentRepositoryPort {
    ...
    createDocument(input: CreateTreasuryDocumentDbInput): TreasuryDocumentEntity {
        ...
        if (!document) {
            throw new DomainError('INTERNAL_ERROR', `Treasury document ${input.id} was not found after create`);
        }
    ...
    updateDocument(input: UpdateTreasuryDocumentDbInput): TreasuryDocumentEntity {
        ...
        const document = this.getDocumentById(input.companyId, input.branchId, input.id);
        if (!document) {
            throw new DomainError('INTERNAL_ERROR', `Treasury document ${input.id} was not found after update`);
        }
    ...
    getDocumentHeaderById(companyId: string, branchId: string, documentId: string): TreasuryDocumentHeaderEntity | null {
        const row = this.db.prepare(`
            ...
            LIMIT 1
        `).get(documentId, companyId, branchId) as TreasuryDocumentHeaderRow | undefined;
    ...
    getDocumentLinesByDocumentId(documentId: string): TreasuryDocumentLineEntity[] {
        ...
        `).all(documentId) as TreasuryDocumentLineRow[];
    ...
    getAccountPostingState(companyId: string, accountId: string): TreasuryAccountPostingState {
        const row = this.db.prepare(`
            ...
            WHERE id = ?
              AND COALESCE(company_id, 'COMP_01') = ?
            LIMIT 1
        `).get(accountId, companyId) as AccountRow | undefined;
        ...
        const isPosting =
            row.is_posting !== null && row.is_posting !== undefined
    ...
    getChequeById(companyId: string, chequeId: string): ChequeRegisterEntity | null {
        const row = this.db.prepare(`
            ...
            WHERE id = ?
              AND company_id = ?
            LIMIT 1
        `).get(chequeId, companyId) as ChequeRow | undefined;
    ...
    getChequeByDocumentId(companyId: string, documentId: string): ChequeRegisterEntity | null {
        const row = this.db.prepare(`
            ...
            LIMIT 1
        `).get(documentId, companyId) as ChequeRow | undefined;
        return row ? this.mapCheque(row) : null;
    }
    ...
    getChequeByNo(companyId: string, chequeNo: string, direction: ChequeDirection): ChequeRegisterEntity | null {
        const row = this.db.prepare(`
            ...
            ORDER BY updated_at DESC, created_at DESC
            LIMIT 1
        `).get(companyId, direction, String(chequeNo || '').trim().toUpperCase()) as ChequeRow | undefined;
        return row ? this.mapCheque(row) : null;
    }
    ...
    upsertDocumentCheque(input: UpsertDocumentChequeInput): ChequeRegisterEntity {
        ...
        if (existingByNo && (!existingByDocument || existingByNo.id !== existingByDocument.id)) {
            throw new DomainError('VALIDATION_ERROR', 'Cheque number already exists for active records', {
        ...
        if (existingByDocument) {
            ...
            if (!updated) {
                throw new DomainError('INTERNAL_ERROR', 'Cheque was not found after update', {
        ...
        const initialStatus: ChequeStatus = input.direction === 'RECEIVED' ? 'IN_SAFE' : 'ISSUED_PENDING';
        ...
        if (!inserted) {
            throw new DomainError('INTERNAL_ERROR', 'Cheque was not found after insert', {
    ...
    getChequeEvent(companyId: string, chequeId: string, eventType: ChequeEventType): TreasuryChequeEventRecord | null {
        const row = this.db.prepare(`
            ...
            WHERE company_id = ?
              ...
              AND event_type = ?
            LIMIT 1
        `).get(companyId, chequeId, eventType) as ChequeEventRow | undefined;
    ...
    private mapCheque(row: ChequeRow): ChequeRegisterEntity {
        return {
            id: row.id,
            companyId: row.company_id,
    ...
    private normalizeDocType(rawType: string | null | undefined): TreasuryDocumentType {
    ...
    private normalizeStatus(rawStatus: string | null | undefined): TreasuryDocumentStatus {
    ...
    private normalizeChequeDirection(rawDirection: string | null | undefined): ChequeDirection {
        const normalized = String(rawDirection || '').trim().toUpperCase();
        if (normalized === 'ISSUED') return 'ISSUED';
        return 'RECEIVED';
    }

    private normalizeChequeStatus(rawStatus: string | null | undefined): ChequeStatus {
        const normalized = String(rawStatus || '').trim().toUpperCase();
        if (normalized === 'DEPOSITED') return 'DEPOSITED';
        if (normalized === 'CLEARED') return 'CLEARED';
        if (normalized === 'RETURNED') return 'RETURNED';
        if (normalized === 'CANCELLED') return 'CANCELLED';
        if (normalized === 'ISSUED_PENDING') return 'ISSUED_PENDING';
        if (normalized === 'ISSUED_CLEARED') return 'ISSUED_CLEARED';
        return 'IN_SAFE';
    }

    private normalizeChequeEventType(rawEventType: string): ChequeEventType {
        const normalized = String(rawEventType || '').trim().toUpperCase();
        if (normalized === 'ISSUE') return 'ISSUE';
        if (normalized === 'DEPOSIT') return 'DEPOSIT';
        if (normalized === 'CLEAR_RECEIVED') return 'CLEAR_RECEIVED';
        if (normalized === 'RETURN_RECEIVED') return 'RETURN_RECEIVED';
        if (normalized === 'CLEAR_ISSUED') return 'CLEAR_ISSUED';
        if (normalized === 'CANCEL') return 'CANCEL';
        return 'RECEIVE';
    }

    private getDocTypePrefix(docType: TreasuryDocumentType): string {
        switch (docType) {
            case 'CASH_RECEIPT':
                return 'CRV';
            case 'CASH_PAYMENT':
