...
import {
    ...
    ResolutionDefinitionQuery,
} from '../../application/ports/AccountingFoundationPorts';
...
type AccountRow = {
    ...
    requires_cost_center: number | null;
    requires_analysis_code: number | null;
};
...
const CATEGORY_DEFAULT_BY_TYPE: Record<AccountType, AccountCategory> = {
...
export class SqliteAccountingFoundationRepo implements AccountRepositoryPort, FinancialDefinitionRepositoryPort {
    ...
    async getByCode(companyId: string, accountCode: string): Promise<Account | null> {
    ...
    async getByIds(companyId: string, accountIds: string[]): Promise<Account[]> {
    ...
    async list(companyId: string, includeInactive: boolean): Promise<Account[]> {
        const rows = this.db
            .prepare(`
                SELECT *
                FROM accounts
            ...
            `)
            .all(companyId, includeInactive ? 1 : 0) as AccountRow[];
        return rows.map((row) => this.mapAccountRow(row));
    }
    ...
    async save(account: Account): Promise<void> {
        ...
        this.db
            ...
            .run({
                ...
                isActive: payload.status === AccountStatus.ACTIVE ? 1 : 0,
    ...
    async hasReferences(companyId: string, accountId: string): Promise<boolean> {
        const checks = [
        ...
        for (const check of checks) {
            if (!this.tableExists(check.table)) continue;
            ...
            const row = check.companyColumn
                ? (this.db.prepare(sql).get(accountId, companyId) as { ok: number } | undefined)
                : (this.db.prepare(sql).get(accountId) as { ok: number } | undefined);
    ...
    async getPostable(companyId: string): Promise<Account[]> {
        const rows = this.db
            .prepare(`
                SELECT *
                FROM accounts
    ...
    async getDefinitionById(companyId: string, definitionId: string): Promise<FinancialDefinition | null> {
        const row = this.db
            .prepare('SELECT * FROM financial_definitions WHERE company_id = ? AND id = ? LIMIT 1')
            .get(companyId, definitionId) as DefinitionRow | undefined;
        return row ? this.mapDefinitionRow(row) : null;
    }

    async listDefinitions(companyId: string, includeInactive: boolean): Promise<FinancialDefinition[]> {
        const rows = this.db
            .prepare(`
                SELECT *
            ...
            `)
            .all(companyId, includeInactive ? 1 : 0) as DefinitionRow[];
        return rows.map((row) => this.mapDefinitionRow(row));
    }

    async listForResolution(query: ResolutionDefinitionQuery): Promise<FinancialDefinition[]> {
        if (!query.mappingKeys.length) return [];
        ...
        const scopeClauses: string[] = ["(scope_type = ? AND scope_id = ?)"];
        const params: Array<string | null> = [
            query.companyId,
            ...query.mappingKeys,
            query.postingDate,
            query.postingDate,
            FinancialDefinitionScopeType.COMPANY,
            'DEFAULT',
        ];

        if (query.itemId) {
            scopeClauses.push('(scope_type = ? AND scope_id = ?)');
            params.push(FinancialDefinitionScopeType.ITEM, query.itemId);
        }
        if (query.itemGroupId) {
            scopeClauses.push('(scope_type = ? AND scope_id = ?)');
            params.push(FinancialDefinitionScopeType.ITEM_GROUP, query.itemGroupId);
        }
        if (query.warehouseId) {
            scopeClauses.push('(scope_type = ? AND scope_id = ?)');
            params.push(FinancialDefinitionScopeType.WAREHOUSE, query.warehouseId);
        }
        if (query.partnerId) {
            scopeClauses.push('(scope_type = ? AND scope_id = ?)');
            params.push(FinancialDefinitionScopeType.PARTNER, query.partnerId);
        }
        if (query.branchId) {
            scopeClauses.push('(scope_type = ? AND scope_id = ?)');
            params.push(FinancialDefinitionScopeType.BRANCH, query.branchId);
        }
        ...
        const rows = this.db.prepare(sql).all(...params) as DefinitionRow[];
        return rows.map((row) => this.mapDefinitionRow(row));
    }

    async saveDefinition(definition: FinancialDefinition): Promise<void> {
    ...
    async deleteDefinition(companyId: string, definitionId: string): Promise<void> {
        this.db
    ...
    private safeAddAccountColumn(columnName: string, ddl: string): void {
    ...
    private safeAddDefinitionColumn(columnName: string, ddl: string): void {
        const columns = this.db.prepare('PRAGMA table_info(financial_definitions)').all() as Array<{ name: string }>;
        const exists = columns.some((column) => column.name === columnName);
        if (!exists) {
    ...
    private mapDefinitionRow(row: DefinitionRow): FinancialDefinition {
        return FinancialDefinition.rehydrate({
            id: String(row.id),
    ...
    private normalizeAccountType(value: string | null): AccountType {
        const normalized = String(value || '').trim().toUpperCase();
        if (Object.values(AccountType).includes(normalized as AccountType)) {
            return normalized as AccountType;
        }
        if (normalized === 'ASSET') return AccountType.ASSET;
        if (normalized === 'LIABILITY') return AccountType.LIABILITY;
        if (normalized === 'EQUITY') return AccountType.EQUITY;
        if (normalized === 'REVENUE') return AccountType.REVENUE;
        if (normalized === 'EXPENSE') return AccountType.EXPENSE;
        return AccountType.ASSET;
    }

    private normalizeAccountCategory(value: string | null, accountType: AccountType): AccountCategory {
        const normalized = String(value || '').trim().toUpperCase();
    ...
    private normalizeAccountSubtype(value: string | null): AccountSubtype {
        const normalized = String(value || '').trim().toUpperCase();
    ...
    private normalizeDefinitionScope(value: string): FinancialDefinitionScopeType {
        const normalized = String(value || '').trim().toUpperCase();
        if (Object.values(FinancialDefinitionScopeType).includes(normalized as FinancialDefinitionScopeType)) {
            return normalized as FinancialDefinitionScopeType;
        }
        return FinancialDefinitionScopeType.COMPANY;
    }

    private normalizeMappingKey(value: string): AccountMappingKey {
        const normalized = String(value || '').trim().toUpperCase();
        if (Object.values(AccountMappingKey).includes(normalized as AccountMappingKey)) {
            return normalized as AccountMappingKey;
        }
        return AccountMappingKey.REVENUE;
    }

    private normalizeDefinitionQualifier(value: string | null): string | null {
        const normalized = String(value || '').trim().toUpperCase();
        return normalized || null;
    }

    private normalizePostingAllowed(row: AccountRow): boolean {
    ...
    private normalizeCurrencyBehavior(row: AccountRow): AccountCurrencyBehavior {
        const normalized = String(row.currency_behavior || '').trim().toUpperCase();
        if (Object.values(AccountCurrencyBehavior).includes(normalized as AccountCurrencyBehavior)) {
            return normalized as AccountCurrencyBehavior;
        }
        return this.normalizeCurrencyCode(row) ? AccountCurrencyBehavior.FIXED_CURRENCY : AccountCurrencyBehavior.BASE_ONLY;
    }

    private normalizeCurrencyCode(row: AccountRow): string | null {
        const candidate = String(row.currency_code || row.currency || '').trim().toUpperCase();
        return candidate ? candidate : null;
    }

    private normalizeReferenceType(value: string | null, subtype: AccountSubtype): AccountReferenceType {
        const normalized = String(value || '').trim().toUpperCase();
        if (Object.values(AccountReferenceType).includes(normalized as AccountReferenceType)) {
            return normalized as AccountReferenceType;
        }

        if (subtype === AccountSubtype.RECEIVABLE || subtype === AccountSubtype.PAYABLE) {
            return AccountReferenceType.GUIDE;
        }
        if (subtype === AccountSubtype.BANK) {
            return AccountReferenceType.BANK_CHEQUE;
        }
        if (subtype === AccountSubtype.CASH) {
            return AccountReferenceType.USER;
        }

        return AccountReferenceType.NONE;
    }

    private normalizeScopeType(row: AccountRow): AccountScopeType {
        const normalized = String(row.scope_type || '').trim().toUpperCase();
        if (Object.values(AccountScopeType).includes(normalized as AccountScopeType)) {
            return normalized as AccountScopeType;
        }
        return row.branch_id ? AccountScopeType.BRANCH : AccountScopeType.COMPANY;
    }

    private normalizeStatus(row: AccountRow): AccountStatus {
        const normalized = String(row.status || '').trim().toUpperCase();
        if (normalized === AccountStatus.ACTIVE) return AccountStatus.ACTIVE;
        if (normalized === AccountStatus.INACTIVE) return AccountStatus.INACTIVE;
        return Number(row.is_active || 1) === 1 ? AccountStatus.ACTIVE : AccountStatus.INACTIVE;
    }

    private estimateLevel(accountCode: string): number {
