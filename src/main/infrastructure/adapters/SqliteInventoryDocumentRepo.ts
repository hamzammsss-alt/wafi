...
import {
    CreateInventoryDocumentDbInput,
    InsertStockLedgerEntryInput,
    InventoryDocumentRepositoryPort,
    InventoryItemRecord,
    InventoryPostingStateRecord,
    InventoryWarehouseRecord,
    SaveInventoryPostingStateInput,
    SaveInventoryReversalStateInput,
    StockLedgerEntryRecord,
    UpdateInventoryDocumentDbInput,
} from '../../application/ports/InventoryDocumentPorts';
import {
...
type InventoryDocumentHeaderRow = {
    id: string;
...
export class SqliteInventoryDocumentRepo implements InventoryDocumentRepositoryPort {
    ...
    nextDocumentNo(companyId: string, branchId: string, docType: InventoryDocumentType): string {
        const normalizedDocType = String(docType || '').trim().toUpperCase() as InventoryDocumentType;
    ...
    createDocument(input: CreateInventoryDocumentDbInput): InventoryDocumentEntity {
        ...
        if (!document) {
            throw new DomainError('INTERNAL_ERROR', `Inventory document ${input.id} was not found after create`);
        }
        return document;
    }
    ...
    updateDocument(input: UpdateInventoryDocumentDbInput): InventoryDocumentEntity {
        ...
        const document = this.getDocumentById(input.companyId, input.branchId, input.id);
        if (!document) {
            throw new DomainError('INTERNAL_ERROR', `Inventory document ${input.id} was not found after update`);
        }
        return document;
    }
    ...
    getDocumentHeaderById(companyId: string, branchId: string, documentId: string): InventoryDocumentHeaderEntity | null {
        const row = this.db.prepare(`
            ...
            LIMIT 1
        `).get(documentId, companyId, branchId) as InventoryDocumentHeaderRow | undefined;
    ...
    getDocumentLinesByDocumentId(documentId: string): InventoryDocumentLineEntity[] {
        ...
        `).all(documentId) as InventoryDocumentLineRow[];
    ...
    getItemById(itemId: string): InventoryItemRecord | null {
        try {
            ...
            const fallback = this.db.prepare(`
                ...
                WHERE id = ?
                LIMIT 1
            `).get(itemId) as { id: string; item_group_id: string | null } | undefined;
        ...
        try {
            const row = this.db.prepare(`
                ...
                ORDER BY
                    ...
                    END
                LIMIT 1
            `).get(companyId) as { value?: string | null } | undefined;
    ...
    getPostingState(companyId: string, branchId: string, documentId: string): InventoryPostingStateRecord | null {
        const row = this.db.prepare(`
            ...
            WHERE id = ?
              ...
              AND COALESCE(branch_id, '') = ?
            LIMIT 1
        `).get(documentId, companyId, branchId) as {
            id: string;
            status: string;
            version: number;
            journal_id: string | null;
    ...
    hasStockLedgerPosting(companyId: string, docType: InventoryDocumentType, docId: string): boolean {
        const row = this.db.prepare(`
            ...
            LIMIT 1
        `).get(companyId, docType, docId) as { ok?: number } | undefined;
    ...
    insertStockLedgerEntries(entries: InsertStockLedgerEntryInput[]): void {
        ...
        const tx = this.db.transaction((rows: InsertStockLedgerEntryInput[]) => {
    ...
    private ensureColumn(tableName: string, columnName: string, typeDefinition: string): void {
    ...
    private normalizeDocType(rawType: string | null): InventoryDocumentType {
    ...
    private normalizeStatus(rawStatus: string | null): InventoryDocumentStatus {
    ...
    private normalizeAdjustmentDirection(rawDirection: string | null): 'IN' | 'OUT' | null {
    ...
    private getDocTypePrefix(docType: InventoryDocumentType): string {
        switch (docType) {
            case 'GOODS_RECEIPT':
                return 'GRN';
            case 'GOODS_ISSUE':
