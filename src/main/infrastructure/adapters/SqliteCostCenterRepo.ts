...
export class SqliteCostCenterRepo implements CostCenterRepoPort {
    ...
    private ensureTableExists() {
        ...
        if (!exists) {
            ...
            const cols = this.db.prepare("PRAGMA table_info(cost_centers)").all() as Array<{ name: string }>;
    ...
    async findById(id: string, companyId: string): Promise<CostCenter | null> {
    ...
    private mapToDomain(row: any): CostCenter {
