...
import {
    ...
    FieldRuleDefinition,
    PolicyGuardDefinition,
    RoleSecurityAssignments,
...
export class SqlitePermissionEngineRepo {
    ...
    seedCatalog(catalog: CapabilityCatalog, registryVersion: number): void {
        ...
        const transaction = this.db.transaction(() => {
            for (const capability of catalog.capabilities) {
                ...
                for (const guard of capability.policyGuards || []) {
                    insertCapabilityPolicyGuard.run(
                        capability.key,
                        guard.guardKey,
                        guard.config ? JSON.stringify(guard.config) : null
                    );
                }
            }
    ...
    seedCapabilityRegistry(rows: Array<{
    ...
    getAclVersions(companyId = 'COMP_01', branchId = 'BR_01'): { companyAclVersion: number; branchAclVersion: number } {
        ...
        const companyRow = this.db.prepare(`
            ...
            FROM acl_versions
            WHERE company_id = ?
        `).get(normalizedCompanyId) as { version?: number } | undefined;

        const branchRow = this.db.prepare(`
            SELECT COALESCE(version, acl_version, 1) AS version
            FROM branch_acl_versions
            WHERE company_id = ? AND branch_id = ?
        `).get(normalizedCompanyId, normalizedBranchId) as { version?: number } | undefined;
    ...
    bumpCompanyAclVersion(companyId = 'COMP_01'): number {
        const normalizedCompanyId = this.normalizeScope(companyId, 'COMP_01');
        this.db.prepare(`
    ...
    bumpBranchAclVersion(companyId = 'COMP_01', branchId = 'BR_01'): number {
        const normalizedCompanyId = this.normalizeScope(companyId, 'COMP_01');
        const normalizedBranchId = this.normalizeScope(branchId, 'BR_01');
        this.db.prepare(`
    ...
    bumpVersion(companyId = 'COMP_01'): number {
    ...
    getRolePermissionsByRoleIds(roleIds: string[]): string[] {
        ...
        try {
            const rows = this.db.prepare(`
                ...
                FROM permissions
                WHERE role_id IN (${placeholders})
            `).all(...ids, ...ids) as Array<{ permission_key: string }>;
            ...
            const rows = this.db.prepare(`
                ...
                FROM role_permissions
                WHERE role_id IN (${placeholders})
            `).all(...ids) as Array<{ permission_key: string }>;
            return Array.from(new Set(rows.map((r) => String(r.permission_key || '').trim()).filter(Boolean)));
        }
    }
    ...
    getRoleCapabilitiesByRoleIds(roleIds: string[]): string[] {
        const ids = Array.from(new Set((roleIds || []).map((x) => String(x || '').trim()).filter(Boolean)));
        if (!ids.length) return [];
        ...
        const rows = this.db.prepare(`
            ...
            WHERE role_id IN (${placeholders})
              AND effect = 'ALLOW'
        `).all(...ids) as Array<{ capability_key: string }>;
    ...
    getCriteriaRulesByCapabilities(params: {
        ...
        `).all(companyId, branchId, ...keys) as Array<{
    ...
    getScopedRoleCapabilitiesByRoleIds(roleIds: string[], companyId: string, branchId: string): string[] {
        const ids = Array.from(new Set((roleIds || []).map((x) => String(x || '').trim()).filter(Boolean)));
        if (!ids.length) return [];
        const placeholders = ids.map(() => '?').join(', ');
        const rows = this.db.prepare(`
        ...
        `).all(...ids, companyId, branchId) as Array<{ capability_key: string }>;
        return Array.from(new Set(rows.map((r) => String(r.capability_key || '').trim()).filter(Boolean)));
    }

    getScopedRoleBundlesByRoleIds(roleIds: string[], companyId: string, branchId: string): string[] {
        const ids = Array.from(new Set((roleIds || []).map((x) => String(x || '').trim()).filter(Boolean)));
        if (!ids.length) return [];
        const placeholders = ids.map(() => '?').join(', ');
        const rows = this.db.prepare(`
            ...
            WHERE role_id IN (${placeholders})
              AND (scope_company_id IS NULL OR scope_company_id = ?)
              AND (scope_branch_id IS NULL OR scope_branch_id = ?)
        `).all(...ids, companyId, branchId) as Array<{ bundle_key: string }>;
        return Array.from(new Set(rows.map((r) => String(r.bundle_key || '').trim()).filter(Boolean)));
    }

    getScopedRoleCapabilities(roleId: string, companyId: string, branchId: string): string[] {
        if (!roleId) return [];
        const rows = this.db.prepare(`
            SELECT capability_key
            FROM sys_role_capabilities
            WHERE role_id = ?
              AND (scope_company_id IS NULL OR scope_company_id = ?)
              AND (scope_branch_id IS NULL OR scope_branch_id = ?)
        `).all(roleId, companyId, branchId) as Array<{ capability_key: string }>;
        return rows.map(r => r.capability_key).filter(Boolean);
    }

    getScopedRoleBundles(roleId: string, companyId: string, branchId: string): string[] {
        if (!roleId) return [];
        const rows = this.db.prepare(`
            SELECT bundle_key
            FROM sys_role_bundles
            WHERE role_id = ?
              AND (scope_company_id IS NULL OR scope_company_id = ?)
              AND (scope_branch_id IS NULL OR scope_branch_id = ?)
        `).all(roleId, companyId, branchId) as Array<{ bundle_key: string }>;
        return rows.map(r => r.bundle_key).filter(Boolean);
    }

    expandBundleCapabilities(bundleKeys: string[]): string[] {
        ...
        `).all(...uniqueKeys) as Array<{ capability_key: string }>;
        return rows.map(r => r.capability_key).filter(Boolean);
    }

    capabilitiesFromPermissions(permissionKeys: string[]): string[] {
    ...
    getFieldRules(capabilityKeys: string[]): Record<string, FieldRuleDefinition[]> {
        const result: Record<string, FieldRuleDefinition[]> = {};
    ...
    getPolicyGuards(capabilityKeys: string[]): Record<string, PolicyGuardDefinition[]> {
        const result: Record<string, PolicyGuardDefinition[]> = {};
        const keys = Array.from(new Set((capabilityKeys || []).filter(Boolean)));
        if (!keys.length) return result;
        const placeholders = keys.map(() => '?').join(', ');
        const rows = this.db.prepare(`
    ...
    getRoleAssignments(roleId: string, scope?: OptionalScope): {
        roleId: string;
        capabilities: Array<{ capabilityKey: string; companyId?: string; branchId?: string; criteria?: string }>;
        bundles: Array<{ bundleKey: string; companyId?: string; branchId?: string; criteria?: string }>;
    } {
    ...
    private cleanScope(value?: string): string | null {
        const raw = String(value || '').trim();
        return raw ? raw : null;
    }

    private buildScopeWhere(scope?: OptionalScope): { sql: string } {
    ...
    private tryParseStringArray(value: string | null | undefined): string[] {
        if (!value) return [];
        try {
    ...
    private normalizeScope(value: any, fallback: string): string {
        const raw = String(value || '').trim();
        return raw || fallback;
    }
}
