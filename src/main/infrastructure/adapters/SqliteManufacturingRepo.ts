...
import {
    ...
    ManufacturingStockLedgerEntryRecord,
    ManufacturingWarehouseRecord,
    SaveProductionIssuePostingStateInput,
    SaveProductionIssueReversalStateInput,
    SaveProductionOrderStatusInput,
    SaveProductionReceiptPostingStateInput,
    SaveProductionReceiptReversalStateInput,
    UpdateBomDbInput,
    UpdateProductionOrderDbInput,
    UpdateProductionOrderProgressInput,
    UpdateRoutingDbInput,
} from '../../application/ports/ManufacturingPorts';
...
type DbLike = {
    ...
    transaction: <T>(work: (...args: any[]) => T) => (...args: any[]) => T;
};

export class SqliteManufacturingRepo implements ManufacturingRepositoryPort {
    ...
    private toSqlIdentifier(raw: string): string {
    ...
    private ensureLegacyCompatibility(): void {
    ...
    private ensureBomHeaderCompatibility(): void {
        if (!this.tableExists('mfg_bom_headers')) return;
    ...
    private ensureBomLineCompatibility(): void {
        if (!this.tableExists('mfg_bom_lines')) return;
    ...
    private ensureRoutingHeaderCompatibility(): void {
        if (!this.tableExists('mfg_routing_headers')) return;
    ...
    private ensureRoutingStepCompatibility(): void {
        if (!this.tableExists('mfg_routing_steps')) return;
    ...
    private ensureProductionOrderCompatibility(): void {
    ...
    private seedModernBomsFromLegacyCatalog(): void {
        if (!this.tableExists('mfg_boms')) return;
        if (!this.tableExists('mfg_bom_headers')) return;
        ...
        if (!this.tableExists('mfg_bom_components')) return;
        if (!this.tableExists('mfg_bom_lines')) return;
    ...
    private seedModernRoutingsFromLegacyCatalog(): void {
        if (!this.tableExists('mfg_routings')) return;
        if (!this.tableExists('mfg_routing_headers')) return;
        ...
        if (!this.tableExists('mfg_routing_operations')) return;
        if (!this.tableExists('mfg_routing_steps')) return;
    ...
    private seedModernOrderComponentsFromLegacyInputs(): void {
        if (!this.tableExists('mfg_production_order_inputs')) return;
        if (!this.tableExists('mfg_production_order_components')) return;
    ...
    private ensureStockLedgerCompatibility(): void {
        if (!this.tableExists('stock_ledger_entries')) return;
    ...
    private createIndexIfPossible(sql: string, tableName: string, requiredColumns: string[]): void {
        if (!this.tableExists(tableName)) return;
        for (const column of requiredColumns) {
            if (!this.hasColumn(tableName, column)) return;
        }
        this.tryExec(sql);
    }

    private ensureManufacturingIndexes(): void {
        this.createIndexIfPossible(
            'CREATE INDEX IF NOT EXISTS idx_mfg_bom_item_default ON mfg_bom_headers(company_id, item_id, status, is_default, version_no DESC)',
            'mfg_bom_headers',
            ['company_id', 'item_id', 'status', 'is_default', 'version_no']
        );
        this.createIndexIfPossible(
            'CREATE UNIQUE INDEX IF NOT EXISTS ux_mfg_bom_line_no ON mfg_bom_lines(bom_id, line_no)',
            'mfg_bom_lines',
            ['bom_id', 'line_no']
        );
        this.createIndexIfPossible(
            'CREATE INDEX IF NOT EXISTS idx_mfg_routing_item_default ON mfg_routing_headers(company_id, item_id, status, is_default, version_no DESC)',
            'mfg_routing_headers',
            ['company_id', 'item_id', 'status', 'is_default', 'version_no']
        );
        this.createIndexIfPossible(
            'CREATE UNIQUE INDEX IF NOT EXISTS ux_mfg_routing_step_no ON mfg_routing_steps(routing_id, step_no)',
            'mfg_routing_steps',
            ['routing_id', 'step_no']
        );
        this.createIndexIfPossible(
            'CREATE INDEX IF NOT EXISTS idx_mfg_order_scope ON mfg_production_orders(company_id, branch_id, status, order_date, id)',
            'mfg_production_orders',
            ['company_id', 'branch_id', 'status', 'order_date', 'id']
        );
        this.createIndexIfPossible(
            'CREATE UNIQUE INDEX IF NOT EXISTS ux_mfg_order_no ON mfg_production_orders(company_id, branch_id, order_no)',
            'mfg_production_orders',
            ['company_id', 'branch_id', 'order_no']
        );
        this.createIndexIfPossible(
            'CREATE UNIQUE INDEX IF NOT EXISTS ux_mfg_order_component_no ON mfg_production_order_components(production_order_id, line_no)',
            'mfg_production_order_components',
            ['production_order_id', 'line_no']
        );
        this.createIndexIfPossible(
            'CREATE UNIQUE INDEX IF NOT EXISTS ux_mfg_order_operation_no ON mfg_production_order_operations(production_order_id, step_no)',
            'mfg_production_order_operations',
            ['production_order_id', 'step_no']
        );
        this.createIndexIfPossible(
            'CREATE INDEX IF NOT EXISTS idx_mfg_issue_scope ON mfg_production_issues(company_id, branch_id, status, issue_date, id)',
            'mfg_production_issues',
            ['company_id', 'branch_id', 'status', 'issue_date', 'id']
        );
        this.createIndexIfPossible(
            'CREATE UNIQUE INDEX IF NOT EXISTS ux_mfg_issue_no ON mfg_production_issues(company_id, branch_id, issue_no)',
            'mfg_production_issues',
            ['company_id', 'branch_id', 'issue_no']
        );
        this.createIndexIfPossible(
            'CREATE UNIQUE INDEX IF NOT EXISTS ux_mfg_issue_line_no ON mfg_production_issue_lines(issue_id, line_no)',
            'mfg_production_issue_lines',
            ['issue_id', 'line_no']
        );
        this.createIndexIfPossible(
            'CREATE INDEX IF NOT EXISTS idx_mfg_receipt_scope ON mfg_production_receipts(company_id, branch_id, status, receipt_date, id)',
            'mfg_production_receipts',
            ['company_id', 'branch_id', 'status', 'receipt_date', 'id']
        );
        this.createIndexIfPossible(
            'CREATE UNIQUE INDEX IF NOT EXISTS ux_mfg_receipt_no ON mfg_production_receipts(company_id, branch_id, receipt_no)',
            'mfg_production_receipts',
            ['company_id', 'branch_id', 'receipt_no']
        );
        this.createIndexIfPossible(
            'CREATE UNIQUE INDEX IF NOT EXISTS ux_mfg_receipt_line_no ON mfg_production_receipt_lines(receipt_id, line_no)',
            'mfg_production_receipt_lines',
            ['receipt_id', 'line_no']
        );
        this.createIndexIfPossible(
            'CREATE INDEX IF NOT EXISTS idx_mfg_stock_ledger_doc ON stock_ledger_entries(company_id, doc_type, doc_id, is_reversal)',
            'stock_ledger_entries',
            ['company_id', 'doc_type', 'doc_id', 'is_reversal']
        );
        this.createIndexIfPossible(
            'CREATE UNIQUE INDEX IF NOT EXISTS ux_mfg_stock_ledger_line ON stock_ledger_entries(company_id, doc_type, doc_id, doc_line_id, warehouse_id, movement_side, is_reversal)',
            'stock_ledger_entries',
            ['company_id', 'doc_type', 'doc_id', 'doc_line_id', 'warehouse_id', 'movement_side', 'is_reversal']
        );
    }
    ...
    nextDocumentNo(companyId: string, branchId: string, key: string): string {
        ...
        `).get(companyId, branchId, likePattern) as { count?: number } | undefined;

        const seq = Number(row?.count || 0) + 1;
        return `${config.prefix}-${today}-${String(seq).padStart(4, '0')}`;
    }

    private resolveDocumentConfig(key: string): { table: string; column: string; prefix: string } {
    ...
    createBom(input: CreateBomDbInput): BomEntity {
        ...
        this.db.transaction((payload: CreateBomDbInput) => {
            insertHeader.run({
                ...payload,
        ...
        return this.getBomById(input.companyId, input.id) as BomEntity;
    }
    ...
    updateBom(input: UpdateBomDbInput): BomEntity {
        ...
        this.db.transaction((payload: UpdateBomDbInput) => {
    ...
    getDefaultBomForItem(companyId: string, itemId: string, asOfDate: string | null): BomEntity | null {
        ...
        const header = this.db.prepare(`
            ...
            LIMIT 1
        `).get(companyId, itemId, effectiveDate, effectiveDate) as any;
    ...
    listBomHeadersByItem(companyId: string, itemId: string): BomEntity[] {
        ...
        `).all(companyId, itemId) as any[];

        return rows.map((row) => ({
            header: this.mapBomHeader(row),
            lines: this.db.prepare('SELECT * FROM mfg_bom_lines WHERE bom_id = ? ORDER BY line_no').all(row.id).map((line: any) => this.mapBomLine(line)),
        }));
    }
    ...
    setBomDefault(companyId: string, itemId: string, bomId: string, updatedAt: string): void {
        ...
        this.db.transaction((payload: { companyId: string; itemId: string; bomId: string; updatedAt: string }) => {
    ...
    createRouting(input: CreateRoutingDbInput): void {
        ...
        this.db.transaction((payload: CreateRoutingDbInput) => {
            insertHeader.run({
                ...payload,
                isDefault: payload.isDefault ? 1 : 0,
            });
    ...
    updateRouting(input: UpdateRoutingDbInput): void {
        ...
        this.db.transaction((payload: UpdateRoutingDbInput) => {
            updateHeader.run(payload);
    ...
    getRoutingById(companyId: string, routingId: string): RoutingEntity | null {
        const header = this.db.prepare(`
            SELECT *
            ...
            WHERE id = ?
              AND company_id = ?
            LIMIT 1
        `).get(routingId, companyId) as any;
    ...
    getDefaultRoutingForItem(companyId: string, itemId: string): RoutingEntity | null {
        const header = this.db.prepare(`
            SELECT *
            FROM mfg_routing_headers
            WHERE company_id = ?
    ...
    setRoutingDefault(companyId: string, itemId: string, routingId: string, updatedAt: string): void {
        ...
        this.db.transaction((payload: { companyId: string; itemId: string; routingId: string; updatedAt: string }) => {
            resetDefaults.run(payload);
            setDefault.run(payload);
    ...
    createProductionOrder(input: CreateProductionOrderDbInput): ProductionOrderEntity {
        ...
        this.db.transaction((payload: CreateProductionOrderDbInput) => {
        ...
        return this.getProductionOrderById(input.companyId, input.branchId, input.id) as ProductionOrderEntity;
    }
    ...
    getProductionOrderById(companyId: string, branchId: string, orderId: string): ProductionOrderEntity | null {
        const row = this.db.prepare(`
            ...
            WHERE id = ?
              ...
              AND branch_id = ?
            LIMIT 1
        `).get(orderId, companyId, branchId) as any;
    ...
    createProductionIssue(input: CreateProductionIssueDbInput): ProductionIssueEntity {
        ...
        this.db.transaction((payload: CreateProductionIssueDbInput) => {
            insertHeader.run(payload);
            for (const line of payload.lines) {
        ...
        return this.getProductionIssueById(input.companyId, input.branchId, input.id) as ProductionIssueEntity;
    }
    ...
    getProductionIssueById(companyId: string, branchId: string, issueId: string): ProductionIssueEntity | null {
        const header = this.db.prepare(`
            ...
            WHERE id = ?
              ...
              AND branch_id = ?
            LIMIT 1
        `).get(issueId, companyId, branchId) as any;
    ...
    listProductionIssueLines(issueId: string): ProductionIssueLineEntity[] {
        const rows = this.db.prepare(`
            SELECT *
            FROM mfg_production_issue_lines
            WHERE issue_id = ?
    ...
    createProductionReceipt(input: CreateProductionReceiptDbInput): ProductionReceiptEntity {
        ...
        this.db.transaction((payload: CreateProductionReceiptDbInput) => {
            insertHeader.run(payload);
            for (const line of payload.lines) {
        ...
        return this.getProductionReceiptById(input.companyId, input.branchId, input.id) as ProductionReceiptEntity;
    }
    ...
    getProductionReceiptById(companyId: string, branchId: string, receiptId: string): ProductionReceiptEntity | null {
        const header = this.db.prepare(`
            ...
            WHERE id = ?
              ...
              AND branch_id = ?
            LIMIT 1
        `).get(receiptId, companyId, branchId) as any;
    ...
    listProductionReceiptLines(receiptId: string): ProductionReceiptLineEntity[] {
        const rows = this.db.prepare(`
            SELECT *
            FROM mfg_production_receipt_lines
            WHERE receipt_id = ?
    ...
    listStockLedgerEntries(companyId: string, docType: string, docId: string, isReversal: boolean): ManufacturingStockLedgerEntryRecord[] {
        ...
        `).all(companyId, docType, docId, isReversal ? 1 : 0) as any[];
    ...
    insertStockLedgerEntries(entries: ManufacturingStockLedgerEntryRecord[]): void {
        ...
        const tx = this.db.transaction((rows: ManufacturingStockLedgerEntryRecord[]) => {
            for (const row of rows) {
                insert.run({
                    ...row,
    ...
    getPolicy(_companyId: string): ManufacturingPolicy {
    ...
    private mapBomHeader(row: any): BomHeaderEntity {
        return {
            id: row.id,
            companyId: row.company_id,
            itemId: row.item_id,
            versionNo: Number(row.version_no || 1),
            status: String(row.status || 'DRAFT').toUpperCase() as ManufacturingBomStatus,
    ...
    private mapBomLine(row: any): BomLineEntity {
        return {
            id: row.id,
            bomId: row.bom_id,
    ...
    private mapRoutingHeader(row: any): RoutingHeaderEntity {
        return {
            id: row.id,
            companyId: row.company_id,
            itemId: row.item_id,
            versionNo: Number(row.version_no || 1),
            status: String(row.status || 'DRAFT').toUpperCase() as ManufacturingRoutingStatus,
            isDefault: Number(row.is_default || 0) === 1,
            remarks: row.remarks || null,
            createdBy: row.created_by,
            approvedBy: row.approved_by || null,
    ...
    private mapRoutingStep(row: any): RoutingStepEntity {
        return {
            id: row.id,
    ...
    private mapProductionOrder(row: any): ProductionOrderEntity {
        return {
            id: row.id,
            companyId: row.company_id,
            branchId: row.branch_id,
            ...
            status: String(row.status || 'DRAFT').toUpperCase() as ProductionOrderStatus,
            itemId: row.item_id,
    ...
    private mapProductionOrderComponent(row: any): ProductionOrderComponentEntity {
        return {
            id: row.id,
    ...
    private mapProductionOrderOperation(row: any): ProductionOrderOperationEntity {
        return {
            id: row.id,
            productionOrderId: row.production_order_id,
            stepNo: Number(row.step_no || 0),
    ...
    private mapProductionIssueHeader(row: any): ProductionIssueHeaderEntity {
        return {
            id: row.id,
            companyId: row.company_id,
            branchId: row.branch_id,
            ...
            status: String(row.status || 'DRAFT').toUpperCase() as any,
            productionOrderId: row.production_order_id,
            referenceNo: row.reference_no || null,
            remarks: row.remarks || null,
            createdBy: row.created_by,
    ...
    private mapProductionIssueLine(row: any): ProductionIssueLineEntity {
        return {
            id: row.id,
    ...
    private mapProductionReceiptHeader(row: any): ProductionReceiptHeaderEntity {
        return {
            id: row.id,
            companyId: row.company_id,
            branchId: row.branch_id,
    ...
    private mapProductionReceiptLine(row: any): ProductionReceiptLineEntity {
        return {
            id: row.id,
