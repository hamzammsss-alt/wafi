...
import { ExpenseDimensionsRepositoryPort } from '../../application/ports/ExpenseDimensionsPorts';
import {
    CostCenterDimension,
...
const EXPENSE_TYPE_SEED: Array<{ id: string; code: string; name: string }> = [
...
const COST_CENTER_SEED: Array<{ id: string; code: string; name: string; parentId: string | null }> = [
...
export class SqliteExpenseDimensionsRepo implements ExpenseDimensionsRepositoryPort {
    private readonly hasExpenseTypeCompanyId: boolean;
    private readonly hasCostCenterCompanyId: boolean;
    private readonly hasVehicleCompanyId: boolean;
    ...
    async listExpenseTypes(companyId: string, query: DimensionListQuery): Promise<ExpenseTypeDimension[]> {
    ...
    async listCostCenters(companyId: string, query: DimensionListQuery): Promise<CostCenterDimension[]> {
        const includeInactive = query.includeInactive ? 1 : 0;
        const search = String(query.search || '').trim();
        const like = `%${search}%`;
    ...
    async listVehicles(companyId: string, query: DimensionListQuery): Promise<VehicleDimension[]> {
        const includeInactive = query.includeInactive ? 1 : 0;
        const search = String(query.search || '').trim();
        const like = `%${search}%`;
    ...
    async validateJournalLineDimensions(companyId: string, dimensions: JournalLineDimensions): Promise<void> {
        ...
        if (dimensions.projectId) {
            if (!this.tableExists('projects')) {
                throw new DomainError('VALIDATION_ERROR', 'Project dimension table is not configured', {
    ...
    async getVehicleExpenseReport(companyId: string, query: ExpenseReportQuery): Promise<VehicleExpenseReportRow[]> {
        const rows = this.db
            .prepare(
                `
                SELECT
    ...
    async getExpenseTypeReport(companyId: string, query: ExpenseReportQuery): Promise<ExpenseTypeReportRow[]> {
        const rows = this.db
            .prepare(
                `
                SELECT
    ...
    async getCostCenterExpenseReport(companyId: string, query: ExpenseReportQuery): Promise<CostCenterExpenseReportRow[]> {
        const rows = this.db
            .prepare(
                `
                SELECT
    ...
    private seedDefaults(): void {
        ...
        for (const seed of EXPENSE_TYPE_SEED) {
            ...
            }) as { id: string; code: string | null; name: string | null; name_ar: string | null; name_en: string | null } | undefined;
    ...
    private async assertRecordIsActive(
        companyId: string,
        tableName: string,
        ...
        if (!this.tableExists(tableName)) {
            throw new DomainError('VALIDATION_ERROR', `Dimension table ${tableName} is not configured`, {
        ...
        if (!row) {
            throw new DomainError('VALIDATION_ERROR', `Invalid or inactive ${fieldName}`, {
    ...
    private safeAddColumn(tableName: string, columnName: string, definition: string): void {
