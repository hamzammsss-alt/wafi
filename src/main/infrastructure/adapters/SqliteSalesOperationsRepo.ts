...
import {
    ...
    SalesPostingStateRecord,
    SalesReservationRecord,
    SalesSavePostingStateInput,
    SalesSaveReversalStateInput,
    SalesStockLedgerEntryRecord,
    SalesUpdateDocumentDbInput,
    SalesWarehouseRecord,
} from '../../application/ports/SalesOperationsPorts';
...
export class SqliteSalesOperationsRepo implements SalesOperationsRepositoryPort {
    ...
    createDocument(input: SalesCreateDocumentDbInput): SalesOperationDocumentEntity {
        ...
        if (!created) {
            throw new DomainError('INTERNAL_ERROR', 'Sales operation document was not found after create', {
    ...
    updateDocument(input: SalesUpdateDocumentDbInput): SalesOperationDocumentEntity {
        ...
        if (Number(info.changes || 0) === 0) {
            throw new DomainError('INVALID_TRANSITION', 'Only draft sales documents can be updated', {
        ...
        if (!updated) {
            throw new DomainError('INTERNAL_ERROR', 'Sales operation document was not found after update', {
                details: { documentId: input.id },
            });
        }
    ...
    getPolicy(_companyId: string): SalesOperationPolicy {
    ...
    private mapLineRow(row: LineRow): SalesOperationLineEntity {
        return {
            id: row.id,
    ...
    private mapLinkRow(row: LinkRow): SalesOperationLineLinkEntity {
        return {
            id: row.id,
            companyId: row.company_id,
            branchId: row.branch_id,
    ...
    private normalizeDocType(raw: string | null | undefined): SalesOperationDocumentType {
    ...
    private normalizeStatus(raw: string | null | undefined): SalesOperationStatus {
        const normalized = String(raw || '').trim().toUpperCase();
    ...
    private getDocTypePrefix(docType: SalesOperationDocumentType): string {
        switch (docType) {
