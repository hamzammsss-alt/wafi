...
exports.TreasuryPostingBuilder = exports.TREASURY_SOURCE_MODULE = void 0;
...
exports.TREASURY_SOURCE_MODULE = 'TREASURY';
class TreasuryPostingBuilder {
    ...
    async build(input) {
        ...
        if (!docDate) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Document date is required for treasury posting', {
                messageKey: 'validation.treasury_document.date_required',
            });
        }
        if (!docNo) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Document number is required for treasury posting', {
        ...
        const resolution = await this.accountResolutionUseCases.resolveRequiredAccounts(input.companyId, {
            ...
            optionalRoles: [FinancialAccountRole_1.FinancialAccountRole.RECEIVABLE_ACCOUNT, FinancialAccountRole_1.FinancialAccountRole.PAYABLE_ACCOUNT],
        });
        if (!resolution.success) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Treasury control account resolution failed', {
        ...
        if (!control) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Treasury control account was not resolved', {
                messageKey: 'error.account_resolution.mapping_not_found',
        ...
        if (!postingLines.length) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Treasury posting command has no lines', {
        ...
        if (Math.abs(totalDebit - totalCredit) > 0.01) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Treasury posting command is not balanced', {
    ...
    resolveControlRole(docType) {
        switch (docType) {
            case 'CASH_RECEIPT':
            case 'CASH_PAYMENT':
                return FinancialAccountRole_1.FinancialAccountRole.CASH_ACCOUNT;
            case 'BANK_RECEIPT':
            case 'BANK_PAYMENT':
                return FinancialAccountRole_1.FinancialAccountRole.BANK_ACCOUNT;
            case 'CHEQUE_RECEIPT':
                return FinancialAccountRole_1.FinancialAccountRole.CHEQUE_IN_SAFE_ACCOUNT;
            case 'CHEQUE_PAYMENT':
                return FinancialAccountRole_1.FinancialAccountRole.ISSUED_CHEQUE_ACCOUNT;
            default:
                return FinancialAccountRole_1.FinancialAccountRole.SUSPENSE_ACCOUNT;
        }
    }
    resolveDirection(docType) {
        if (this.isReceiptType(docType))
            return ResolutionDirection_1.ResolutionDirection.SALE;
        return ResolutionDirection_1.ResolutionDirection.PURCHASE;
    }
...
exports.TreasuryPostingBuilder = TreasuryPostingBuilder;
