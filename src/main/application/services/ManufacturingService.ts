...
import {
    CreateBomDbInput,
    CreateProductionIssueDbInput,
    CreateProductionOrderDbInput,
    CreateProductionReceiptDbInput,
    CreateRoutingDbInput,
    ManufacturingRepositoryPort,
    UpdateBomDbInput,
    UpdateRoutingDbInput,
} from '../ports/ManufacturingPorts';
import { JournalDto, JournalEngineUseCases } from '../useCases/JournalEngineUseCases';
import { ManufacturingAccountingBuilder } from './ManufacturingAccountingBuilder';
import { ManufacturingStockLedgerService } from './ManufacturingStockLedgerService';
...
export class ManufacturingService {
    constructor(
        ...
        private readonly stockLedgerService: ManufacturingStockLedgerService,
        private readonly accountingBuilder: ManufacturingAccountingBuilder,
        private readonly journalEngineUseCases: JournalEngineUseCases,
    ) {
        this.repository.ensureSchema();
    }

    createBom(context: ManufacturingContext, input: CreateBomInput): BomEntity {
        ...
        const payload: CreateBomDbInput = {
        ...
        const bom = this.repository.runInTransaction(() => {
            ...
            return this.repository.getBomById(companyId, created.header.id) as BomEntity;
        });

        return bom;
    }

    updateBom(context: ManufacturingContext, input: UpdateBomInput): BomEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        ...
        if (current.header.status === 'CANCELLED') {
            throw new DomainError('INVALID_TRANSITION', 'Cancelled BOM cannot be updated', {
        ...
        const now = new Date().toISOString();
        const payload: UpdateBomDbInput = {
            id: bomId,
            companyId,
            outputQty,
            effectiveFrom: this.normalizeDateOrNull(input.effectiveFrom),
            ...
            updatedAt: now,
            lines: lines.map((line, index) => ({
                id: this.normalizeNullable((input.lines[index] as any)?.id) || this.repository.nextIdentity(),
                lineNo: index + 1,
                componentItemId: line.componentItemId,
                warehouseId: line.warehouseId,
                qtyPer: line.qtyPer,
    ...
    getBomById(context: Pick<ManufacturingContext, 'companyId'>, bomId: string): BomEntity {
        return this.requireBom(
            this.normalizeRequired(context.companyId, 'Company id is required'),
            this.normalizeRequired(bomId, 'BOM id is required'),
        );
    }

    getDefaultBomForItem(context: Pick<ManufacturingContext, 'companyId'>, itemId: string, asOfDate?: string | null): BomEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        ...
        if (!bom) {
            throw new DomainError('VALIDATION_ERROR', 'Default BOM was not found for item', {
    ...
    setBomDefault(context: ManufacturingContext, bomId: string): BomEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        ...
        if (bom.header.status !== 'CONFIRMED') {
            throw new DomainError('VALIDATION_ERROR', 'Only confirmed BOM can be set as default', {
    ...
    confirmBom(context: ManufacturingContext, bomId: string): BomEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const normalizedBomId = this.normalizeRequired(bomId, 'BOM id is required');
        const bom = this.requireBom(companyId, normalizedBomId);
        if (!bom.lines.length) {
            throw new DomainError('VALIDATION_ERROR', 'BOM must include at least one component line', {
    ...
    cancelBom(context: ManufacturingContext, bomId: string): BomEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const normalizedBomId = this.normalizeRequired(bomId, 'BOM id is required');
    ...
    createRouting(context: ManufacturingContext, input: CreateRoutingInput): RoutingEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const itemId = this.normalizeRequired(input.itemId, 'Finished item is required');
        const createdBy = this.normalizeRequired(input.createdBy || context.userId, 'Created by is required');
        ...
        const now = new Date().toISOString();
        const payload: CreateRoutingDbInput = {
            id: this.repository.nextIdentity(),
            companyId,
            itemId,
            versionNo: nextVersion,
    ...
    updateRouting(context: ManufacturingContext, input: UpdateRoutingInput): RoutingEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        ...
        if (current.header.status === 'CANCELLED') {
            throw new DomainError('INVALID_TRANSITION', 'Cancelled routing cannot be updated', {
        ...
        const steps = this.normalizeRoutingSteps(input.steps || []);
        const now = new Date().toISOString();
        const payload: UpdateRoutingDbInput = {
            id: routingId,
            companyId,
            remarks: this.normalizeNullable(input.remarks),
            approvedBy: this.normalizeNullable(input.approvedBy),
            updatedAt: now,
            steps: steps.map((step, index) => ({
                id: this.normalizeNullable((input.steps[index] as any)?.id) || this.repository.nextIdentity(),
                stepNo: index + 1,
                workCenterCode: step.workCenterCode,
                operationCode: step.operationCode,
                setupTimeMinutes: step.setupTimeMinutes,
    ...
    getRoutingById(context: Pick<ManufacturingContext, 'companyId'>, routingId: string): RoutingEntity {
        return this.requireRouting(
            this.normalizeRequired(context.companyId, 'Company id is required'),
            this.normalizeRequired(routingId, 'Routing id is required'),
        );
    }

    getDefaultRoutingForItem(context: Pick<ManufacturingContext, 'companyId'>, itemId: string): RoutingEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const normalizedItemId = this.normalizeRequired(itemId, 'Item id is required');
        const routing = this.repository.getDefaultRoutingForItem(companyId, normalizedItemId);
        if (!routing) {
            throw new DomainError('VALIDATION_ERROR', 'Default routing was not found for item', {
    ...
    setRoutingDefault(context: ManufacturingContext, routingId: string): RoutingEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        ...
        if (routing.header.status !== 'CONFIRMED') {
            throw new DomainError('VALIDATION_ERROR', 'Only confirmed routing can be set as default', {
    ...
    confirmRouting(context: ManufacturingContext, routingId: string): RoutingEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const normalizedRoutingId = this.normalizeRequired(routingId, 'Routing id is required');
        const routing = this.requireRouting(companyId, normalizedRoutingId);
        if (!routing.steps.length) {
            throw new DomainError('VALIDATION_ERROR', 'Routing must include at least one step', {
    ...
    cancelRouting(context: ManufacturingContext, routingId: string): RoutingEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const normalizedRoutingId = this.normalizeRequired(routingId, 'Routing id is required');
    ...
    createProductionOrder(context: ManufacturingContext, input: CreateProductionOrderInput): ProductionOrderDocumentEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        ...
        if (bom && bom.header.status !== 'CONFIRMED') {
            throw new DomainError('VALIDATION_ERROR', 'BOM must be confirmed before production order creation', {
        ...
        if (routing && routing.header.status !== 'CONFIRMED') {
            throw new DomainError('VALIDATION_ERROR', 'Routing must be confirmed before production order creation', {
        ...
        const payload: CreateProductionOrderDbInput = {
            id: this.repository.nextIdentity(),
            companyId,
            branchId,
    ...
    createProductionOrderFromBom(context: ManufacturingContext, input: CreateProductionOrderFromBomInput): ProductionOrderDocumentEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const itemId = this.normalizeRequired(input.itemId, 'Finished item is required');
        const orderDate = this.normalizeDate(input.orderDate, 'Order date is required');
        ...
        if (!bom) {
            throw new DomainError('VALIDATION_ERROR', 'BOM is required for createFromBom', {
    ...
    updateProductionOrder(context: ManufacturingContext, input: UpdateProductionOrderInput): ProductionOrderDocumentEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeRequired(context.branchId, 'Branch id is required');
        ...
        if (order.status === 'CANCELLED' || order.status === 'COMPLETED') {
            throw new DomainError('INVALID_TRANSITION', 'Completed/cancelled production order cannot be updated', {
        ...
        if (qtyPlanned + EPSILON < Math.max(order.qtyCompleted, order.qtyIssued)) {
            throw new DomainError('VALIDATION_ERROR', 'Planned quantity cannot be less than issued/completed quantity', {
    ...
    getProductionOrderById(context: Pick<ManufacturingContext, 'companyId' | 'branchId'>, orderId: string): ProductionOrderDocumentEntity {
        return this.getOrderDocumentById(
            this.normalizeRequired(context.companyId, 'Company id is required'),
            this.normalizeRequired(context.branchId, 'Branch id is required'),
            this.normalizeRequired(orderId, 'Production order id is required'),
        );
    }

    releaseProductionOrder(context: ManufacturingContext, orderId: string): ProductionOrderDocumentEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeRequired(context.branchId, 'Branch id is required');
        ...
        if (order.status === 'CANCELLED') {
            throw new DomainError('INVALID_TRANSITION', 'Cancelled production order cannot be released', {
    ...
    async cancelProductionOrder(context: ManufacturingContext, orderId: string): Promise<ProductionOrderDocumentEntity> {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeRequired(context.branchId, 'Branch id is required');
        const normalizedOrderId = this.normalizeRequired(orderId, 'Production order id is required');
        const order = this.requireOrder(companyId, branchId, normalizedOrderId);
    ...
    getProductionOrderStatusSummary(
        context: Pick<ManufacturingContext, 'companyId' | 'branchId'>,
        orderId: string,
    ): ProductionOrderStatusSummary {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeRequired(context.branchId, 'Branch id is required');
    ...
    createProductionIssue(context: ManufacturingContext, input: CreateProductionIssueInput): ProductionIssueEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeRequired(context.branchId, 'Branch id is required');
        ...
        const now = new Date().toISOString();
        const payload: CreateProductionIssueDbInput = {
            id: this.repository.nextIdentity(),
            companyId,
            branchId,
    ...
    getProductionIssueById(
        ...
        return this.requireIssue(
            this.normalizeRequired(context.companyId, 'Company id is required'),
            this.normalizeRequired(context.branchId, 'Branch id is required'),
            this.normalizeRequired(issueId, 'Production issue id is required'),
        );
    }

    async postProductionIssue(context: ManufacturingContext, input: { issueId: string; allowOverIssue?: boolean | null }): Promise<ProductionDocumentPostResult> {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeRequired(context.branchId, 'Branch id is required');
        ...
        if (header.status === 'CANCELLED') {
            throw new DomainError('INVALID_TRANSITION', 'Cancelled issue cannot be posted', {
        ...
        this.repository.runInTransaction(() => {
            ...
            for (const line of lines) {
                ...
                if (!componentLineId) continue;
                this.repository.updateProductionOrderComponentProgress(
                    order.id,
                    componentLineId,
                    line.qty,
                    0,
    ...
    async cancelProductionIssue(context: ManufacturingContext, input: CancelProductionIssueInput): Promise<ProductionDocumentCancelResult> {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeRequired(context.branchId, 'Branch id is required');
        const issueId = this.normalizeRequired(input.issueId, 'Production issue id is required');
    ...
    createProductionReceipt(context: ManufacturingContext, input: CreateProductionReceiptInput): ProductionReceiptEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeRequired(context.branchId, 'Branch id is required');
        const orderId = this.normalizeRequired(input.productionOrderId, 'Production order id is required');
        const order = this.requireOrder(companyId, branchId, orderId);
        ...
        const now = new Date().toISOString();
        const payload: CreateProductionReceiptDbInput = {
            id: this.repository.nextIdentity(),
            companyId,
            branchId,
    ...
    getProductionReceiptById(
        ...
        return this.requireReceipt(
            this.normalizeRequired(context.companyId, 'Company id is required'),
            this.normalizeRequired(context.branchId, 'Branch id is required'),
            this.normalizeRequired(receiptId, 'Production receipt id is required'),
        );
    }

    async postProductionReceipt(context: ManufacturingContext, input: { receiptId: string; allowOverReceipt?: boolean | null }): Promise<ProductionDocumentPostResult> {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeRequired(context.branchId, 'Branch id is required');
        ...
        if (header.status === 'CANCELLED') {
            throw new DomainError('INVALID_TRANSITION', 'Cancelled receipt cannot be posted', {
    ...
    async cancelProductionReceipt(context: ManufacturingContext, input: CancelProductionReceiptInput): Promise<ProductionDocumentCancelResult> {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeRequired(context.branchId, 'Branch id is required');
        const receiptId = this.normalizeRequired(input.receiptId, 'Production receipt id is required');
        const reverseDate = this.normalizeDate(input.reverseDate, 'Reverse date is required');
    ...
    private refreshOrderAggregates(companyId: string, branchId: string, orderId: string): ProductionOrderEntity {
        const order = this.requireOrder(companyId, branchId, orderId);
        ...
        const totalWipCost = this.round(
            roundedMaterial + Number(order.laborCostEstimated || 0) + Number(order.machineCostEstimated || 0) - roundedCapitalized,
        );
    ...
    private getOrderDocumentById(companyId: string, branchId: string, orderId: string): ProductionOrderDocumentEntity {
    ...
    private requireOrder(companyId: string, branchId: string, orderId: string): ProductionOrderEntity {
        const order = this.repository.getProductionOrderById(companyId, branchId, orderId);
        if (!order) {
            throw new DomainError('VALIDATION_ERROR', 'Production order was not found', {
    ...
    private requireIssue(companyId: string, branchId: string, issueId: string): ProductionIssueEntity {
        const issue = this.repository.getProductionIssueById(companyId, branchId, issueId);
        if (!issue) {
            throw new DomainError('VALIDATION_ERROR', 'Production issue was not found', {
    ...
    private requireReceipt(companyId: string, branchId: string, receiptId: string): ProductionReceiptEntity {
        const receipt = this.repository.getProductionReceiptById(companyId, branchId, receiptId);
        if (!receipt) {
            throw new DomainError('VALIDATION_ERROR', 'Production receipt was not found', {
    ...
    private requireBom(companyId: string, bomId: string): BomEntity {
        const bom = this.repository.getBomById(companyId, bomId);
        if (!bom) {
            throw new DomainError('VALIDATION_ERROR', 'BOM was not found', {
    ...
    private requireRouting(companyId: string, routingId: string): RoutingEntity {
        const routing = this.repository.getRoutingById(companyId, routingId);
        if (!routing) {
            throw new DomainError('VALIDATION_ERROR', 'Routing was not found', {
    ...
    private assertNoCircularBom(companyId: string, rootItemId: string, componentItemIds: string[]): void {
        for (const componentItemId of componentItemIds) {
            if (componentItemId === rootItemId) {
                throw new DomainError('VALIDATION_ERROR', 'BOM cannot contain circular component references', {
                    messageKey: 'validation.manufacturing.bom.circular',
                    details: { rootItemId, componentItemId },
                });
            }
            if (this.hasBomPathToRoot(companyId, componentItemId, rootItemId, new Set<string>())) {
                throw new DomainError('VALIDATION_ERROR', 'BOM circular dependency detected', {
                    messageKey: 'validation.manufacturing.bom.circular',
                    details: { rootItemId, componentItemId },
                });
            }
    ...
    private hasBomPathToRoot(companyId: string, currentItemId: string, targetItemId: string, visited: Set<string>): boolean {
        if (currentItemId === targetItemId) return true;
        if (visited.has(currentItemId)) return false;
        visited.add(currentItemId);

        const bom = this.repository.getDefaultBomForItem(companyId, currentItemId, null);
        if (!bom || !bom.lines.length) return false;

        for (const line of bom.lines) {
            if (line.componentItemId === targetItemId) return true;
    ...
    private explodeBomToComponents(bom: BomEntity, qtyPlanned: number, fallbackWarehouseId: string): CreateProductionOrderDbInput['components'] {
        if (Number(bom.header.outputQty || 0) <= 0) {
            throw new DomainError('VALIDATION_ERROR', 'BOM output quantity must be greater than zero', {
    ...
    private expandRoutingOperations(routing: RoutingEntity, qtyPlanned: number): CreateProductionOrderDbInput['operations'] {
    ...
    private resolveIssueComponentLineId(
        components: ProductionOrderComponentEntity[],
        line: ProductionIssueEntity['lines'][number],
    ): string | null {
    ...
    private validateIssuePostingLines(order: ProductionOrderEntity, lines: ProductionIssueEntity['lines'], allowOverIssue: boolean): void {
        if (!lines.length) {
            throw new DomainError('VALIDATION_ERROR', 'Production issue requires at least one line', {
        ...
        const requestedByComponent = new Map<string, number>();

        for (const line of lines) {
            const qty = this.normalizePositive(line.qty, 'Issue quantity must be greater than zero');

            const explicitComponentId = this.normalizeNullable(line.componentLineId);
            let targetComponent: ProductionOrderComponentEntity | undefined;
            if (explicitComponentId) {
                targetComponent = componentById.get(explicitComponentId);
                if (!targetComponent) {
                    throw new DomainError('VALIDATION_ERROR', 'Component line does not belong to production order', {
                ...
                if (candidates.length === 1) {
                    ...
                    throw new DomainError('VALIDATION_ERROR', 'Issue line must reference a production component line', {
            ...
            requestedByComponent.set(
                targetComponent.id,
                this.round((requestedByComponent.get(targetComponent.id) || 0) + qty),
            );
        }
        ...
        if (!allowOverIssue) {
            for (const [componentId, requestedQty] of requestedByComponent.entries()) {
                ...
                throw new DomainError('VALIDATION_ERROR', 'Issue quantity exceeds remaining requirement', {
    ...
    private validateReceiptPostingLines(order: ProductionOrderEntity, lines: ProductionReceiptEntity['lines'], allowOverReceipt: boolean): void {
        if (!lines.length) {
            throw new DomainError('VALIDATION_ERROR', 'Production receipt requires at least one line', {
        ...
        if (totalReceived <= EPSILON) {
            throw new DomainError('VALIDATION_ERROR', 'Production receipt quantity must be greater than zero', {
        ...
        if (!allowOverReceipt && totalReceived - remaining > EPSILON) {
            throw new DomainError('VALIDATION_ERROR', 'Receipt quantity exceeds planned remaining quantity', {
        ...
        for (const line of lines) {
            if (line.itemId !== order.itemId) {
                throw new DomainError('VALIDATION_ERROR', 'Receipt line item must match production order item', {
    ...
    private resolveReceiptLineCosts(
        order: ProductionOrderEntity,
        lines: ProductionReceiptEntity['lines'],
    ): ProductionReceiptEntity['lines'] {
        const totalReceived = this.round(lines.reduce((sum, line) => sum + Number(line.qtyReceived || 0), 0));
    ...
    private normalizeBomLines(lines: CreateBomInput['lines'], rootItemId: string): Array<{
        ...
        if (!lines.length) {
            throw new DomainError('VALIDATION_ERROR', 'BOM requires at least one component line', {
                messageKey: 'validation.manufacturing.bom.lines_required',
                details: { rootItemId },
            });
        }
        ...
        return lines.map((line, index) => {
            ...
            if (componentItemId === rootItemId) {
                throw new DomainError('VALIDATION_ERROR', 'BOM component cannot equal finished item', {
                    messageKey: 'validation.manufacturing.bom.circular',
            ...
            if (warehouseId) this.assertWarehouseExists(warehouseId, 'Warehouse was not found or inactive');
    ...
    private normalizeRoutingSteps(steps: CreateRoutingInput['steps']): Array<{
        ...
        if (!steps.length) {
            throw new DomainError('VALIDATION_ERROR', 'Routing requires at least one step', {
                messageKey: 'validation.manufacturing.routing.steps_required',
            });
        }
    ...
    private normalizeIssueLines(lines: CreateProductionIssueInput['lines'], order: ProductionOrderEntity): CreateProductionIssueDbInput['lines'] {
        if (!lines.length) {
            throw new DomainError('VALIDATION_ERROR', 'Production issue requires at least one line', {
                messageKey: 'validation.manufacturing.issue.lines_required',
                details: { orderId: order.id },
        ...
        return lines.map((line, index) => {
            ...
            if (component && component.componentItemId !== componentItemId) {
                throw new DomainError('VALIDATION_ERROR', 'Issue component item mismatch with production order component', {
    ...
    private normalizeReceiptLines(lines: CreateProductionReceiptInput['lines'], order: ProductionOrderEntity): CreateProductionReceiptDbInput['lines'] {
        if (!lines.length) {
            throw new DomainError('VALIDATION_ERROR', 'Production receipt requires at least one line', {
                messageKey: 'validation.manufacturing.receipt.lines_required',
                details: { orderId: order.id },
        ...
        return lines.map((line, index) => {
            ...
            if (itemId !== order.itemId) {
                throw new DomainError('VALIDATION_ERROR', 'Receipt item must match production order item', {
                    messageKey: 'validation.manufacturing.receipt.item_mismatch',
    ...
    private assertOrderAllowsTransactions(order: ProductionOrderEntity, message: string): void {
        if (order.status === 'CANCELLED' || order.status === 'COMPLETED') {
    ...
    private assertStockItemExists(itemId: string, message: string): void {
        const item = this.repository.getItemById(itemId);
    ...
    private assertWarehouseExists(warehouseId: string, message: string): void {
    ...
    private resolveOriginalJournal(
        ...
        if (!input.fallbackJournalId) return null;
        return this.journalEngineUseCases.getById(companyId, input.fallbackJournalId);
    }

    private resolveExistingReversal(companyId: string, originalJournal: JournalDto | null, explicitReversalJournalId: string | null): JournalDto | null {
    ...
    private estimateLaborCost(operation: CreateProductionOrderDbInput['operations'][number]): number {
    ...
    private estimateMachineCost(operation: CreateProductionOrderDbInput['operations'][number]): number {
        const totalMinutes = Number(operation.setupTimeMinutes || 0) + Number(operation.runTimeMinutes || 0);
        const hours = totalMinutes / 60;
    ...
    private normalizeDateOrNull(value: string | null | undefined): string | null {
        ...
        if (!/^\d{4}-\d{2}-\d{2}$/.test(normalized)) {
            throw new DomainError('VALIDATION_ERROR', 'Date must be in YYYY-MM-DD format', {
                messageKey: 'error.validation',
                details: { value: normalized },
            });
        }
    ...
    private normalizePositive(value: number | string | null | undefined, message: string): number {
    ...
    private normalizeNonNegative(value: number | string | null | undefined, message: string): number {
        const numeric = Number(value || 0);
        if (!Number.isFinite(numeric) || numeric < 0) {
            throw new DomainError('VALIDATION_ERROR', message, {
                messageKey: 'error.validation',
