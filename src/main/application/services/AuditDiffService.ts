import { AuditFieldChangeInput } from '../../domain/audit/AuditTypes';
...
function normalizeKeyHint(line: Record<string, unknown>, index: number): string {
...
function pushChange(target: AuditFieldChangeInput[], item: AuditFieldChangeInput, maxChanges: number) {
    if (target.length >= maxChanges) return;
    target.push(item);
}

function internalDiffObjects(
    before: Record<string, unknown>,
    after: Record<string, unknown>,
    opts: Required<DiffOptions>,
    output: AuditFieldChangeInput[],
    currentPath: string,
) {
...
export function diffPlainObjects(
    beforeValue: unknown,
    afterValue: unknown,
    options?: DiffOptions,
    ...
    const output: AuditFieldChangeInput[] = [];
...
export function diffLineArrays(
    beforeLines: unknown,
    afterLines: unknown,
    options?: Omit<DiffOptions, 'basePath'>,
): AuditFieldChangeInput[] {
    ...
    for (const key of keys) {
        if (output.length >= maxChanges) break;
...
export function diffDocumentPayload(
