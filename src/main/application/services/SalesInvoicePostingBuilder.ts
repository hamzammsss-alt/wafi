...
import {
    SalesInvoiceAccountingRepositoryPort,
    SalesInvoiceHeaderRecord,
    SalesInvoiceLineRecord,
} from '../ports/SalesInvoiceAccountingPorts';
import { PostJournalInput } from '../useCases/JournalEngineUseCases';

type SalesInvoicePostingBuildInput = {
    companyId: string;
    branchId: string;
    userId: string;
    sourceVersion: number;
    header: SalesInvoiceHeaderRecord;
...
export const SALES_SOURCE_MODULE = 'SALES';
export const SALES_SOURCE_TYPE = 'INVOICE';
export const SALES_JOURNAL_SOURCE_TYPE = `${SALES_SOURCE_MODULE}_${SALES_SOURCE_TYPE}`;

export class SalesInvoicePostingBuilder {
    constructor(
    ...
    async build(input: SalesInvoicePostingBuildInput): Promise<PostJournalInput> {
        ...
        for (let index = 0; index < input.lines.length; index += 1) {
            ...
            const resolution = await this.accountResolutionUseCases.resolveRequiredAccounts(input.companyId, {
                ...
                requiredRoles: [
                    ...
                    ...(isService ? [FinancialAccountRole.SERVICE_REVENUE_ACCOUNT] : [FinancialAccountRole.REVENUE_ACCOUNT]),
                    ...(vatAmount > EPSILON ? [FinancialAccountRole.VAT_OUTPUT_ACCOUNT] : []),
                    ...(requiresInventory ? [FinancialAccountRole.COGS_ACCOUNT, FinancialAccountRole.INVENTORY_ACCOUNT] : []),
                ],
                optionalRoles: [
            ...
            if (!resolution.success) {
                throw new DomainError('VALIDATION_ERROR', `Account resolution failed for sales invoice line ${index + 1}`, {
            ...
            if (!receivable) {
                throw new DomainError('VALIDATION_ERROR', `Missing receivable account for sales invoice line ${index + 1}`, {
                    messageKey: 'error.account_resolution.mapping_not_found',
                    details: { lineNo: index + 1, role: FinancialAccountRole.RECEIVABLE_ACCOUNT },
                });
            }
            if (!revenue) {
                throw new DomainError('VALIDATION_ERROR', `Missing revenue account for sales invoice line ${index + 1}`, {
                    messageKey: 'error.account_resolution.mapping_not_found',
                    details: { lineNo: index + 1, role: FinancialAccountRole.REVENUE_ACCOUNT },
                });
            }
            if (vatAmount > EPSILON && !vatOutput) {
                throw new DomainError('VALIDATION_ERROR', `Missing VAT output account for sales invoice line ${index + 1}`, {
                    messageKey: 'error.account_resolution.mapping_not_found',
                    details: { lineNo: index + 1, role: FinancialAccountRole.VAT_OUTPUT_ACCOUNT },
                });
            }
            if (requiresInventory && (!cogs || !inventory)) {
                throw new DomainError('VALIDATION_ERROR', `Missing inventory COGS accounts for sales invoice line ${index + 1}`, {
                    messageKey: 'error.account_resolution.mapping_not_found',
                    details: {
                        lineNo: index + 1,
        ...
        if (!postingLines.length) {
            throw new DomainError('VALIDATION_ERROR', 'No accounting lines generated for sales invoice', {
        ...
        if (Math.abs(totalDebit - totalCredit) > 0.01) {
            throw new DomainError('VALIDATION_ERROR', 'Sales invoice posting command is not balanced', {
    ...
    private resolveLineCostAmount(explicitCostAmount: number, fallbackUnitCost: number, qty: number): number {
        const direct = this.roundAmount(explicitCostAmount);
        if (direct > EPSILON) return direct;
