...
import {
    CreateTreasuryDocumentDbInput,
    TreasuryDocumentRepositoryPort,
    UpdateTreasuryDocumentDbInput,
} from '../ports/TreasuryPorts';
import { JournalDto, JournalEngineUseCases } from '../useCases/JournalEngineUseCases';
import { TREASURY_SOURCE_MODULE, TreasuryPostingBuilder } from './TreasuryPostingBuilder';
import { TreasuryChequeLifecycleService } from './TreasuryChequeLifecycleService';
...
export interface PostTreasuryDocumentResult {
    documentId: string;
    sourceModule: string;
    sourceType: TreasuryDocumentType;
    sourceId: string;
    documentNo: string;
...
export interface ReverseTreasuryDocumentResult {
    documentId: string;
    sourceModule: string;
    sourceType: TreasuryDocumentType;
    sourceId: string;
...
export class TreasuryDocumentService {
    constructor(
        private readonly repository: TreasuryDocumentRepositoryPort,
        private readonly postingBuilder: TreasuryPostingBuilder,
        private readonly chequeLifecycleService: TreasuryChequeLifecycleService,
        private readonly journalEngineUseCases: JournalEngineUseCases,
    ) {
        this.repository.ensureSchema();
    }

    createDocument(context: TreasuryContext, input: CreateTreasuryDocumentInput): TreasuryDocumentEntity {
        ...
        return this.repository.runInTransaction(() => {
            const created = this.repository.createDocument({
                ...
                updatedAt: nowIso,
                lines,
            } satisfies CreateTreasuryDocumentDbInput);
            ...
            if (!document) {
                throw new DomainError('INTERNAL_ERROR', 'Treasury document was not found after create', {
    ...
    updateDocument(context: TreasuryContext, input: UpdateTreasuryDocumentInput): TreasuryDocumentEntity {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeRequired(context.branchId, 'Branch id is required');
        ...
        if (current.header.status !== 'DRAFT') {
            throw new DomainError('INVALID_TRANSITION', 'Only draft treasury documents can be updated', {
        ...
        return this.repository.runInTransaction(() => {
            const updated = this.repository.updateDocument({
                ...
                updatedAt: nowIso,
                lines,
            } satisfies UpdateTreasuryDocumentDbInput);
            ...
            if (!document) {
                throw new DomainError('INTERNAL_ERROR', 'Treasury document was not found after update', {
                    details: { documentId },
                });
            }
            return document;
    ...
    getById(context: Pick<TreasuryContext, 'companyId' | 'branchId'>, documentId: string): TreasuryDocumentEntity {
        return this.requireDocument(context.companyId, context.branchId, documentId);
    }
    async post(context: TreasuryContext, documentId: string): Promise<PostTreasuryDocumentResult> {
        ...
        if (existingJournal) {
            ...
            return {
                documentId: header.id,
                sourceModule: TREASURY_SOURCE_MODULE,
    ...
    async reverse(context: TreasuryContext, input: ReverseTreasuryDocumentCommand): Promise<ReverseTreasuryDocumentResult> {
        ...
        if (!originalJournal) {
            throw new DomainError('VALIDATION_ERROR', 'Treasury document has not been posted yet', {
        ...
        if (document.cheque && (document.cheque.status === 'CLEARED' || document.cheque.status === 'ISSUED_CLEARED')) {
            throw new DomainError('INVALID_TRANSITION', 'Cannot reverse treasury document after cheque cleared', {
    ...
    getPostingStatus(
        context: Pick<TreasuryContext, 'companyId' | 'branchId'>,
        documentId: string,
    ): TreasuryDocumentPostingStatus {
        const document = this.requireDocument(context.companyId, context.branchId, documentId);
        const header = document.header;
    ...
    private requireDocument(companyId: string, branchId: string, documentId: string): TreasuryDocumentEntity {
        ...
        if (!document) {
            throw new DomainError('DOCUMENT_NOT_FOUND', `Treasury document ${normalizedId} was not found`, {
    ...
    private validateControlReferences(header: TreasuryDocumentHeaderEntity): void {
        if ((header.docType === 'CASH_RECEIPT' || header.docType === 'CASH_PAYMENT') && !this.normalizeNullable(header.cashAccountId)) {
            throw new DomainError('VALIDATION_ERROR', 'Cash account is required for cash document', {
        ...
        if ((header.docType === 'BANK_RECEIPT' || header.docType === 'BANK_PAYMENT') && !this.normalizeNullable(header.bankAccountId)) {
            throw new DomainError('VALIDATION_ERROR', 'Bank account is required for bank document', {
    ...
    private assertCanPost(header: TreasuryDocumentHeaderEntity): void {
        if (header.status === 'CANCELLED') {
            throw new DomainError('INVALID_TRANSITION', 'Cancelled treasury document cannot be posted', {
    ...
    private validateLines(companyId: string, docType: TreasuryDocumentType, lines: TreasuryDocumentLineEntity[]): void {
        if (!lines.length) {
            throw new DomainError('VALIDATION_ERROR', 'Treasury document lines are required', {
        ...
        for (let index = 0; index < lines.length; index += 1) {
            ...
            if (Number(line.amount || 0) <= 0) {
                throw new DomainError('VALIDATION_ERROR', `Line ${index + 1}: amount must be greater than zero`, {
            ...
            if (!accountState.exists || !accountState.isActive || !accountState.isPosting) {
                throw new DomainError('VALIDATION_ERROR', `Line ${index + 1}: account is invalid for posting`, {
    ...
    private requirePartner(partnerId: string): void {
        const partner = this.repository.getPartnerById(partnerId);
        if (!partner || !partner.isActive) {
            throw new DomainError('VALIDATION_ERROR', 'Partner was not found for treasury posting', {
    ...
    private requireChequeForType(companyId: string, docType: TreasuryDocumentType, documentId: string) {
        if (docType !== 'CHEQUE_RECEIPT' && docType !== 'CHEQUE_PAYMENT') {
            return null;
        }
        const cheque = this.repository.getChequeByDocumentId(companyId, documentId);
        if (!cheque) {
            throw new DomainError('VALIDATION_ERROR', 'Cheque details are required for cheque treasury document', {
    ...
    private upsertChequeIfProvided(
        companyId: string,
        branchId: string,
        document: TreasuryDocumentEntity,
        chequeInput: CreateTreasuryDocumentInput['cheque'] | UpdateTreasuryDocumentInput['cheque'] | null,
        nowIso: string,
    ): void {
        ...
        if (!chequeInput && this.isChequeDocument(document.header.docType) && !existingCheque) {
            throw new DomainError('VALIDATION_ERROR', 'Cheque information is required for cheque document', {
                messageKey: 'validation.treasury_document.cheque_required',
                details: { documentId: document.header.id, docType: document.header.docType },
            });
        }
        if (!chequeInput) return;

        const direction: 'RECEIVED' | 'ISSUED' = document.header.docType === 'CHEQUE_RECEIPT' ? 'RECEIVED' : 'ISSUED';
    ...
    private resolveOriginalJournal(companyId: string, header: TreasuryDocumentHeaderEntity, sourceVersion: number): JournalDto | null {
    ...
    private resolveExistingReversal(
        ...
        fallbackReversalId: string | null,
    ): JournalDto | null {
    ...
    private isChequeDocument(docType: TreasuryDocumentType): boolean {
        return docType === 'CHEQUE_RECEIPT' || docType === 'CHEQUE_PAYMENT';
    }

    private normalizeDocType(docType: TreasuryDocumentType): TreasuryDocumentType {
        const normalized = String(docType || '').trim().toUpperCase();
        if (normalized === 'CASH_PAYMENT') return 'CASH_PAYMENT';
        if (normalized === 'BANK_RECEIPT') return 'BANK_RECEIPT';
        if (normalized === 'BANK_PAYMENT') return 'BANK_PAYMENT';
        if (normalized === 'CHEQUE_RECEIPT') return 'CHEQUE_RECEIPT';
        if (normalized === 'CHEQUE_PAYMENT') return 'CHEQUE_PAYMENT';
        if (normalized === 'CASH_RECEIPT') return 'CASH_RECEIPT';

        throw new DomainError('VALIDATION_ERROR', 'Unsupported treasury document type', {
