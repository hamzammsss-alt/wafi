...
import {
    ...
    PurchaseInvoicePreparationLineDto,
    PurchaseLineRemainingDto,
...
} from '../ports/PurchaseOperationsPorts';
import { JournalDto, JournalEngineUseCases } from '../useCases/JournalEngineUseCases';
import { PurchaseOperationsAccountingBuilder } from './PurchaseOperationsAccountingBuilder';
import { PurchaseStockLedgerService } from './PurchaseStockLedgerService';
...
export interface PurchaseDocumentPostResult {
    documentId: string;
...
export interface PurchaseDocumentCancelResult {
    documentId: string;
    docType: PurchaseOperationDocumentType;
    docNo: string;
...
export class PurchaseOperationsService {
    constructor(
        ...
        private readonly stockLedgerService: PurchaseStockLedgerService,
        private readonly accountingBuilder: PurchaseOperationsAccountingBuilder,
        private readonly journalEngineUseCases: JournalEngineUseCases,
    ) {
        this.repository.ensureSchema();
    }

    createRequest(context: PurchaseContext, input: CreatePurchaseOperationDocumentInput): PurchaseOperationDocumentEntity {
        return this.createDocument(context, 'PURCHASE_REQUEST', input);
    }

    updateRequest(context: PurchaseContext, input: UpdatePurchaseOperationDocumentInput): PurchaseOperationDocumentEntity {
        return this.updateDocument(context, 'PURCHASE_REQUEST', input);
    }

    getRequestById(context: Pick<PurchaseContext, 'companyId' | 'branchId'>, documentId: string): PurchaseOperationDocumentEntity {
        return this.getDocument(context, 'PURCHASE_REQUEST', documentId);
    }

    confirmRequest(context: PurchaseContext, documentId: string): PurchaseOperationDocumentEntity {
        ...
        if (request.header.status === 'CANCELLED') {
            throw new DomainError('INVALID_TRANSITION', 'Cancelled request cannot be confirmed', {
    ...
    cancelRequest(context: PurchaseContext, documentId: string): PurchaseOperationDocumentEntity {
        const request = this.requireDocument(context.companyId, context.branchId, documentId, 'PURCHASE_REQUEST');
        if (request.header.status === 'CANCELLED') return request;
    ...
    convertRequestToRfq(context: PurchaseContext, input: ConvertRequestToRfqInput): PurchaseConversionResult {
        return this.convertSourceToTarget(
            context,
            this.normalizeRequired(input.requestId, 'Request id is required'),
            'PURCHASE_REQUEST',
            'PURCHASE_RFQ',
        );
    }

    convertRequestToOrder(context: PurchaseContext, input: ConvertRequestToOrderInput): PurchaseConversionResult {
        return this.convertSourceToTarget(
            context,
            this.normalizeRequired(input.requestId, 'Request id is required'),
            'PURCHASE_REQUEST',
    ...
    createRfq(context: PurchaseContext, input: CreatePurchaseOperationDocumentInput): PurchaseOperationDocumentEntity {
        return this.createDocument(context, 'PURCHASE_RFQ', input);
    }

    updateRfq(context: PurchaseContext, input: UpdatePurchaseOperationDocumentInput): PurchaseOperationDocumentEntity {
        return this.updateDocument(context, 'PURCHASE_RFQ', input);
    }

    getRfqById(context: Pick<PurchaseContext, 'companyId' | 'branchId'>, documentId: string): PurchaseOperationDocumentEntity {
        return this.getDocument(context, 'PURCHASE_RFQ', documentId);
    }

    confirmRfq(context: PurchaseContext, documentId: string): PurchaseOperationDocumentEntity {
        const rfq = this.requireDocument(context.companyId, context.branchId, documentId, 'PURCHASE_RFQ');
        if (rfq.header.status === 'CANCELLED') {
            throw new DomainError('INVALID_TRANSITION', 'Cancelled RFQ cannot be confirmed', {
    ...
    cancelRfq(context: PurchaseContext, documentId: string): PurchaseOperationDocumentEntity {
        const rfq = this.requireDocument(context.companyId, context.branchId, documentId, 'PURCHASE_RFQ');
        if (rfq.header.status === 'CANCELLED') return rfq;
    ...
    convertRfqToOrder(context: PurchaseContext, input: ConvertRfqToOrderInput): PurchaseConversionResult {
        return this.convertSourceToTarget(
            context,
            this.normalizeRequired(input.rfqId, 'RFQ id is required'),
            'PURCHASE_RFQ',
            'PURCHASE_ORDER',
        );
    }

    createOrder(context: PurchaseContext, input: CreatePurchaseOperationDocumentInput): PurchaseOperationDocumentEntity {
        return this.createDocument(context, 'PURCHASE_ORDER', input);
    }

    updateOrder(context: PurchaseContext, input: UpdatePurchaseOperationDocumentInput): PurchaseOperationDocumentEntity {
        return this.updateDocument(context, 'PURCHASE_ORDER', input);
    }

    getOrderById(context: Pick<PurchaseContext, 'companyId' | 'branchId'>, documentId: string): PurchaseOperationDocumentEntity {
        return this.getDocument(context, 'PURCHASE_ORDER', documentId);
    }

    confirmOrder(context: PurchaseContext, orderId: string): PurchaseOperationDocumentEntity {
        ...
        if (order.header.status === 'CANCELLED') {
            throw new DomainError('INVALID_TRANSITION', 'Cancelled purchase order cannot be confirmed', {
    ...
    cancelOrder(context: PurchaseContext, orderId: string): PurchaseOperationDocumentEntity {
        const order = this.requireDocument(context.companyId, context.branchId, orderId, 'PURCHASE_ORDER');
        if (order.header.status === 'CANCELLED') {
            return order;
        }
        ...
        const hasFulfillment = order.lines.some((line) =>
            this.round(line.receivedQty) > EPSILON || this.round(line.billedQty) > EPSILON,
        );
        if (hasFulfillment) {
    ...
    getOrderFulfillmentStatus(context: Pick<PurchaseContext, 'companyId' | 'branchId'>, orderId: string): PurchaseOrderFulfillmentSummary {
        const order = this.requireDocument(context.companyId, context.branchId, orderId, 'PURCHASE_ORDER');
        return this.buildFulfillmentSummary(order);
    }

    convertOrderToReceipt(context: PurchaseContext, input: ConvertOrderToReceiptInput): PurchaseConversionResult {
        ...
        if (!sourceRows.length) {
            throw new DomainError('VALIDATION_ERROR', 'Order has no remaining quantity for receipt conversion', {
        ...
        const targetLines: PurchaseCreateDocumentDbInput['lines'] = [];
        const links: PurchaseCreateLinkInput[] = [];
        ...
        for (const source of sourceRows) {
            ...
            if (!policy.allowOverReceipt && qty - source.remaining > EPSILON) {
                throw new DomainError('VALIDATION_ERROR', 'Over conversion to receipt is not allowed', {
        ...
        if (!targetLines.length) {
            throw new DomainError('VALIDATION_ERROR', 'No receipt lines generated from order conversion', {
                messageKey: 'validation.purchase_operations.conversion.no_remaining_qty',
                details: { orderId },
            });
        }
    ...
    orderToInvoicePreparation(
        context: Pick<PurchaseContext, 'companyId' | 'branchId'>,
        orderId: string,
    ): PurchaseInvoicePreparationDto {
        const order = this.requireDocument(context.companyId, context.branchId, orderId, 'PURCHASE_ORDER');
    ...
    createGoodsReceiptNote(context: PurchaseContext, input: CreatePurchaseOperationDocumentInput): PurchaseOperationDocumentEntity {
        return this.createDocument(context, 'GOODS_RECEIPT_NOTE', input);
    }

    updateGoodsReceiptNote(context: PurchaseContext, input: UpdatePurchaseOperationDocumentInput): PurchaseOperationDocumentEntity {
        return this.updateDocument(context, 'GOODS_RECEIPT_NOTE', input);
    }

    getGoodsReceiptNoteById(context: Pick<PurchaseContext, 'companyId' | 'branchId'>, documentId: string): PurchaseOperationDocumentEntity {
        return this.getDocument(context, 'GOODS_RECEIPT_NOTE', documentId);
    }

    async postGoodsReceiptNote(context: PurchaseContext, input: PostGoodsReceiptNoteInput): Promise<PurchaseDocumentPostResult> {
    ...
    async cancelGoodsReceiptNote(context: PurchaseContext, input: CancelGoodsReceiptNoteInput): Promise<PurchaseDocumentCancelResult> {
        const documentId = this.normalizeRequired(input.documentId, 'Receipt note id is required');
    ...
    receiptToInvoicePreparation(
        ...
        return this.prepareInvoiceDtoFromDocument(
            receipt,
            (line) => this.round(Math.max(0, line.qty - line.returnedQty - line.billedQty)),
        );
    }

    convertReceiptToReturn(context: PurchaseContext, input: ConvertReceiptToReturnInput): PurchaseConversionResult {
        const receiptId = this.normalizeRequired(input.receiptId, 'Receipt id is required');
        const receipt = this.requireDocument(context.companyId, context.branchId, receiptId, 'GOODS_RECEIPT_NOTE');
        if (receipt.header.status === 'CANCELLED') {
            throw new DomainError('INVALID_TRANSITION', 'Cancelled receipt cannot be converted', {
        ...
        if (!sourceRows.length) {
            throw new DomainError('VALIDATION_ERROR', 'Receipt has no remaining quantity for return conversion', {
                messageKey: 'validation.purchase_operations.conversion.no_remaining_qty',
                details: { receiptId },
            });
        }
        ...
        if (!targetLines.length) {
            throw new DomainError('VALIDATION_ERROR', 'No purchase return lines generated from receipt conversion', {
                messageKey: 'validation.purchase_operations.conversion.no_remaining_qty',
                details: { receiptId },
            });
        }
    ...
    createPurchaseReturn(context: PurchaseContext, input: CreatePurchaseOperationDocumentInput): PurchaseOperationDocumentEntity {
        return this.createDocument(context, 'PURCHASE_RETURN', input);
    }

    updatePurchaseReturn(context: PurchaseContext, input: UpdatePurchaseOperationDocumentInput): PurchaseOperationDocumentEntity {
        return this.updateDocument(context, 'PURCHASE_RETURN', input);
    }

    getPurchaseReturnById(context: Pick<PurchaseContext, 'companyId' | 'branchId'>, documentId: string): PurchaseOperationDocumentEntity {
        return this.getDocument(context, 'PURCHASE_RETURN', documentId);
    }

    async postPurchaseReturn(context: PurchaseContext, input: PostPurchaseReturnInput): Promise<PurchaseDocumentPostResult> {
    ...
    async cancelPurchaseReturn(context: PurchaseContext, input: CancelPurchaseReturnInput): Promise<PurchaseDocumentCancelResult> {
        const documentId = this.normalizeRequired(input.documentId, 'Purchase return id is required');
        const reverseDate = this.normalizeDate(input.reverseDate, 'Reverse date is required');
    ...
    private convertSourceToTarget(
        context: PurchaseContext,
        sourceDocumentId: string,
        sourceDocType: 'PURCHASE_REQUEST' | 'PURCHASE_RFQ',
        targetDocType: 'PURCHASE_RFQ' | 'PURCHASE_ORDER',
        ...
        if (source.header.status === 'CANCELLED') {
            throw new DomainError('INVALID_TRANSITION', 'Cancelled document cannot be converted', {
        ...
        if (this.requiresVendor(targetDocType) && !this.normalizeNullable(source.header.vendorId)) {
            throw new DomainError('VALIDATION_ERROR', 'Vendor is required for conversion target document', {
        ...
        if (!sourceLines.length) {
            throw new DomainError('VALIDATION_ERROR', 'Source document has no remaining quantity for conversion', {
                messageKey: 'validation.purchase_operations.conversion.no_remaining_qty',
                details: { sourceDocumentId, sourceDocType, targetDocType },
            });
        }
        ...
        this.repository.runInTransaction(() => {
            ...
            const links: PurchaseCreateLinkInput[] = sourceLines.map((entry, index) => ({
                id: this.repository.nextIdentity(),
                companyId: context.companyId,
                branchId: context.branchId,
                sourceDocType,
    ...
    private createDocument(
        ...
        return this.repository.runInTransaction(() => this.repository.createDocument({
            ...
            updatedAt: now,
            lines,
        } satisfies PurchaseCreateDocumentDbInput));
    }
    ...
    private prepareInvoiceDtoFromDocument(
        document: PurchaseOperationDocumentEntity,
        quantitySelector: (line: PurchaseOperationLineEntity) => number,
    ): PurchaseInvoicePreparationDto {
        const lines: PurchaseInvoicePreparationLineDto[] = [];
    ...
    private mapLineForInvoice(line: PurchaseOperationLineEntity, qty: number): PurchaseInvoicePreparationLineDto {
    ...
    private assertCanPost(header: PurchaseOperationHeaderEntity): void {
    ...
    private validatePostingLines(
        ...
        lines: Array<{
            id: string;
            itemId: string | null;
            lineType: 'INVENTORY' | 'EXPENSE' | 'SERVICE';
    ...
    private validateDraftLines(
        ...
        for (let index = 0; index < lines.length; index += 1) {
            ...
            if (isInventoryLine && !item) {
                throw new DomainError('VALIDATION_ERROR', `Line ${lineNo}: item is required`, {
    ...
    private applyReceiptProgressByLinks(context: PurchaseContext, receiptId: string, sign: 1 | -1): void {
    ...
    private applyReturnProgressByLinks(context: PurchaseContext, purchaseReturnId: string, sign: 1 | -1): void {
        ...
        const touchedOrders = new Set<string>();
        const cachedOrderLinksByReceipt = new Map<string, ReturnType<PurchaseOperationsRepositoryPort['listLinksByTarget']>>();
    ...
    private refreshOrderCommitmentSnapshot(context: PurchaseContext, orderId: string): void {
        const order = this.requireDocument(context.companyId, context.branchId, orderId, 'PURCHASE_ORDER');
        if (order.header.status === 'CANCELLED') {
        ...
        const now = new Date().toISOString();
        const commitments: PurchaseCommitmentRecord[] = [];
    ...
    private buildFulfillmentSummary(order: PurchaseOperationDocumentEntity): PurchaseOrderFulfillmentSummary {
    ...
    private mapLineRemaining(line: PurchaseOperationLineEntity): PurchaseLineRemainingDto {
        const remaining = this.computeOrderRemainingQty(line);
        let lineStatus: PurchaseOrderFulfillmentStatus = 'OPEN';
    ...
    private computeFulfillmentStatus(
        orderStatus: PurchaseOperationStatus,
    ...
    private mapOrderStatusFromFulfillment(
        currentStatus: PurchaseOperationStatus,
    ...
    private computeOrderRemainingQty(line: PurchaseOperationLineEntity): number {
        return this.round(Math.max(0, line.qty - line.receivedQty - line.returnedQty - line.billedQty));
    }

    private assertNoActiveReturnsForReceipt(companyId: string, branchId: string, receiptId: string): void {
        ...
        for (const link of links) {
            ...
            if (targetHeader.status !== 'CANCELLED') {
                throw new DomainError('INVALID_TRANSITION', 'Receipt note with active purchase return cannot be cancelled', {
    ...
    private mapSourceLineToTarget(
        lineNo: number,
        sourceLine: PurchaseOperationLineEntity,
        qty: number,
        now: string,
    ): PurchaseCreateDocumentDbInput['lines'][number] {
    ...
    private normalizeInputLines(
        inputLines: Array<{
            id?: string;
            itemId?: string | null;
            lineType?: 'INVENTORY' | 'EXPENSE' | 'SERVICE' | null;
            warehouseId?: string | null;
            qty: number;
            unitPrice: number;
        ...
        nowIso: string,
    ): PurchaseCreateDocumentDbInput['lines'] {
    ...
    private requiresVendor(docType: PurchaseOperationDocumentType): boolean {
        return docType !== 'PURCHASE_REQUEST';
    }

    private ensureVendor(vendorId: string): void {
    ...
    private requireDocument(
        ...
        branchId: string,
        documentId: string,
        expectedDocType?: PurchaseOperationDocumentType,
    ): PurchaseOperationDocumentEntity {
        ...
        if (!document) {
            throw new DomainError('DOCUMENT_NOT_FOUND', 'Purchase document was not found', {
    ...
    private normalizeDocTypeNullable(docType: PurchaseOperationDocumentType | null | undefined): PurchaseOperationDocumentType | null {
        ...
        if (normalized === 'PURCHASE_REQUEST') return 'PURCHASE_REQUEST';
    ...
    private normalizeRequired(value: string | null | undefined, errorMessage: string): string {
