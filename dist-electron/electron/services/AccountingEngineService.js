...
exports.AccountingEngineService = void 0;
...
class AccountingEngineService {
    static listSubAccounts(accountId) {
        if (accountId) {
    ...
    static createSubAccount(data) {
        ...
        const account = database_1.db.prepare('SELECT id FROM accounts WHERE id = ?').get(data.account_id);
        if (!account)
            throw new Error('Parent account does not exist');
        const normalizedName = normalizeText(data.name);
        const duplicate = database_1.db
            .prepare('SELECT id FROM ae_sub_accounts WHERE account_id = ? AND normalized_name = ? LIMIT 1')
            .get(data.account_id, normalizedName);
        if (duplicate)
            throw new Error('Sub account already exists for this parent account');
    ...
    `).run(id, data.account_id, data.code || null, data.name.trim(), normalizedName);
        return database_1.db.prepare('SELECT * FROM ae_sub_accounts WHERE id = ?').get(id);
    }
    static createReference(data) {
        if (!data?.ref_type?.trim())
            throw new Error('ref_type is required');
        if (!data?.ref_name?.trim())
            throw new Error('ref_name is required');
        const refType = data.ref_type.trim().toUpperCase();
        const normalizedName = normalizeText(data.ref_name);
        const duplicateByName = database_1.db
            .prepare('SELECT id FROM ae_references WHERE ref_type = ? AND normalized_name = ? LIMIT 1')
            .get(refType, normalizedName);
        if (duplicateByName)
            throw new Error('Reference already exists in this reference type');
        if (data.ref_code?.trim()) {
            const duplicateByCode = database_1.db
                .prepare('SELECT id FROM ae_references WHERE ref_type = ? AND ref_code = ? LIMIT 1')
                .get(refType, data.ref_code.trim());
            if (duplicateByCode)
                throw new Error('Reference code already exists in this reference type');
        }
        const id = (0, uuid_1.v4)();
        database_1.db.prepare(`
    ...
    `).run(id, refType, data.ref_code?.trim() || null, data.ref_name.trim(), normalizedName);
        return database_1.db.prepare('SELECT * FROM ae_references WHERE id = ?').get(id);
    }
    static saveDraftVoucher(input) {
    ...
    static postDraftVoucher(voucherId) {
    ...
    static getVoucher(id) {
        const header = database_1.db.prepare('SELECT * FROM ae_vouchers WHERE id = ?').get(id);
        if (!header)
            return null;
        const lines = database_1.db
    ...
    static ensureVoucherInput(input) {
        if (!input?.voucher_type?.trim())
            throw new Error('voucher_type is required');
        if (!input?.voucher_date?.trim())
            throw new Error('voucher_date is required');
    ...
    static validateLines(lines) {
        ...
        lines.forEach((line, index) => {
            if (!line.account_id)
                throw new Error(`Line ${index + 1}: account_id is required`);
            ...
            const account = database_1.db.prepare(`
        SELECT id, code, name,
    ...
    static buildVoucherPayload(input) {
    ...
    static getNextVoucherNo(prefix, year) {
        const tx = database_1.db.transaction(() => {
            ...
            if (row) {
                ...
                database_1.db.prepare('INSERT INTO ae_voucher_counters (prefix, year, current_value) VALUES (?, ?, 0)').run(prefix, year);
            }
            currentValue += 1;
            database_1.db.prepare('UPDATE ae_voucher_counters SET current_value = ? WHERE prefix = ? AND year = ?').run(currentValue, prefix, year);
            return `${prefix}-${year}-${String(currentValue).padStart(4, '0')}`;
        });
        return tx();
    }
}
exports.AccountingEngineService = AccountingEngineService;
