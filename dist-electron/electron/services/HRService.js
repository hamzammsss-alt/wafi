...
class HRService {
    ...
    static generateChildAccountCode(parentId) {
        ...
        siblings.forEach((s) => {
            ...
            if (!Number.isNaN(val) && val > maxSuffix)
                maxSuffix = val;
        });
        return parentAcc.account_code + String(maxSuffix + 1).padStart(4, '0');
    }
    static ensureEmployeeLinkedAccount(personal) {
        let linkedAccountId = personal?.linked_account_id || null;
        let explicitParentId = null;
        if (linkedAccountId) {
            const account = database_1.db.prepare('SELECT id, is_transactional FROM gl_chart_of_accounts WHERE id = ?').get(linkedAccountId);
            if (account?.id) {
                if (Number(account.is_transactional) === 1)
        ...
        if (!parentId)
            return null;
        const accountTypeRow = database_1.db.prepare('SELECT account_type FROM gl_chart_of_accounts WHERE id = ?').get(parentId);
