    ...
    static save(params, scope, auditCtx) {
        ...
        database_1.db.transaction(() => {
            ...
            (Array.isArray(lines) ? lines : []).forEach((line, index) => {
                ...
                insertLine.run((0, uuid_1.v4)(), id, index + 1, accountId, debit, credit, String(line?.line_description || line?.description || '').trim() || null, line?.cost_center_id || null, String(line?.invoice_ref || '').trim() || null, String(line?.tax_ref || '').trim() || null, String(line?.sub_account_id || '').trim() || null, String(line?.due_date || '').trim() || null, String(line?.customer_id || '').trim() || null, line?.is_returned ? 1 : 0, String(line?.bank_account_id || '').trim() || null);
            });
        })();
