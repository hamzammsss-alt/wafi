...
class AssetService {
    // --- Asset Management ---
    static getAssets() {
        return database_1.db.prepare('SELECT * FROM assets ORDER BY name').all();
    }
    static saveAsset(data) {
        const { id, code, name, type, category_id, purchase_date, purchase_cost, salvage_value, life_years, depreciation_rate, status, payment_method, payment_account_id } = data;
        if (id && id !== 0 && id !== '0') {
            // Update
            ...
            if (payment_method && payment_account_id && category_id) {
                try {
                    const category = database_1.db.prepare('SELECT * FROM asset_categories WHERE id = ?').get(category_id);
                    if (category && category.asset_account_id) {
                        ...
                        if (amount > 0) {
                            ...
                            database_1.db.prepare('UPDATE accounts SET balance = COALESCE(balance, 0) + ? WHERE id = ?')
    ...
    static getCategories() {
        const categories = database_1.db.prepare('SELECT * FROM asset_categories ORDER BY code').all();
        if (categories.length === 0) {
            ...
            `);
            const updateStmt = database_1.db.prepare("UPDATE asset_categories SET depreciation_rate = ? WHERE code = ?");
            seed.forEach(cat => {
                stmt.run((0, uuid_1.v4)(), cat.code, cat.name_ar, cat.name_en, cat.rate);
            });
            return database_1.db.prepare('SELECT * FROM asset_categories ORDER BY code').all();
        }
        return categories;
    }
    ...
    static getNextCode() {
        try {
            const last = database_1.db.prepare('SELECT code FROM assets ORDER BY created_at DESC LIMIT 1').get();
            if (!last || !last.code)
                return 'AST-0001';
    ...
    // --- Depreciation ---
    static calculateDepreciation(assetId) {
        // Simple Straight Line: (Cost - Salvage) / Life
        const asset = database_1.db.prepare('SELECT * FROM assets WHERE id = ?').get(assetId);
        if (!asset)
    ...
    static postDepreciation(assetId, amount, date) {
        ...
        const stmt = database_1.db.prepare(`
            ...
            VALUES (?, ?, ?, ?)
        `);
        stmt.run((0, uuid_1.v4)(), assetId, date, amount.toString());
        return { success: true };
    }
}
