...
class AccountService {
    ...
    static createAccount(data) {
        if (!data.account_code || !data.name_ar)
            throw new Error("Code and Name (AR) are required");
        // Check duplicate code
        const existing = database_1.db.prepare('SELECT id FROM gl_chart_of_accounts WHERE account_code = ?').get(data.account_code);
        if (existing)
            throw new Error("Account Code already exists");
        const normalizedName = this.normalizeName(data.name_ar);
        const sameName = database_1.db.prepare(`
            SELECT id
        ...
        `).get(normalizedName);
        if (sameName)
            throw new Error("اسم الحساب موجود مسبقاً");
        let validCurrencyId = data.currency_id || null;
        if (validCurrencyId) {
            const exists = database_1.db.prepare('SELECT id FROM currencies WHERE id = ?').get(validCurrencyId);
            if (!exists) {
                const byCode = database_1.db.prepare('SELECT id FROM currencies WHERE code = ?').get(validCurrencyId);
                if (byCode)
    ...
    static deleteAccount(id) {
        ...
        const children = database_1.db.prepare('SELECT count(*) as c FROM gl_chart_of_accounts WHERE parent_id = ?').get(id).c;
        if (children > 0)
        ...
        database_1.db.prepare('DELETE FROM gl_chart_of_accounts WHERE id = ?').run(id);
        return { success: true };
    }
}
