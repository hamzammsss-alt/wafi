...
class TreasuryService {
    static safeConsole(method, ...args) {
        try {
    ...
    static resolveBankAccountPostingDetails(bankAccountId) {
    ...
    static getApprovalInboxViewSql() {
        return `
...
WHERE status LIKE 'PENDING_APPROVAL%';
    ...
    static refreshApprovalInboxView() {
        database_1.db.exec(TreasuryService.getApprovalInboxViewSql());
        TreasuryService.safeConsole('log', '[TreasuryService] Refreshed approval inbox view.');
    }
    static cleanupBrokenTreasuryObjects() {
    ...
    static ensureTreasuryVoucherColumns() {
        ...
        if (!cols.some((c) => c.name === 'extra_data')) {
            database_1.db.prepare("ALTER TABLE treasury_vouchers ADD COLUMN extra_data TEXT").run();
        }
    }
    static ensureChequesColumns() {
    ...
    static ensureJournalEntryLineColumns() {
    ...
    static prepareTreasuryPersistence() {
        try {
    ...
    static resolveReferenceLink(reference, payeeType) {
        const rawReference = String(reference || '').trim();
        if (!rawReference) {
            return { partnerId: null, linkedAccountId: null };
        }
        const normalized = rawReference.toLowerCase();
        const codeCandidate = rawReference.split('-')[0]?.trim();
        const matchPartnerByType = (type) => {
            if (type === 'EMPLOYEE') {
                const byCode = codeCandidate
                    ? database_1.db.prepare('SELECT id, linked_account_id FROM hr_employees WHERE employee_code = ? LIMIT 1').get(codeCandidate)
                    : null;
                if (byCode?.id)
                    return byCode;
                return database_1.db.prepare("SELECT id, linked_account_id FROM hr_employees WHERE LOWER(COALESCE(name_ar, name_en, full_name, '')) = ? LIMIT 1").get(normalized);
            }
            const byCode = codeCandidate
                ? database_1.db.prepare('SELECT id, linked_account_id FROM business_partners WHERE code = ? LIMIT 1').get(codeCandidate)
                : null;
            if (byCode?.id)
                return byCode;
            return database_1.db.prepare("SELECT id, linked_account_id FROM business_partners WHERE LOWER(COALESCE(name_ar, name_en, name, '')) = ? LIMIT 1").get(normalized);
        };
    ...
    static reserveTreasuryVoucherNo(prefix, dateValue) {
        const year = dateValue ? new Date(dateValue).getFullYear() : new Date().getFullYear();
        for (let i = 0; i < 50; i++) {
            const candidate = JournalService_1.JournalService.getNextVoucherNo(prefix, year);
            const exists = database_1.db.prepare('SELECT 1 FROM treasury_vouchers WHERE voucher_no = ? LIMIT 1').get(candidate);
            if (!exists) {
                JournalService_1.JournalService.incrementVoucherNo(prefix, year);
                return candidate;
            }
            JournalService_1.JournalService.incrementVoucherNo(prefix, year);
        }
    ...
    static createReceiptVoucher(data) {
        ...
        if (voucherNo === 'NEW' || !voucherNo) {
            ...
            const duplicate = database_1.db.prepare('SELECT 1 FROM treasury_vouchers WHERE voucher_no = ? LIMIT 1').get(voucherNo);
            if (duplicate)
                throw new Error('رقم السند مستخدم مسبقاً');
        }
            ...
            if (checks && checks.length > 0) {
                ...
                for (const c of checks) {
                    ...
                    if (!persistedBankId && c.bank_account_id) {
                        const bankAccountRow = database_1.db.prepare("SELECT bank_id FROM bank_accounts WHERE id = ?").get(c.bank_account_id);
    ...
    static deletePaymentVoucher(id) {
        const voucher = database_1.db.prepare(`
        ...
        const deleteVoucher = database_1.db.prepare(`
            DELETE FROM treasury_vouchers
            WHERE id = ? AND voucher_type = 'PAYMENT'
        `);
        const deleteJournalLines = database_1.db.prepare(`
            DELETE FROM journal_entry_lines
            WHERE journal_entry_id = ?
        `);
        const deleteJournalHeader = database_1.db.prepare(`
    ...
    static updatePaymentVoucherStatus(id, status) {
        const voucher = database_1.db.prepare(`
        ...
        const updateVoucher = database_1.db.prepare(`
            UPDATE treasury_vouchers
            SET status = ?
            WHERE id = ? AND voucher_type = 'PAYMENT'
        `);
        const updateJournal = database_1.db.prepare(`
    ...
    static deleteReceiptVoucher(id) {
        const voucher = database_1.db.prepare(`
            SELECT id, voucher_no, journal_header_id
            FROM treasury_vouchers
    ...
    static updateReceiptVoucherStatus(id, status) {
        const voucher = database_1.db.prepare(`
            SELECT id, voucher_no, status, journal_header_id
            FROM treasury_vouchers
            WHERE id = ? AND voucher_type = 'RECEIPT'
