...
exports.seedPalestinianData = void 0;
const uuid_1 = require("uuid");
const seedPalestinianData = (db) => {
    console.log('[Seed] Starting Palestinian Data Seeding (Standard COA)...');
    // --- 1. Currencies ---
    const currenciesMap = new Map(); // Code -> ID
    ...
    if (currencyCount === 0) {
        ...
        rows.forEach((r) => currenciesMap.set(r.code, r.id));
    }
    // --- 2. Taxes ---
    const taxCount = db.prepare('SELECT COUNT(*) as count FROM taxes').get().count;
    if (taxCount === 0) {
        console.log('[Seed] Seeding Taxes...');
        db.prepare(`INSERT INTO taxes (id, name_ar, name_en, rate, type, is_active) VALUES (?, ?, ?, ?, ?, ?)`).run((0, uuid_1.v4)(), 'ضريبة القيمة المضافة', 'VAT', 0.16, 'VAT', 1);
        db.prepare(`INSERT INTO taxes (id, name_ar, name_en, rate, type, is_active) VALUES (?, ?, ?, ?, ?, ?)`).run((0, uuid_1.v4)(), 'ضريبة الشراء', 'Purchase Tax', 0.00, 'PURCHASE', 1);
    }
    // --- 3. Chart of Accounts ---
    ...
    const accounts = [
        ...
        { code: '1111', name: 'الصندوق شيكل', type: 'ASSET', trans: 0, currency: 'NIS' },
        { code: '1112', name: 'الصندوق دينار', type: 'ASSET', trans: 0, currency: 'JOD' },
        { code: '1113', name: 'الصندوق دولار', type: 'ASSET', trans: 0, currency: 'USD' },
        { code: '112', name: 'البنوك', type: 'ASSET', trans: 0 },
        { code: '1121', name: 'البنوك شيكل', type: 'ASSET', trans: 0, currency: 'NIS' },
        { code: '1122', name: 'البنوك دينار', type: 'ASSET', trans: 0, currency: 'JOD' },
        { code: '1123', name: 'البنوك دولار', type: 'ASSET', trans: 0, currency: 'USD' },
    ...
    const insertAccountLegacy = db.prepare(`
        ...
        `);
    const updateChartAccount = db.prepare(`
            UPDATE gl_chart_of_accounts SET
                name_ar = @name_ar,
                name_en = @name_en,
                parent_id = @parent_id,
                account_type = @type,
                ...
                requires_cost_center = @cc
            WHERE account_code = @code
        `);
    const updateLegacyAccount = db.prepare(`
            UPDATE accounts SET
                name = @name,
                type = @type,
                parent_id = @parent_id,
                account_level = @level,
                posting_allowed = @is_trans,
                is_transactional = @is_trans,
                is_group = @is_group,
                currency = @currency,
    ...
    db.transaction(() => {
        for (const acc of accounts) {
            const existingChart = db.prepare('SELECT id FROM gl_chart_of_accounts WHERE account_code = ?').get(acc.code);
            const existingLegacy = db.prepare('SELECT id FROM accounts WHERE code = ?').get(acc.code);
            const id = existingChart?.id || existingLegacy?.id || (0, uuid_1.v4)();
            ...
            if (existingLegacy) {
                ...
                insertAccountLegacy.run({
                    ...
                    balance: '0',
                    parent_id: parentId,
                    account_level: acc.code.length,
                    is_trans: acc.trans,
                    currency: acc.currency || 'ILS'
                });
                db.prepare(`
                        UPDATE accounts
                        SET posting_allowed = ?, is_group = ?, currency_code = ?
                        WHERE code = ?
                    `).run(acc.trans, acc.trans ? 0 : 1, acc.currency || 'ILS', acc.code);
            }
        }
    })();
...
exports.seedPalestinianData = seedPalestinianData;
