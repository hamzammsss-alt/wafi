...
const AccountingEngineService_1 = require("./services/AccountingEngineService");
...
const printing_ipc_1 = require("../src/main/ipc/printing.ipc");
...
safeHandle('email:send', async (payload) => {
    const { host, port, secure, user, pass, to, cc, subject, text, html, } = payload;
...
const registerIPCHandlers = (db) => {
  ...
  `);
    ...
    safeHandle('ae:list-sub-accounts', (event, accountId) => AccountingEngineService_1.AccountingEngineService.listSubAccounts(accountId));
    safeHandle('ae:create-sub-account', (event, data) => AccountingEngineService_1.AccountingEngineService.createSubAccount(data));
    safeHandle('ae:list-references', (event, refType) => AccountingEngineService_1.AccountingEngineService.listReferences(refType));
    safeHandle('ae:create-reference', (event, data) => AccountingEngineService_1.AccountingEngineService.createReference(data));
    safeHandle('ae:save-draft-voucher', (event, payload) => AccountingEngineService_1.AccountingEngineService.saveDraftVoucher(payload));
    safeHandle('ae:post-voucher', (event, payload) => AccountingEngineService_1.AccountingEngineService.postVoucher(payload));
    safeHandle('ae:post-draft-voucher', (event, voucherId) => AccountingEngineService_1.AccountingEngineService.postDraftVoucher(voucherId));
    safeHandle('ae:get-voucher', (event, id) => AccountingEngineService_1.AccountingEngineService.getVoucher(id));
    safeHandle('ae:get-vouchers', (event, filters) => AccountingEngineService_1.AccountingEngineService.getVouchers(filters));
    safeHandle('ae:get-trial-balance', (event, params) => AccountingEngineService_1.AccountingEngineService.getTrialBalance(params));
    ...
    // Cash Boxes
    safeHandle('md-get-cash-boxes', () => MasterDataService_1.MasterDataService.getCashBoxes());
    safeHandle('md-save-cash-box', (event, data) => MasterDataService_1.MasterDataService.saveCashBox(data));
    safeHandle('md-delete-cash-box', (event, id) => MasterDataService_1.MasterDataService.deleteCashBox(id));
    // Cost Centers
  ...
  `).all({ val: `%${search}%` });
    ...
    */
    ...
    safeHandle('treasury-delete-receipt', (event, id) => TreasuryService_1.TreasuryService.deleteReceiptVoucher(id));
    safeHandle('treasury-delete-payment', (event, id) => TreasuryService_1.TreasuryService.deletePaymentVoucher(id));
    safeHandle('treasury-post-receipt', (event, id) => TreasuryService_1.TreasuryService.postReceiptVoucher(id));
    safeHandle('treasury-post-payment', (event, id) => TreasuryService_1.TreasuryService.postPaymentVoucher(id));
    safeHandle('treasury-update-receipt-status', (event, { id, status }) => TreasuryService_1.TreasuryService.updateReceiptVoucherStatus(id, status));
    safeHandle('treasury-update-payment-status', (event, { id, status }) => TreasuryService_1.TreasuryService.updatePaymentVoucherStatus(id, status));
...
const getBundledRendererPath = () => path_1.default.resolve(__dirname, '../../dist/index.html');
const loadRenderer = async (mainWindow) => {
    ...
    if (!fs_1.default.existsSync(bundledRendererPath)) {
        throw new Error(`[Renderer] Bundled UI not found at ${bundledRendererPath}`);
    }
    const bundledRendererUrl = (0, url_1.pathToFileURL)(bundledRendererPath).toString();
...
const reportWindowCreationFailure = (error) => {
    ...
    electron_1.dialog.showErrorBox('WAFI ERP', error?.message || 'Failed to load the application UI.');
};
...
electron_1.app.on('ready', async () => {
    ...
    // --- Printing IPC ---
    (0, printing_ipc_1.registerPrintingIPC)();
