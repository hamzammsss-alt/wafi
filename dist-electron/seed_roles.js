...
const seedRoles = (db) => {
    ...
    const transaction = db.transaction(() => {
        ...
        for (const [engName, arData] of Object.entries(roleMappings)) {
            ...
            if (hasEnglish) {
                console.log(`Migrating Role: ${engName} -> ${arData.name}`);
                updateRoleName.run({
                    newName: arData.name,
                    newDesc: arData.desc,
                    oldName: engName
    ...
    try {
        ...
        if (hasRolePermissions) {
            ...
            if (hasUserAssignments) {
                const adminRole = db.prepare("SELECT id FROM roles WHERE role_key = 'role.admin' OR code = 'role.admin' LIMIT 1").get();
        ...
        console.warn('[seed_roles] Permissions engine defaults skipped:', error?.message || error);
    }
