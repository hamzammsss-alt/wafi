...
class ReportService {
    // 1. Partner Ledger
    static getPartnerLedger(filters) {
        let query = `
            SELECT 
                tl.account_id,
                p.id as partner_id,
    ...
    static getItemMovement(filters) {
        const { itemId, startDate, endDate } = filters;
        if (!startDate)
            return []; // Should require date range for opening balance calculation logic
        // 1. Opening Balance
        const openingQuery = `
        ...
        const transactions = database_1.db.prepare(query).all(...params);
        return [
            {
                date: startDate,
                type: 'رصيد افتتاحي',
    ...
    static getDashboardKPIs() {
        try {
            const sales = database_1.db.prepare("SELECT SUM(grand_total) as total FROM sales_invoices WHERE status = 'POSTED'").get();
            ...
            let cash = { balance: 0 };
            try {
                cash = database_1.db.prepare("SELECT SUM(balance) as balance FROM gl_chart_of_accounts WHERE account_code LIKE '111%'").get() || { balance: 0 };
            }
            catch (e) {
            ...
            let checks = { total: 0 };
            try {
                checks = database_1.db.prepare("SELECT SUM(amount) as total FROM cheques WHERE status = 'Pending'").get() || { total: 0 };
            }
            catch (e) { }
            let lowStockCount = { count: 0 };
            try {
                lowStockCount = database_1.db.prepare("SELECT count(*) as count FROM items WHERE quantity_on_hand <= min_stock").get() || { count: 0 };
            }
            catch (e) { }
            return {
                // @ts-ignore
    ...
    static getBalanceSheet() {
        const assets = database_1.db.prepare("SELECT * FROM accounts WHERE type = 'Asset' OR type = 'ASSET'").all(); // Handle case sensitivity
        const liabilities = database_1.db.prepare("SELECT * FROM accounts WHERE type = 'Liability' OR type = 'LIABILITY'").all();
        const equity = database_1.db.prepare("SELECT * FROM accounts WHERE type = 'Equity' OR type = 'EQUITY'").all();
    ...
    // 6. Sales Analytics
    static getSalesAnalytics(range) {
        let dateFilter = '';
        const params = [];
        if (range.startDate) {
        ...
        const byRegion = database_1.db.prepare(`
        ...
        const topReps = database_1.db.prepare(`
    ...
    static getProfitabilityReport(range) {
        let dateFilter = '';
        const params = [];
        if (range.startDate) {
            dateFilter += ' AND i.date >= ?';
        ...
        `;
        const data = database_1.db.prepare(query).all(...params);
    ...
    static getPurchasingAnalysis(range) {
        let dateFilter = '';
        const params = [];
        if (range.startDate) {
            dateFilter += ' AND date >= ?';
        ...
        `).get(...params);
        // Top Suppliers
        const topSuppliers = database_1.db.prepare(`
        ...
        const typeBreakdown = database_1.db.prepare(`
            SELECT 
    ...
    static getPurchasesByVendor(range) {
        let dateFilter = '';
        const params = [];
        if (range.startDate) {
            dateFilter += ' AND i.date >= ?';
    ...
    static getImportReports() {
        ...
        `;
        const rows = database_1.db.prepare(query).all();
        return rows.map((r) => ({
            ...r,
    ...
    // 10. Cheques Report
    static getChequesReport(filters) {
        let query = `
            SELECT 
    ...
    static getAccountStatement(filters) {
        ...
        `;
        const openingBalance = database_1.db.prepare(openingBalanceQuery).get(filters.accountId, filters.fromDate)?.balance || 0;
        ...
        `;
        const moves = database_1.db.prepare(movesQuery).all(filters.accountId, filters.fromDate, filters.toDate);
    ...
    static getTaxReport(range) {
        ...
        const outputTaxRaw = database_1.db.prepare(`
            SELECT 
        ...
        const inputTaxRaw = database_1.db.prepare(`
             SELECT 
    ...
    static getSlowMovingItems(days = 90) {
    ...
    static getExpiryReport(days = 30) {
        ...
        return database_1.db.prepare(query).all(cutoffStr, '1900-01-01');
    }
}
