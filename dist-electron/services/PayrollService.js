...
class PayrollService {
    ...
    static getActiveAdvances(employeeId) {
    ...
    // 2. PAYROLL ENGINE
    // =================================================================================================
    static generatePayrollPreview(month, year) {
        const monthStr = `${year}-${String(month).padStart(2, '0')}`;
        const startDate = `${year}-${String(month).padStart(2, '0')}-01`;
        // 1. Get Eligible Employees (Active + Contract)
        const employees = database_1.db.prepare(`
        ...
        for (const emp of employees) {
            ...
            const attendance = database_1.db.prepare(`
                SELECT 
            ...
            if (salaryType === 'COMMISSION' || salaryType === 'PRODUCTION')
                basic = 0;
            ...
            // Commission
            const empComm = commissions.find((c) => c.employee_id === emp.id);
            const commissionAmount = empComm ? empComm.commission_amount : 0;
            // Production
            const empProd = production.find((p) => p.employee_id === emp.id);
            ...
            const advances = database_1.db.prepare(`
            ...
            advances.forEach((a) => advanceDeduction += a.installment);
            // Penalties
            const penalties = database_1.db.prepare(`
    ...
    static postPayroll(month, year, slips) {
        const monthStr = `${year}-${String(month).padStart(2, '0')}`;
        const tx = database_1.db.transaction(() => {
            ...
            let totalNet = 0;
            // 2. Save Slips
            const insertSlip = database_1.db.prepare(`
            ...
            JournalService_1.JournalService.createJournalEntry({
                ...
                exchange_rate: 1,
                status: 'POSTED'
            }, lines); // Cast lines if typing is strict or mismatched
        });
        tx();
        return { success: true };
    }
    static getSlips(month, year) {
    ...
    static ensurePayrollAccountsExist() {
        ...
        for (const acc of accounts) {
            let existing = database_1.db.prepare('SELECT id FROM accounts WHERE code = ?').get(acc.code);
            if (!existing) {
                const id = (0, uuid_1.v4)();
    ...
    static generateSalaryEntry(month, year) {
        // 1. Get Totals
        const summary = database_1.db.prepare(`
            ...
            WHERE p.month = ? AND p.year = ?
        `).get(month, year);
        if (!summary || !summary.total_earnings)
            throw new Error('لا يوجد بيانات رواتب لهذه الفترة');
        // 2. Ensure Accounts
    ...
    // 3. END OF SERVICE
    // =================================================================================================
    static calculateEOS(employeeId, endDate) {
        const emp = database_1.db.prepare('SELECT * FROM hr_employees WHERE id = ?').get(employeeId);
        const contract = database_1.db.prepare('SELECT * FROM hr_employee_contracts WHERE employee_id = ? ORDER BY start_date DESC LIMIT 1').get(employeeId);
        if (!emp || !contract)
            throw new Error('Employee or Contract not found');
