...
class SalesInvoiceService {
    ...
    customer_id = ?,
        ...
        updated_at = CURRENT_TIMESTAMP
            WHERE id = ? AND status IN ('DRAFT', 'REJECTED')
              ...
              AND COALESCE(company_id, 'COMP_01') = ?
              AND COALESCE(branch_id, '') = ?
                `).run(header.customer_id ?? null, header.customer_name ?? null, docDate, docDate, header.currency_id ?? null, header.warehouse_id ?? null, header.tax_group_id ?? null, header.exchange_rate ?? 1, subtotal, discountTotal, taxTotal, grandTotal, header.notes ?? null, header.remarks ?? header.notes ?? null, id, Number(existing.version || 0), scope.companyId, scope.branchId);
    ...
    static register() {
        ...
        electron_1.ipcMain.handle('salesInvoices:searchItems', (0, ipcWrap_1.ipcWrap)((0, withGuards_1.withGuards)({
            ...
            const q = `%${String(search || '').trim()}%`;
            return database_1.db.prepare(`
                            SELECT id, code, name_ar AS name, sale_price AS price, COALESCE(tax_included, 0) AS tax_included
                            FROM items
                            WHERE is_active = 1 AND (name_ar LIKE ? OR code LIKE ?)
                            ORDER BY name_ar
                            LIMIT 50
                        `).all(q, q);
        })));
