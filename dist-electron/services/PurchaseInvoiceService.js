...
class PurchaseInvoiceService {
    ...
    static register() {
        ...
        electron_1.ipcMain.handle('purchaseInvoices:searchItems', (0, ipcWrap_1.ipcWrap)((0, withGuards_1.withGuards)({
            ...
            const q = `%${String(search || '').trim()}%`;
            return database_1.db.prepare(`
                            SELECT id, code, COALESCE(name_ar, name_en, name, code) AS name, CAST(COALESCE(cost_price, sale_price, 0) AS REAL) AS price, COALESCE(tax_included, 0) AS tax_included
                            FROM items
                            WHERE is_active = 1 AND (COALESCE(name_ar, '') LIKE ? OR COALESCE(name_en, '') LIKE ? OR COALESCE(name, '') LIKE ? OR code LIKE ?)
                            ORDER BY COALESCE(name_ar, name_en, name, code)
                            LIMIT 50
                        `).all(q, q, q, q);
        })));
