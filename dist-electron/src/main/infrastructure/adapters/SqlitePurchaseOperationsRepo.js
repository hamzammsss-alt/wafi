...
exports.SqlitePurchaseOperationsRepo = void 0;
const uuid_1 = require("uuid");
const errors_1 = require("../../domain/errors");
const TRUE_VALUES = new Set(['1', 'TRUE', 'YES', 'Y', 'ON', 'DELIVERY', 'RECEIPT']);
class SqlitePurchaseOperationsRepo {
    constructor(db) {
        this.db = db;
    }
    ensureSchema() {
    ...
    createDocument(input) {
        ...
        if (!created) {
            throw new errors_1.DomainError('INTERNAL_ERROR', 'Purchase operation document was not found after create', {
    ...
    updateDocument(input) {
        ...
        if (Number(info.changes || 0) === 0) {
            throw new errors_1.DomainError('INVALID_TRANSITION', 'Only draft purchase documents can be updated', {
        ...
        if (!updated) {
            throw new errors_1.DomainError('INTERNAL_ERROR', 'Purchase operation document was not found after update', {
                details: { documentId: input.id },
            });
        }
        return updated;
    ...
    listCommitmentsByOrder(companyId, orderId) {
        const rows = this.db.prepare(`
            SELECT
                id,
                company_id,
    ...
    replaceCommitmentsForOrder(companyId, branchId, orderId, entries) {
        this.db.prepare(`
...
exports.SqlitePurchaseOperationsRepo = SqlitePurchaseOperationsRepo;
