...
exports.VendorPayablesService = void 0;
const errors_1 = require("../../domain/errors");
const EPSILON = 0.000001;
class VendorPayablesService {
    constructor(repository) {
        this.repository = repository;
        this.repository.ensureSchema();
    }
    createVendor(context, input) {
        ...
        if (existingByCode) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Vendor code already exists', {
    ...
    setVendorActive(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const vendorId = this.normalizeRequired(input.id, 'Vendor id is required');
        this.requireVendor(companyId, vendorId);
        const updated = this.repository.setVendorActive(companyId, vendorId, Boolean(input.isActive), new Date().toISOString());
        if (!updated) {
            throw new errors_1.DomainError('INTERNAL_ERROR', 'Vendor active state update failed', {
    ...
    getVendorContacts(context, vendorId) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
    ...
    saveVendorContact(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
    ...
    getVendorAddresses(context, vendorId) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const normalizedVendorId = this.normalizeRequired(vendorId, 'Vendor id is required');
        this.requireVendor(companyId, normalizedVendorId);
        return this.repository.listVendorAddresses(normalizedVendorId);
    }
    saveVendorAddress(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const vendorId = this.normalizeRequired(input.vendorId, 'Vendor id is required');
        this.requireVendor(companyId, vendorId);
        const now = new Date().toISOString();
    ...
    getVendorPaymentProfile(context, vendorId) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const normalizedVendorId = this.normalizeRequired(vendorId, 'Vendor id is required');
        this.requireVendor(companyId, normalizedVendorId);
    ...
    saveVendorPaymentProfile(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const vendorId = this.normalizeRequired(input.vendorId, 'Vendor id is required');
        this.requireVendor(companyId, vendorId);
        ...
        if (input.maxBillAgeDays != null && Number(input.maxBillAgeDays) < 0) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Max bill age days cannot be negative', {
    ...
    evaluateVendorPaymentControl(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const branchId = this.normalizeNullable(context.branchId);
        const vendorId = this.normalizeRequired(input.vendorId, 'Vendor id is required');
        const vendor = this.requireVendor(companyId, vendorId);
        if (!vendor.isActive) {
            throw new errors_1.DomainError('POLICY_VIOLATION', 'Inactive vendor cannot pass payment policy evaluation', {
    ...
    placeVendorOnHold(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const vendorId = this.normalizeRequired(input.vendorId, 'Vendor id is required');
        ...
        this.repository.runInTransaction(() => {
            const vendor = this.requireVendor(companyId, vendorId);
            const nextStatus = vendor.isActive ? 'ON_HOLD' : 'INACTIVE';
        ...
        return this.evaluateVendorPaymentControl({ companyId, branchId: context.branchId }, { vendorId, asOfDate: new Date().toISOString().slice(0, 10) });
    }
    releaseVendorHold(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const vendorId = this.normalizeRequired(input.vendorId, 'Vendor id is required');
        ...
        this.repository.runInTransaction(() => {
            const vendor = this.requireVendor(companyId, vendorId);
            const nextStatus = vendor.isActive ? 'ACTIVE' : 'INACTIVE';
            this.repository.updateVendor({
                id: vendor.id,
                companyId,
                code: vendor.code,
    ...
    getVendorExposure(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const vendorId = this.normalizeRequired(input.vendorId, 'Vendor id is required');
        this.requireVendor(companyId, vendorId);
        const asOfDate = this.normalizeDate(input.asOfDate || new Date().toISOString().slice(0, 10), 'As of date is invalid');
        const profile = this.getVendorPaymentProfile({ companyId }, vendorId);
        return this.buildExposureSummary(companyId, vendorId, asOfDate, this.normalizeNullable(context.branchId), profile.paymentLimit);
    }
    getVendorStatement(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const vendorId = this.normalizeRequired(input.vendorId, 'Vendor id is required');
        this.requireVendor(companyId, vendorId);
    ...
    getVendorAging(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const vendorId = this.normalizeRequired(input.vendorId, 'Vendor id is required');
        this.requireVendor(companyId, vendorId);
        const asOfDate = this.normalizeDate(input.asOfDate || new Date().toISOString().slice(0, 10), 'As of date is invalid');
        const openInvoices = this.repository.listOpenPurchaseInvoices(companyId, vendorId, asOfDate, this.normalizeNullable(input.branchId) || this.normalizeNullable(context.branchId));
        const buckets = {
            CURRENT: 0,
            '1_30': 0,
            '31_60': 0,
    ...
    getVendorTimeline(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const vendorId = this.normalizeRequired(input.vendorId, 'Vendor id is required');
        this.requireVendor(companyId, vendorId);
        const fromDate = this.normalizeDateOrNull(input.fromDate);
    ...
    createVendorFollowUp(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const vendorId = this.normalizeRequired(input.vendorId, 'Vendor id is required');
        this.requireVendor(companyId, vendorId);
    ...
    updateVendorFollowUp(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
    ...
    getVendorFollowUps(context, vendorId, includeClosed = true) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const normalizedVendorId = this.normalizeRequired(vendorId, 'Vendor id is required');
        this.requireVendor(companyId, normalizedVendorId);
        return this.repository.listVendorFollowUps(companyId, normalizedVendorId, includeClosed);
    }
    markVendorFollowUpDone(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
    ...
    cancelVendorFollowUp(context, input) {
        const companyId = this.normalizeRequired(context.companyId, 'Company id is required');
        const followUp = this.requireFollowUp(companyId, this.normalizeRequired(input.id, 'Follow-up id is required'));
    ...
    buildExposureSummary(companyId, vendorId, asOfDate, branchId, paymentLimit) {
        const policy = this.repository.getPolicy(companyId);
        const aging = this.getVendorAging({ companyId, branchId: branchId || '' }, {
            vendorId,
            asOfDate,
            includeDetails: true,
            branchId,
    ...
    requireVendor(companyId, vendorId) {
    ...
    normalizeCode(value) {
        ...
        if (!/^[A-Z0-9._-]{2,40}$/.test(normalized)) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Vendor code format is invalid', {
    ...
    normalizeVendorStatus(value, fallback) {
...
exports.VendorPayablesService = VendorPayablesService;
