...
exports.InventoryDocumentService = void 0;
const errors_1 = require("../../domain/errors");
const InventoryPostingBuilder_1 = require("./InventoryPostingBuilder");
...
class InventoryDocumentService {
    ...
    async post(context, documentId) {
        ...
        if (header.status === 'POSTED' && stockAlreadyPosted) {
            return {
                ...
                sourceModule: InventoryPostingBuilder_1.INVENTORY_SOURCE_MODULE,
        ...
        try {
            this.repository.runInTransaction(() => {
                ...
                if (buildResult.requiresFinancialPosting) {
                    if (!existingJournal) {
                        const postResult = this.journalEngineUseCases.postJournal(context.companyId, context.branchId, context.userId, this.requirePostingCommand(buildResult));
                        existingJournal = this.journalEngineUseCases.getById(context.companyId, postResult.journalId);
                        didInsertJournal = true;
                    }
                    if (!existingJournal) {
                        throw new errors_1.DomainError('INTERNAL_ERROR', 'Inventory posting journal was not resolved', {
    ...
    reverse(context, input) {
        ...
        if (!originalJournal && !stockAlreadyPosted) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Inventory document has not been posted yet', {
        ...
        try {
            ...
            if (String(error?.message || '').includes('UNIQUE constraint failed: stock_ledger_entries')) {
                const resolvedReversal = this.resolveExistingReversal(context.companyId, originalJournal, header.reversalJournalId);
                return {
                    documentId: header.id,
                    sourceModule: InventoryPostingBuilder_1.INVENTORY_SOURCE_MODULE,
                    sourceType: header.docType,
    ...
    requirePostingCommand(buildResult) {
        if (!buildResult.command) {
            throw new errors_1.DomainError('INTERNAL_ERROR', 'Posting command is required');
        }
    ...
    requireDocument(companyId, branchId, documentId) {
        ...
        if (!document) {
            throw new errors_1.DomainError('DOCUMENT_NOT_FOUND', `Inventory document ${normalizedDocumentId} was not found`, {
    ...
    assertCanPost(header) {
        if (header.status === 'CANCELLED') {
            throw new errors_1.DomainError('INVALID_TRANSITION', 'Cancelled inventory document cannot be posted', {
                messageKey: 'error.inventory_document.posting.cancelled_not_allowed',
                details: { documentId: header.id },
            });
        }
    }
    validateDraftLines(docType, lines) {
        for (let index = 0; index < lines.length; index += 1) {
            const line = lines[index];
            if (!line.itemId) {
                throw new errors_1.DomainError('VALIDATION_ERROR', `Line ${index + 1}: item is required`, {
                    messageKey: 'validation.inventory_document.line_item_required',
                    details: { lineNo: index + 1 },
                });
            }
            if (line.qty <= EPSILON) {
                throw new errors_1.DomainError('VALIDATION_ERROR', `Line ${index + 1}: quantity must be greater than zero`, {
                    messageKey: 'validation.inventory_document.qty_positive',
                    details: { lineNo: index + 1 },
                });
            }
            if (line.totalCost < -EPSILON) {
                throw new errors_1.DomainError('VALIDATION_ERROR', `Line ${index + 1}: total cost cannot be negative`, {
            ...
            if (docType === 'STOCK_ADJUSTMENT' && normalizedDirection && normalizedDirection !== 'IN' && normalizedDirection !== 'OUT') {
                throw new errors_1.DomainError('VALIDATION_ERROR', `Line ${index + 1}: adjustment direction must be IN or OUT`, {
    ...
    validatePostingLines(header, lines) {
        if (!lines.length) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Inventory document lines are required for posting', {
        ...
        for (let index = 0; index < lines.length; index += 1) {
            ...
            if (Number(line.totalCost || 0) < -EPSILON) {
                throw new errors_1.DomainError('VALIDATION_ERROR', `Line ${lineNo}: total cost cannot be negative`, {
                    messageKey: 'validation.inventory_document.total_cost_non_negative',
                    details: { lineNo },
                });
            }
            ...
            if (!item.isStockItem) {
                throw new errors_1.DomainError('VALIDATION_ERROR', `Line ${lineNo}: item must be stock item`, {
            ...
            if (header.docType === 'STOCK_TRANSFER') {
                ...
                if (sourceWarehouse === destinationWarehouse) {
                    throw new errors_1.DomainError('VALIDATION_ERROR', `Line ${lineNo}: source and destination warehouses must differ`, {
    ...
    requireWarehouse(warehouseId, lineNo, field) {
        if (!warehouseId) {
    ...
    buildStockLedgerEntries(header, lines, movementDateTime, isReversal, originalEntries) {
    ...
    normalizeDocType(docType) {
        ...
        throw new errors_1.DomainError('VALIDATION_ERROR', 'Unsupported inventory document type', {
...
exports.InventoryDocumentService = InventoryDocumentService;
