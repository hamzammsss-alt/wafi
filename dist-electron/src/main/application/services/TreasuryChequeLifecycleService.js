...
exports.TreasuryChequeLifecycleService = void 0;
...
class TreasuryChequeLifecycleService {
    constructor(repository, accountResolutionUseCases, journalEngineUseCases) {
        this.repository = repository;
        this.accountResolutionUseCases = accountResolutionUseCases;
        this.journalEngineUseCases = journalEngineUseCases;
    }
    receiveChequeFromDocument(context, chequeId, eventDate, journalId) {
        const cheque = this.requireCheque(context.companyId, chequeId);
        if (cheque.direction !== 'RECEIVED') {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Cheque direction must be RECEIVED', {
    ...
    issueChequeFromDocument(context, chequeId, eventDate, journalId) {
        const cheque = this.requireCheque(context.companyId, chequeId);
        if (cheque.direction !== 'ISSUED') {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Cheque direction must be ISSUED', {
    ...
    async depositCheque(context, command) {
        const cheque = this.requireCheque(context.companyId, command.chequeId);
        if (cheque.direction !== 'RECEIVED') {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Only received cheques can be deposited', {
        ...
        if (cheque.status !== 'IN_SAFE') {
            throw new errors_1.DomainError('INVALID_TRANSITION', 'Cheque must be in safe before deposit', {
        ...
        const accounts = await this.resolveAccounts(context, {
            ...
            requiredRoles: [FinancialAccountRole_1.FinancialAccountRole.CHEQUES_DEPOSITED_ACCOUNT, FinancialAccountRole_1.FinancialAccountRole.CHEQUE_IN_SAFE_ACCOUNT],
        ...
        const journal = this.postLifecycleJournal(context, {
            ...
            debitAccountId: accounts[FinancialAccountRole_1.FinancialAccountRole.CHEQUES_DEPOSITED_ACCOUNT],
            creditAccountId: accounts[FinancialAccountRole_1.FinancialAccountRole.CHEQUE_IN_SAFE_ACCOUNT],
        });
    ...
    async clearReceivedCheque(context, command) {
        const cheque = this.requireCheque(context.companyId, command.chequeId);
        if (cheque.direction !== 'RECEIVED') {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Only received cheques can be cleared', {
        ...
        if (cheque.status !== 'DEPOSITED' && cheque.status !== 'IN_SAFE') {
            throw new errors_1.DomainError('INVALID_TRANSITION', 'Cheque cannot be cleared from current state', {
                messageKey: 'validation.treasury.cheque.clear_received.invalid_state',
                details: { chequeId: cheque.id, status: cheque.status },
            });
        }
        const sourceRole = cheque.status === 'DEPOSITED'
            ? FinancialAccountRole_1.FinancialAccountRole.CHEQUES_DEPOSITED_ACCOUNT
            : FinancialAccountRole_1.FinancialAccountRole.CHEQUE_IN_SAFE_ACCOUNT;
        const accounts = await this.resolveAccounts(context, {
            documentType: 'CHEQUE_RECEIPT',
            lineType: 'CLEAR_RECEIVED',
            partnerId: cheque.partnerId,
            warehouseId: cheque.depositedBankAccountId,
            requiredRoles: [FinancialAccountRole_1.FinancialAccountRole.BANK_ACCOUNT, sourceRole],
            direction: ResolutionDirection_1.ResolutionDirection.TRANSFER,
            currencyCode: cheque.currencyCode,
        });
        const journal = this.postLifecycleJournal(context, {
            ...
            debitAccountId: accounts[FinancialAccountRole_1.FinancialAccountRole.BANK_ACCOUNT],
        ...
        this.repository.runInTransaction(() => {
            ...
            this.repository.saveChequeEvent(this.buildEventRecord(context, cheque.id, 'CLEAR_RECEIVED', eventDate, journal?.id || null, 'TREASURY_CHEQUE_CLEAR_RECEIVED', now));
        });
        return {
            chequeId: cheque.id,
            eventType: 'CLEAR_RECEIVED',
    ...
    async returnReceivedCheque(context, command) {
        const cheque = this.requireCheque(context.companyId, command.chequeId);
        if (cheque.direction !== 'RECEIVED') {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Only received cheques can be returned', {
        ...
        if (cheque.status === 'CANCELLED') {
            throw new errors_1.DomainError('INVALID_TRANSITION', 'Cancelled cheque cannot be returned', {
        ...
        const sourceRole = cheque.status === 'DEPOSITED'
            ...
            : cheque.status === 'CLEARED'
                ? FinancialAccountRole_1.FinancialAccountRole.BANK_ACCOUNT
                : FinancialAccountRole_1.FinancialAccountRole.CHEQUE_IN_SAFE_ACCOUNT;
        const accounts = await this.resolveAccounts(context, {
            documentType: 'CHEQUE_RECEIPT',
            ...
            partnerId: cheque.partnerId,
            warehouseId: cheque.depositedBankAccountId,
            requiredRoles: [FinancialAccountRole_1.FinancialAccountRole.RETURNED_CHEQUE_ACCOUNT, sourceRole],
        ...
        const journal = this.postLifecycleJournal(context, {
            ...
            debitAccountId: accounts[FinancialAccountRole_1.FinancialAccountRole.RETURNED_CHEQUE_ACCOUNT],
            creditAccountId: accounts[sourceRole],
        });
        const eventDate = String(command.date || '').slice(0, 10);
        const now = new Date().toISOString();
        this.repository.runInTransaction(() => {
            ...
            this.repository.saveChequeEvent(this.buildEventRecord(context, cheque.id, 'RETURN_RECEIVED', eventDate, journal?.id || null, 'TREASURY_CHEQUE_RETURN_RECEIVED', now));
        });
        return {
            chequeId: cheque.id,
            eventType: 'RETURN_RECEIVED',
    ...
    async clearIssuedCheque(context, command) {
        const cheque = this.requireCheque(context.companyId, command.chequeId);
        if (cheque.direction !== 'ISSUED') {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Only issued cheques can be cleared', {
        ...
        if (cheque.status !== 'ISSUED_PENDING') {
            throw new errors_1.DomainError('INVALID_TRANSITION', 'Issued cheque must be pending before clear', {
        ...
        const accounts = await this.resolveAccounts(context, {
            ...
            partnerId: cheque.partnerId,
            warehouseId: cheque.depositedBankAccountId,
            requiredRoles: [FinancialAccountRole_1.FinancialAccountRole.ISSUED_CHEQUE_ACCOUNT, FinancialAccountRole_1.FinancialAccountRole.BANK_ACCOUNT],
            direction: ResolutionDirection_1.ResolutionDirection.TRANSFER,
            currencyCode: cheque.currencyCode,
        });
        const journal = this.postLifecycleJournal(context, {
            ...
            debitAccountId: accounts[FinancialAccountRole_1.FinancialAccountRole.ISSUED_CHEQUE_ACCOUNT],
            creditAccountId: accounts[FinancialAccountRole_1.FinancialAccountRole.BANK_ACCOUNT],
        });
        const eventDate = String(command.date || '').slice(0, 10);
        const now = new Date().toISOString();
        this.repository.runInTransaction(() => {
            ...
            this.repository.saveChequeEvent(this.buildEventRecord(context, cheque.id, 'CLEAR_ISSUED', eventDate, journal?.id || null, 'TREASURY_CHEQUE_CLEAR_ISSUED', now));
        });
        return {
            chequeId: cheque.id,
            eventType: 'CLEAR_ISSUED',
    ...
    async cancelCheque(context, command) {
        const cheque = this.requireCheque(context.companyId, command.chequeId);
        if (cheque.status === 'CANCELLED') {
        ...
        if (cheque.status === 'CLEARED' || cheque.status === 'ISSUED_CLEARED') {
            throw new errors_1.DomainError('INVALID_TRANSITION', 'Cleared cheque cannot be cancelled', {
        ...
        if (cheque.direction === 'RECEIVED' && cheque.status === 'DEPOSITED') {
            const accounts = await this.resolveAccounts(context, {
                ...
                partnerId: cheque.partnerId,
                warehouseId: cheque.depositedBankAccountId,
                requiredRoles: [FinancialAccountRole_1.FinancialAccountRole.CHEQUE_IN_SAFE_ACCOUNT, FinancialAccountRole_1.FinancialAccountRole.CHEQUES_DEPOSITED_ACCOUNT],
                direction: ResolutionDirection_1.ResolutionDirection.RETURN,
                currencyCode: cheque.currencyCode,
            });
            journal = this.postLifecycleJournal(context, {
                ...
                debitAccountId: accounts[FinancialAccountRole_1.FinancialAccountRole.CHEQUE_IN_SAFE_ACCOUNT],
                creditAccountId: accounts[FinancialAccountRole_1.FinancialAccountRole.CHEQUES_DEPOSITED_ACCOUNT],
            });
        }
        const eventDate = String(command.date || '').slice(0, 10);
        const now = new Date().toISOString();
        this.repository.runInTransaction(() => {
            ...
            this.repository.saveChequeEvent(this.buildEventRecord(context, cheque.id, 'CANCEL', eventDate, journal?.id || null, 'TREASURY_CHEQUE_CANCEL', now));
        });
        return {
            chequeId: cheque.id,
            eventType: 'CANCEL',
    ...
    async resolveAccounts(context, input) {
        ...
        if (!result.success) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Cheque account resolution failed', {
        ...
        for (const role of input.requiredRoles) {
            ...
            if (!resolved) {
                throw new errors_1.DomainError('VALIDATION_ERROR', 'Required account role was not resolved', {
                    messageKey: 'error.account_resolution.mapping_not_found',
                    details: { role },
                });
            }
    ...
    postLifecycleJournal(context, input) {
    ...
    buildEventRecord(context, chequeId, eventType, eventDate, journalId, sourceType, nowIso) {
        return {
            id: this.repository.nextIdentity(),
            companyId: context.companyId,
            chequeId,
    ...
    requireCheque(companyId, chequeId) {
        const normalizedId = String(chequeId || '').trim();
        if (!normalizedId) {
            throw new errors_1.DomainError('VALIDATION_ERROR', 'Cheque id is required', {
                messageKey: 'validation.treasury.cheque.id_required',
            });
        }
        const cheque = this.repository.getChequeById(companyId, normalizedId);
        if (!cheque) {
            throw new errors_1.DomainError('DOCUMENT_NOT_FOUND', `Cheque ${normalizedId} was not found`, {
    ...
    resolveJournalNo(companyId, journalId) {
        if (!journalId)
            return null;
...
exports.TreasuryChequeLifecycleService = TreasuryChequeLifecycleService;
