...
exports.ChartOfAccountsUseCases = void 0;
...
class ChartOfAccountsUseCases {
    constructor(repository, seedService) {
        this.repository = repository;
        this.seedService = seedService;
        this.validation = new ChartOfAccountsValidationService_1.ChartOfAccountsValidationService();
    }
    async seedDefaultChart(authenticatedCompanyId, input) {
    ...
    async listFlat(authenticatedCompanyId, query) {
    ...
    async listTree(authenticatedCompanyId, query) {
        const normalizedQuery = this.normalizeListQuery(query);
    ...
    async findByCode(authenticatedCompanyId, code) {
        const normalizedCode = String(code || '').trim();
        if (!normalizedCode) {
            throw new errors_1.DomainError('ERR_ACCOUNT_CODE_REQUIRED', 'Account code is required', {
    ...
    async createAccount(authenticatedCompanyId, input) {
        const companyId = this.assertCompanyScope(authenticatedCompanyId, input.companyId);
        ...
        const account = AccountEntity_1.AccountEntity.create({
    ...
    async updateAccount(authenticatedCompanyId, input) {
        const companyId = this.assertCompanyScope(authenticatedCompanyId, input.companyId);
        ...
        if (!existing) {
            throw new errors_1.DomainError('ERR_ACCOUNT_NOT_FOUND', `Account ${id} was not found`, {
        ...
        const path = parent ? `${parent.path}/${normalizedCode}` : normalizedCode;
        const systemTag = this.normalizeSystemTag(input.systemTag);
        const next = AccountEntity_1.AccountEntity.create({
            id: existing.id,
            companyId,
            code: normalizedCode,
            name: this.normalizeName(input.name),
    ...
    normalizeSeedStrategy(strategy) {
    ...
    normalizeSystemTag(systemTag) {
        const normalized = String(systemTag || '').trim().toUpperCase();
        return normalized || null;
    }
    async resolveParent(companyId, parentCode) {
        if (!parentCode)
            return null;
        const parent = await this.repository.findByCode(companyId, parentCode);
        if (!parent) {
            throw new errors_1.DomainError('ERR_ACCOUNT_PARENT_NOT_FOUND', `Parent code ${parentCode} was not found`, {
    ...
    async resolveParentCode(companyId, parentId) {
        if (!parentId)
            return null;
    ...
    assertCompanyScope(authenticatedCompanyId, payloadCompanyId) {
        ...
        if (!normalizedAuthCompanyId) {
            throw new errors_1.DomainError('ERR_SCOPE_INVALID', 'Authenticated company scope is missing', {
                messageKey: 'error.scope.invalid',
            });
        }
        if (normalizedAuthCompanyId !== normalizedPayloadCompanyId) {
            throw new errors_1.DomainError('ERR_ACCOUNT_COMPANY_SCOPE_MISMATCH', 'Company scope mismatch', {
...
exports.ChartOfAccountsUseCases = ChartOfAccountsUseCases;
