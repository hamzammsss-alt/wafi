...
export const initDB = (dbPath: string) => {
  ...
  try {
    ...
    if (!has('calculation_mode')) db.exec("ALTER TABLE units ADD COLUMN calculation_mode TEXT DEFAULT 'MANUAL'");
    if (!has('requires_length')) db.exec("ALTER TABLE units ADD COLUMN requires_length INTEGER DEFAULT 0");
    if (!has('requires_width')) db.exec("ALTER TABLE units ADD COLUMN requires_width INTEGER DEFAULT 0");
    if (!has('requires_height')) db.exec("ALTER TABLE units ADD COLUMN requires_height INTEGER DEFAULT 0");
    if (!has('requires_count')) db.exec("ALTER TABLE units ADD COLUMN requires_count INTEGER DEFAULT 0");
    if (!has('formula_hint')) db.exec("ALTER TABLE units ADD COLUMN formula_hint TEXT");
...
`);
  ...
  try {
    ...
    if (cbCols.length > 0) {
      ...
      if (!cbCols.some((c: any) => c.name === 'currency_code')) db.prepare("ALTER TABLE cash_boxes ADD COLUMN currency_code TEXT DEFAULT 'ILS'").run();
