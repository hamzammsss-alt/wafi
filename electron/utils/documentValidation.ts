export function validateSalesInvoice(header: any, lines: any[]) {
    ...
    if (!lines || lines.length === 0) {
        ...
        lines.forEach((l, i) => {
            ...
            if (qty < 0) errors.push({ field: `lines[${i}].qty`, message: 'Quantity cannot be negative' });
            if (price < 0) errors.push({ field: `lines[${i}].price`, message: 'Price cannot be negative' });
            if (tax < 0 || tax > 100) errors.push({ field: `lines[${i}].tax_rate`, message: 'Tax rate must be between 0 and 100' });
        });
    }
