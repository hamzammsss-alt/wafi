...
export class PurchaseService {
    ...
    static createInvoice(data: any) {
        ...
        try {
            const cols = db.prepare("PRAGMA table_info(purchase_invoices)").all();
            if (!cols.some((c: any) => c.name === 'manual_ref')) {
                db.prepare("ALTER TABLE purchase_invoices ADD COLUMN manual_ref TEXT").run();
            }
            if (!cols.some((c: any) => c.name === 'cost_center_id')) {
                db.prepare("ALTER TABLE purchase_invoices ADD COLUMN cost_center_id TEXT").run();
            }
        } catch (e) {
        ...
        if (invoiceNo === 'NEW' || !invoiceNo) {
            invoiceNo = JournalService.getNextVoucherNo('PINV');
        }
        ...
        const transaction = db.transaction(() => {
            ...
            const supplier = db.prepare('SELECT name_ar, linked_account_id FROM business_partners WHERE id = ?').get(header.supplier_id);
    ...
    static getPurchaseAccount() {
        ...
        const acc = db.prepare("SELECT id FROM accounts WHERE name LIKE '%مشتريات%' OR name LIKE '%Purchases%'").get();
        return acc ? acc.id : '';
    }

    static getGoodsInTransitAccount() {
        // Goods In Transit ( Asset )
        const acc = db.prepare("SELECT id FROM accounts WHERE name LIKE '%بضاعة بالطريق%' OR name LIKE '%Transit%'").get();
    ...
    static getInputVATAccount() {
        // Input VAT
        const acc = db.prepare("SELECT id FROM accounts WHERE name LIKE '%مدخلات%' OR name LIKE '%Input%'").get();
        return acc ? acc.id : '';
    }

    static getSupplierAccount(supplierId: string) {
        // Get linked account from partner
        const partner = db.prepare("SELECT linked_account_id FROM business_partners WHERE id = ?").get(supplierId);
        if (partner && partner.linked_account_id) return partner.linked_account_id;

        // Fallback: Suppliers Control Account
        const acc = db.prepare("SELECT id FROM accounts WHERE name LIKE '%موردين%' OR name LIKE '%Suppliers%'").get();
        return acc ? acc.id : '';
    }

    static getNextInvoiceNo() {
        return JournalService.getNextVoucherNo('PINV');
    }
    ...
    static deleteOrder(id: string) {
        const order = db.prepare('SELECT status, request_id FROM purchase_orders WHERE id = ?').get(id);
        if (!order) throw new Error("Order not found");
        if (order.status !== 'DRAFT') throw new Error("Cannot delete processed order");

        const runTx = db.transaction(() => {
            // 1. Revert Linked PR Status
            if (order.request_id) {
                db.prepare("UPDATE purchase_requests SET status = 'DRAFT' WHERE id = ?").run(order.request_id);
            }
    ...
    static updateOrder(data: { header: any, lines: any[] }) { // Header might be typed properly later
        const { header, lines } = data;
        const id = header.id;
        ...
        // Validate Status
        const current = db.prepare('SELECT status FROM purchase_orders WHERE id = ?').get(id) as any;
        if (!current) throw new Error("Order not found");
        if (current.status !== 'DRAFT') throw new Error("Cannot update processed order");
    ...
    static createRequest(data: { header: PurchaseRequestHeader, lines: PurchaseRequestLine[] }) {
        const { header, lines } = data;
        const id = uuidv4();
        let no = header.request_no;

        if (!no || no === 'NEW') {
            no = JournalService.getNextVoucherNo('PRQ');
        }
    ...
    static getRequests(): PurchaseRequestHeader[] {
        return db.prepare(`
    ...
    static getRequest(id: string): { header: PurchaseRequestHeader, lines: PurchaseRequestLine[] } | null {
        const header = db.prepare('SELECT * FROM purchase_requests WHERE id = ?').get(id) as PurchaseRequestHeader;
        if (!header) return null;
        ...
        const lines = db.prepare(`
            ...
            LEFT JOIN units u ON l.unit_id = u.id
            WHERE l.request_id = ?
        `).all(id) as PurchaseRequestLine[];

        return { header, lines };
    }

    static updateRequest(data: { header: PurchaseRequestHeader, lines: PurchaseRequestLine[] }) {
        const { header, lines } = data;
        const id = header.id;

        if (!id) throw new Error("Request ID missing for update");
        ...
        const runTx = db.transaction(() => {
            ...
            // Delete old lines
            db.prepare('DELETE FROM purchase_request_lines WHERE request_id = ?').run(id);
    ...
    static deleteRequest(id: string) {
        const r = db.prepare('SELECT status FROM purchase_requests WHERE id = ?').get(id);
        if (r && r.status !== 'DRAFT') throw new Error("Cannot delete processed request");
    ...
    static createReturn(data: any) {
        ...
        if (!returnNo || returnNo === 'NEW') {
            returnNo = JournalService.getNextVoucherNo('PR');
        }
    ...
    static createRFQ(data: { header: RFQHeader, lines: RFQLine[] }) {
        const { header, lines } = data;
        const id = uuidv4();
        let no = header.request_no;

        if (!no || no === 'NEW') {
            no = JournalService.getNextVoucherNo('RFQ');
        }
        ...
        const runTx = db.transaction(() => {
            ...
            for (const line of lines) {
                insertLine.run({
                    id: uuidv4(), rfqId: id, itemId: line.item_id, desc: line.description,
                    qty: line.quantity, unitId: line.unit_id, notes: line.notes || ''
                });
            }
        });
    ...
    static getRFQ(id: string): { header: RFQHeader, lines: RFQLine[] } | null {
        const header = db.prepare('SELECT * FROM purchase_rfqs WHERE id = ?').get(id) as RFQHeader;
        if (!header) return null;
    ...
    static updateRFQ(data: { header: RFQHeader, lines: RFQLine[] }) {
        const { header, lines } = data;
        const id = header.id;

        if (!id) throw new Error("RFQ ID missing for update");
        ...
        const runTx = db.transaction(() => {
            ...
            // Delete old lines
            db.prepare('DELETE FROM purchase_rfq_lines WHERE rfq_id = ?').run(id);
