...
export class SalesInvoiceService {
    ...
    private static ensurePartnerPricingColumns() {
        const cols = db.prepare("PRAGMA table_info('business_partners')").all();
        const names = new Set((cols || []).map((col: any) => String(col.name)));
        const add = (column: string, definition: string) => {
            if (names.has(column)) return;
            try {
                db.prepare(`ALTER TABLE business_partners ADD COLUMN ${column} ${definition}`).run();
                names.add(column);
            } catch (error) {
    ...
    private static ensurePriceListSchema() {
        db.exec(`
        ...
        const itemColNames = new Set((itemCols || []).map((col: any) => String(col.name)));
        if (!itemColNames.has('wholesale_price')) {
            try {
                db.prepare(`ALTER TABLE items ADD COLUMN wholesale_price REAL DEFAULT 0`).run();
            } catch (error) {
    ...
    private static getCustomerPricing(customerId: string | null) {
        if (!customerId) return { priceListId: null as string | null, discountPercent: 0 };
        ...
        const row = db.prepare(`
            ...
            WHERE id = ?
            LIMIT 1
        `).get(customerId) as { price_list_id?: string | null; customer_discount_percent?: number | null } | undefined;
    ...
    private static inferFallbackItemPrice(item: any, priceList: any | null): { price: number; source: string } {
        const listBucket = [
            priceList?.id,
            priceList?.name_ar,
            priceList?.name_en,
    ...
    static resolveItemPrice(input: {
        ...
        const priceListId =
            SalesInvoiceService.normalizeNullableId(input?.priceListId || input?.price_list_id) ||
        ...
        if (priceListId) {
            priceList = db.prepare('SELECT * FROM price_lists WHERE id = ? LIMIT 1').get(priceListId) || null;

            const itemPrice = db.prepare(`
                SELECT price
                ...
                LIMIT 1
            `).get(priceListId, itemId, unitId, unitId, qty, unitId) as { price?: number | null } | undefined;
            ...
            const legacyPrice = db.prepare(`
                ...
                LIMIT 1
            `).get(priceListId, itemId, unitId, unitId, unitId) as { price?: number | null } | undefined;
    ...
    static register() {
        ...
        ipcMain.handle(
            ...
            ipcWrap(
                withGuards(
                    ...
                    async (_ctx, _event, search: string, pricingContext?: any) => {
                        ...
                        const rows = ItemService.searchItemProfiles(search, 50);
        ...
        ipcMain.handle(
            'salesInvoices:resolveItemPrice',
            ipcWrap(
                withGuards(
                    {
