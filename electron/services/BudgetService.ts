...
export class BudgetService {
    ...
    static getBudgetById(id: string) {
        const header = db.prepare('SELECT * FROM gl_budget_headers WHERE id = ?').get(id);
    ...
    static createBudget(data: { name: string, fiscal_year: number, description?: string, lines: any[] }) {
        const id = uuidv4();
        ...
        const transaction = db.transaction(() => {
            ...
            for (const line of data.lines) {
                insertLine.run(uuidv4(), id, line.account_id, line.period || 0, line.amount || 0, line.notes || '');
            }
        });
    ...
    static updateBudgetStatus(id: string, status: string, userId: string) {
        if (status === 'APPROVED') {
            db.prepare(`
            ...
            db.prepare('UPDATE gl_budget_headers SET status = ? WHERE id = ?').run(status, id);
        }
        return { success: true };
    }
    ...
    static getBudgetVsActual(budgetId: string, period?: number) {
        // 1. Get Budget Header info
        const budget = db.prepare('SELECT * FROM gl_budget_headers WHERE id = ?').get(budgetId);
        ...
        */
        ...
        const validParams: any[] = [budgetId];
        if (period) validParams.push(period);

        validParams.push(budget.fiscal_year.toString());
        if (period) validParams.push(period.toString().padStart(2, '0'));

        const results = db.prepare(safeSql).all(...validParams);
