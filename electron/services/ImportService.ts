...
export class ImportService {
    ...
    private registerHandlers() {
        // Shipments
        ipcMain.handle('import-get-shipments', async (event, filters) => {
            return this.getShipments(filters);
        });

        ipcMain.handle('import-get-shipment-by-id', async (event, id) => {
            return this.getShipmentById(id);
        });

        ipcMain.handle('import-save-shipment', async (event, shipment) => {
            return this.saveShipment(shipment);
        });

        ipcMain.handle('import-delete-shipment', async (event, id) => {
            return this.deleteShipment(id);
        });

        // Containers
        ipcMain.handle('import-get-containers', async (event, shipmentId) => {
            return this.getContainers(shipmentId);
        });

        ipcMain.handle('import-save-container', async (event, container) => {
            return this.saveContainer(container);
        });

        ipcMain.handle('import-delete-container', async (event, id) => {
            return this.deleteContainer(id);
        });

        ipcMain.handle('import-get-shipment-items', async (event, shipmentId) => {
            return this.getShipmentItems(shipmentId);
        });

        // Expenses (Legacy)
        ipcMain.handle('import-get-expenses', async (event, shipmentId) => {
            return this.getExpenses(shipmentId);
        });

        ipcMain.handle('import-save-expense', async (event, expense) => {
            return this.saveExpense(expense);
        });

        ipcMain.handle('import-allocate-costs', async (event, { lcId, expenses, method }) => {
            return this.allocateCosts(lcId, expenses, method);
        });

        ipcMain.handle('import-get-dashboard-stats', async () => {
            return this.getDashboardStats();
        });

        // Proforma Handlers
        ipcMain.handle('import-get-proformas', async (event, filters) => {
            return this.getProformas(filters);
        });
        ipcMain.handle('import-get-proforma', async (event, id) => {
            return this.getProforma(id);
        });
        ipcMain.handle('import-save-proforma', async (event, data) => {
        ...
        // ============================================================================
        ipcMain.handle('import-get-commercial-invoices', async (event, shipmentId) => {
            return this.getCommercialInvoices(shipmentId);
        });

        ipcMain.handle('import-get-commercial-invoice', async (event, id) => {
            return this.getCommercialInvoice(id);
        });

        ipcMain.handle('import-save-commercial-invoice', async (event, data) => {
            return this.saveCommercialInvoice(data);
        });

        ipcMain.handle('import-delete-commercial-invoice', async (event, id) => {
            return this.deleteCommercialInvoice(id);
        });

        ipcMain.handle('import-get-all-commercial-invoices', async () => {
        ...
        // ============================================================================
        ipcMain.handle('import-get-clearance-expenses', async (event, shipmentId) => {
            return this.getClearanceExpenses(shipmentId);
        });

        ipcMain.handle('import-get-clearance-expense', async (event, id) => {
            return this.getClearanceExpense(id);
        });

        ipcMain.handle('import-save-clearance-expense', async (event, data) => {
            return this.saveClearanceExpense(data);
        });

        ipcMain.handle('import-delete-clearance-expense', async (event, id) => {
        ...
        // ============================================================================
        ipcMain.handle('import-get-shipment-documents', async (event, shipmentId) => {
            return this.getShipmentDocuments(shipmentId);
        });

        ipcMain.handle('import-save-shipment-document', async (event, data) => {
            return this.saveShipmentDocument(data);
        });

        ipcMain.handle('import-delete-shipment-document', async (event, id) => {
        ...
        // ============================================================================
        ipcMain.handle('import-calculate-landed-cost', async (event, shipmentId, method) => {
            return this.calculateLandedCost(shipmentId, method);
        });

        ipcMain.handle('import-apply-landed-cost', async (event, shipmentId, allocations) => {
            return this.applyLandedCost(shipmentId, allocations);
        });

        ipcMain.handle('import-get-landed-cost-history', async (event, shipmentId) => {
        ...
        // ============================================================================
        ipcMain.handle('import-get-shipment-cost-breakdown', async (event, shipmentId) => {
            return this.getShipmentCostBreakdown(shipmentId);
        });

        ipcMain.handle('import-get-containers-near-demurrage', async (event, days) => {
            return this.getContainersNearDemurrage(days);
        });

        ipcMain.handle('import-get-item-cost-comparison', async (event, itemId) => {
    ...
    private allocateCosts(shipmentId: string, expenses: any[], method: string) {
        ...
        if (!items || items.length === 0) throw new Error("No items in shipment to allocate costs to");
        ...
        const allocatableExpenses = dbExpenses.filter((e: any) => e.is_allocatable).reduce((sum: any, e: any) => sum + (e.amount_local || 0), 0);

        if (totalFOB === 0) throw new Error("Total FOB value is 0, cannot allocate");
    ...
    // --- Shipments Logic ---
    public getShipments(filters: any) {
        let query = `
    ...
    private getShipmentById(id: string) {
    ...
    private getShipmentItems(shipmentId: string) {
    ...
    private deleteContainer(id: string) {
        this.db.prepare(`DELETE FROM shipment_containers WHERE id = ?`).run(id);
        return { success: true };
    }

    // --- Expenses Logic ---
    private getExpenses(shipmentId: string) {
    ...
    // --- Proforma Logic ---
    private getProformas(filters: any) {
        let query = `
    ...
    private getCommercialInvoices(shipmentId: string) {
    ...
    private getAllCommercialInvoices() {
        return this.db.prepare(`
    ...
    private getCommercialInvoice(id: string) {
    ...
    private saveCommercialInvoice(data: any) {
        const { header, lines } = data;
        const id = header.id || crypto.randomUUID();
        const isNew = !header.id;
    ...
    private deleteCommercialInvoice(id: string) {
    ...
    private getClearanceExpenses(shipmentId: string) {
        return this.db.prepare(`
    ...
    private getClearanceExpense(id: string) {
        return this.db.prepare('SELECT * FROM clearance_expenses WHERE id = ?').get(id);
    }

    private saveClearanceExpense(data: any) {
        ...
        if (isNew) {
            ...
            `).run(
                ...
                data.description,
                data.amount,
                data.currency_id || 'ILS',
                data.exchange_rate || 1,
                data.allocation_method || 'VALUE',
                data.payment_method,
                data.paid_to,
    ...
    private deleteClearanceExpense(id: string) {
    ...
    private getShipmentDocuments(shipmentId: string) {
        return this.db.prepare(`
    ...
    private saveShipmentDocument(data: any) {
    ...
    private deleteShipmentDocument(id: string) {
    ...
    private calculateLandedCost(shipmentId: string, method: string) {
        ...
        const itemsWithAllocation = items.map((item: any) => {
            ...
            if (method === 'VALUE') {
                ...
                const totalWeight = items.reduce((sum: number, i: any) => sum + (i.weight_kg || 0), 0);
                allocationPercentage = totalWeight > 0 ? ((item.weight_kg || 0) / totalWeight) * 100 : 0;
            } else if (method === 'VOLUME') {
                const totalVolume = items.reduce((sum: number, i: any) => sum + (i.volume_cbm || 0), 0);
    ...
    private applyLandedCost(shipmentId: string, allocations: any[]) {
        ...
        const runTx = this.db.transaction(() => {
            ...
            `).run(
                allocationId,
                shipmentId,
                allocations[0]?.allocation_method || 'VALUE',
                totalGoodsValue,
                totalExpenses,
                totalLandedCost,
            ...
            allocations.forEach((allocation: any) => {
                ...
                insertDetail.run(
                    ...
                    allocation.item_id,
                    allocation.quantity,
                    allocation.fob_value,
                    allocation.weight_kg || 0,
                    allocation.volume_cbm || 0,
                    allocation.allocation_percentage,
                    allocation.allocated_expense,
                    allocation.old_cost,
                    allocation.new_cost
                );
    ...
    private getLandedCostHistory(shipmentId: string) {
    ...
    private getShipmentCostBreakdown(shipmentId: string) {
        ...
        const totalGoods = commercialInvoices.reduce((sum: number, inv: any) =>
            sum + (inv.total_amount * inv.exchange_rate), 0);

        const totalExpenses = clearanceExpenses.reduce((sum: number, exp: any) =>
            sum + (exp.amount_base_currency || 0), 0);
    ...
    private getContainersNearDemurrage(days: number = 3) {
    ...
    private getItemCostComparison(itemId: string) {
        return this.db.prepare(`
            SELECT 
                lcad.*, 
                lca.allocation_date,
