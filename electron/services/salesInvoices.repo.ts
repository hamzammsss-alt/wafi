...
import { ipcWrap } from '../ipc/ipcWrapper';
import { computeSalesInvoiceTotals } from '../utils/documentTotals';
import { validateSalesInvoice } from '../utils/documentValidation';
import { getContext } from '../../src/main/ipc/AuthContext';
...
const SALES_INVOICE_POST_PERMISSION_KEYS = ['sales.invoice.post', 'DOC.POST'] as const;
const SALES_INVOICE_REOPEN_PERMISSION_KEYS = ['sales.invoice.update', 'DOC.REOPEN_REJECTED'] as const;

function hasAnyPermission(perms: string[], keys: readonly string[]): boolean {
...
export class SalesInvoiceRepo {
    ...
    static createDraft(userId = 'admin', auditCtx?: AuditContext | null) {
        const id = uuidv4();
        let no = '';
    ...
    static save(params: { id: string; header: any; lines: any[]; userId?: string }, auditCtx?: AuditContext | null) {
        const { id, header, lines } = params;

        const existing: any = db.prepare('SELECT status, version FROM sales_invoices WHERE id = ?').get(id);
        ...
        const computed = computeSalesInvoiceTotals(lines);
        ...
        db.transaction(() => {
            ...
            `).run(
                header.customer_id ?? null, header.customer_name ?? null,
                header.doc_date ?? header.date ?? new Date().toISOString().split('T')[0],
                header.doc_date ?? header.date ?? new Date().toISOString().split('T')[0],
                header.currency_id ?? null, header.exchange_rate ?? 1,
                computed.totals.subtotal, computed.totals.tax_total, computed.totals.grand_total,
                header.notes ?? null, id, existing.version
            );
            ...
            computed.lines.forEach((l: any, idx: number) => {
                insertLine.run(
                    uuidv4(), id, idx + 1, l.item_id || null, l.item_name || l.description || '',
                    l.qty || 0, l.price || 0, l.discount || 0, l.tax_rate || 0,
                    l.total_price, l.tax_amount, l.net_total
                );
            });
        })();
    ...
    static validate(id: string) {
        ...
        const errors = validateSalesInvoice(doc.header, doc.lines);
        return { errors };
    }

    static postOrSubmit(params: { id: string; userId?: string; perms?: string[] }, auditCtx?: AuditContext | null) {
        const { id, userId = 'admin', perms = [] } = params;
        const doc: any = db.prepare('SELECT status FROM sales_invoices WHERE id = ?').get(id);
        ...
        if (hasPost) {
            ...
            if (Number(posted.changes || 0) === 0) {
                const current: any = db.prepare('SELECT status FROM sales_invoices WHERE id = ?').get(id);
    ...
    static reopenRejected(params: { id: string; userId?: string; perms?: string[] }, auditCtx?: AuditContext | null) {
        const { id, userId = 'admin', perms = [] } = params;
    ...
    static voidInvoice(params: { id: string; userId?: string }, auditCtx?: AuditContext | null) {
        const { id, userId = 'admin' } = params;
        const doc: any = db.prepare('SELECT status FROM sales_invoices WHERE id = ?').get(id);
        if (!doc) throw Object.assign(new Error('Invoice not found'), { code: 'DOCUMENT_NOT_FOUND' });
        if (doc.status === 'VOID') return { status: 'VOID' };
    ...
    static registerHandlers() {
        ipcMain.handle('salesInvoices:listKeyset', ipcWrap((_, params) => SalesInvoiceRepo.listKeyset(params)));
        ipcMain.handle('salesInvoices:get', ipcWrap((_, id: string) => SalesInvoiceRepo.get(id)));
        ipcMain.handle('salesInvoices:createDraft', ipcWrap((event, userId?: string) => {
            const auditCtx = createAuditContext(event as any, userId || undefined);
            return SalesInvoiceRepo.createDraft(userId, auditCtx);
        }));
        ipcMain.handle('salesInvoices:save', ipcWrap((event, params) => {
        ...
        ipcMain.handle('salesInvoices:validate', ipcWrap((_, id: string) => SalesInvoiceRepo.validate(id)));
        ipcMain.handle('salesInvoices:postOrSubmit', ipcWrap((event, params) => {
            const auditCtx = createAuditContext(
                event as any,
                params?.userId || undefined,
                params?.correlationId,
        ...
        ipcMain.handle('salesInvoices:reopenRejected', ipcWrap((event, params) => {
            const auditCtx = createAuditContext(event as any, params?.userId || undefined);
            return SalesInvoiceRepo.reopenRejected(params, auditCtx);
        }));
        ipcMain.handle('salesInvoices:void', ipcWrap((event, params) => {
            const auditCtx = createAuditContext(event as any, params?.userId || undefined);
