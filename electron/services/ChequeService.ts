...
export class ChequeService {
    ...
    static getCheques(filters: { status?: string, type?: 'INCOMING' | 'OUTGOING' }) {
        let query = `
    ...
    static updateStatus(id: string, newStatus: string, date: string, options: { bankAccountId?: string, notes?: string, endorsedTo?: string } = {}) {
        const check = db.prepare('SELECT * FROM cheques WHERE id = ?').get(id);
        if (!check) throw new Error("Check not found");

        if (check.status === newStatus) return { success: true, message: "No change" };

        const runUpdate = db.transaction(() => {
            // 1. Update Status
            db.prepare('UPDATE cheques SET status = ?, endorsed_to = ? WHERE id = ?').run(newStatus, options.endorsedTo || check.endorsed_to, id);
            ...
            const getAccount = (namePattern: string) => {
                const acc = db.prepare(`SELECT id FROM accounts WHERE name_ar LIKE ? OR name_en LIKE ?`).get(`%${namePattern}%`, `%${namePattern}%`);
                return acc ? acc.id : null;
            };

            const getPartnerAccount = (partnerId: string) => {
                const partner = db.prepare('SELECT linked_account_id FROM business_partners WHERE id = ?').get(partnerId);
            ...
            if (debitAcc && creditAcc) {
                JournalService.createJournalEntry({
                    ...
                    branch_id: db.prepare('SELECT id FROM branches LIMIT 1').get()?.id, // Main branch for now
                    status: 'POSTED'
                }, [
