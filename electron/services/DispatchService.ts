...
type DispatchStatus = "محفوظ" | "عالق" | "مرحل";

function calcDispatchStatus(header: any, lines: any[]): DispatchStatus {
    ...
    if (header.status === "مرحل") return "مرحل";
    if (header.status === "عالق") return "عالق";
    return "محفوظ";
}
...
export class DispatchService {
    ...
    static async getById(id: string): Promise<any> {
        ...
        `).all(header.from_warehouse_id, id) as any[];
    ...
    static async update(id: string | null, payload: any): Promise<any> {
        return db.transaction(() => {
            ...
            const status: DispatchStatus = calcDispatchStatus(header, lines);

            if (currentId) {
                // Update
                const existing = db.prepare(`SELECT status FROM dispatch_header WHERE id = ?`).get(currentId) as any;
                if (!existing) throw new Error("السند غير موجود");
                if (existing.status === "مرحل") throw new Error("لا يمكن تعديل سند مرحّل");
                if (existing.status === "عالق") throw new Error("السند عالق بانتظار الفاتورة ولا يمكن تعديله");
                ...
                // replace lines...
                db.prepare(`DELETE FROM dispatch_lines WHERE header_id=?`).run(currentId);
            } else {
                // Create
                currentId = uuidv4();

                // Gen serial
                const countQuery = db.prepare(`SELECT COUNT(*) as count FROM dispatch_header`).get() as any;
    ...
    static async postToPending(id: string): Promise<void> {
        return db.transaction(() => {
            ...
            if (header.status === "مرحل") throw new Error("السند مرحّل نهائياً");
            if (header.status === "عالق") throw new Error("السند عالق مسبقاً");

            const lines = db.prepare(`SELECT * FROM dispatch_lines WHERE header_id=?`).all(id) as any[];
            ...
            if (header.source_type === 'SALES_ORDER' && header.source_id) {
                ...
                const updateOrderLine = db.prepare(`
                ...
                const orderLines = db.prepare(`SELECT quantity, dispatched_qty FROM sales_order_lines WHERE order_id = ?`).all(header.source_id) as any[];
            ...
            for (const l of lines) {
                const stock = db.prepare(`SELECT quantity FROM stock_balances WHERE item_id = ? AND warehouse_id = ?`).get(l.item_id, header.from_warehouse_id) as any;
            ...
            for (const l of lines) {
                ...
                const stockInfo = InventoryService.getStock(l.item_id, header.from_warehouse_id);
                // @ts-ignore
                ...
                InventoryService.updateStock(
                    l.item_id,
                    qtyToIssue, // positive number is out
                    'OUT',
                    refNo,
                    'Dispatch (Delivery)',
                    cost,
            ...
            if (totalCogs > 0) {
                const cogsAccount = db.prepare("SELECT id FROM accounts WHERE name LIKE '%تكلفة البضاعة المباعة%' OR name LIKE '%COGS%'").get() as any;
                const invAccount = db.prepare("SELECT id FROM accounts WHERE name LIKE '%مخزون%' OR name LIKE '%Inventory%'").get() as any;
    ...
    static async invoiceFromDispatch(dispatchId: string): Promise<string> {
        return db.transaction(() => {
            ...
            const lines = db.prepare(`SELECT * FROM dispatch_lines WHERE header_id=?`).all(dispatchId) as any[];
            ...
            `).run({
                ...
                branch_id: db.prepare("SELECT id FROM branches WHERE is_main = 1").get()?.id || 'MAIN',
