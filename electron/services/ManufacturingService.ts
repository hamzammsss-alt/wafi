...
export class ManufacturingService {
    ...
    static getWorkCenters() {
        return db.prepare('SELECT * FROM mfg_work_centers ORDER BY name').all();
    }

    static saveWorkCenter(data: any) {
        const isNew = !data.id;
    ...
    static deleteWorkCenter(id: string) {
        db.prepare('DELETE FROM mfg_work_centers WHERE id = ?').run(id);
        return { success: true };
    }
    ...
    static deleteMachine(id: string) {
        db.prepare('DELETE FROM mfg_machines WHERE id = ?').run(id);
        return { success: true };
    }
    ...
    // --- BOM ---
    static createBOM(header: any, lines: any[]) {
        const id = uuidv4();
        // BOM-001 format
        const bom_number = JournalService.getNextVoucherNo('BOM');
        ...
        const tx = db.transaction(() => {
            ...
            JournalService.incrementVoucherNo('BOM');
        });
    ...
    // --- Routing ---
    static saveRouting(header: any, operations: any[]) {
        const id = header.id || uuidv4();
        const isNew = !header.id;

        const tx = db.transaction(() => {
            if (isNew) {
                db.prepare(`INSERT INTO mfg_routings (id, bom_id, name) VALUES (@id, @bom_id, @name)`).run({
                    id, bom_id: header.bom_id, name: header.name
                });
            } else {
                db.prepare(`UPDATE mfg_routings SET name = @name WHERE id = @id`).run({
                    id, name: header.name
                });
                // Delete old ops to replace
                db.prepare('DELETE FROM mfg_routing_operations WHERE routing_id = ?').run(id);
            }
    ...
    static getRoutings(bomId: string) {
        const routings = db.prepare('SELECT * FROM mfg_routings WHERE bom_id = ?').all(bomId);
    ...
    static createProductionOrder(order: any) {
        const id = uuidv4();
        const order_number = JournalService.getNextVoucherNo('MFG');
        ...
        JournalService.incrementVoucherNo('MFG');
    ...
    static updateOrderStatus(id: string, status: string) {
        db.prepare('UPDATE mfg_production_orders SET status = ? WHERE id = ?').run(status, id);
        return { success: true };
    }
    ...
    static executeProductionOrder(orderId: string, actualQuantity: number, date: string) {
        const tx = db.transaction(() => {
            const order = db.prepare('SELECT * FROM mfg_production_orders WHERE id = ?').get(orderId);
            if (!order) throw new Error("Order not found");
            if (order.status === 'COMPLETED') throw new Error("Already completed");

            // 1. Get BOM Components
            const components = db.prepare('SELECT * FROM mfg_bom_components WHERE bom_id = ?').all(order.bom_id);
            if (components.length === 0) throw new Error("BOM has no components");

            const bomHeader = db.prepare('SELECT batch_size FROM mfg_boms WHERE id = ?').get(order.bom_id);
            ...
            for (const comp of components) {
                ...
                const stock = InventoryService.getStock(comp.item_id, order.warehouse_id);
                ...
                InventoryService.updateStock(
                    comp.item_id,
                    finalQty, // Deducting quantity
                    'OUT',
                    order.order_number,
                    `Consumed for MFG ${order.order_number}`,
                    unitCost,
                    order.warehouse_id
                );
            }
            ...
            InventoryService.updateStock(
                ...
                'IN',
                order.order_number,
                `Produced from MFG ${order.order_number}`,
                unitCost,
                order.warehouse_id
            );
    ...
    static getJobCards(filters: any = {}) {
        let query = `
            SELECT jc.*, 
                   po.order_number, 
    ...
    static stopJobCard(id: string, data: any) {
        db.prepare(`
    ...
    static getQCTests() {
        return db.prepare('SELECT * FROM mfg_qc_tests').all();
    }

    static saveQCTest(data: any) {
        const id = data.id || uuidv4();
        if (data.id) {
            db.prepare(`
                UPDATE mfg_qc_tests 
    ...
    static getInspections(filters: any = {}) {
    ...
    static saveInspection(data: any) {
        const id = uuidv4();
        db.prepare(`
    ...
    static getMaintenanceRequests(filters: any = {}) {
        let query = `
    ...
    static saveMaintenanceRequest(data: any) {
        const id = data.id || uuidv4();
        const request_number = JournalService.getNextVoucherNo('MAINT');
        ...
        if (data.id) {
            ...
            JournalService.incrementVoucherNo('MAINT');
        }
        return { success: true, id };
    }
