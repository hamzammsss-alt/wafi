...
export class LeaveService {
    ...
    static getLeaveTypes() {
        return db.prepare('SELECT * FROM hr_leave_types ORDER BY name').all();
    }

    static saveLeaveType(data: any) {
    ...
    static deleteLeaveType(id: string) {
    ...
    static getLeaveRequests(filter: any = {}) {
        let query = `
            SELECT r.*, 
    ...
    static saveLeaveRequest(data: any) {
    ...
    static updateRequestStatus(id: string, status: string, rejection_reason: string = '') {
        db.prepare('UPDATE hr_leave_requests SET status = ?, rejection_reason = ? WHERE id = ?').run(status, rejection_reason, id);
        return { success: true };
    }
    ...
    static getEmployeeBalances(employeeId: string, year: number) {
        // 1. Get all configured Leave Types
        const types = db.prepare('SELECT * FROM hr_leave_types').all();
        ...
        types.forEach((t: any) => resultMap.set(t.id, {
            type_id: t.id,
            type_name: t.name,
